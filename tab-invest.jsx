...
const INV_VERDICT_TIP = {
  ATTRACTIVE: "Profitable, growing, and the earnings yield clears the 10-year " +
    "Treasury by the cushion set in the configuration. This is the tab saying " +
    "the four questions all answered yes — not a recommendation, and not a score.",
  WATCH: "Worth following. Either the valuation is close but not yet at the " +
    "cushion, or the valuation is fine and something in the business is not.",
  WAIT: "Nothing wrong with the business, but you are being asked to pay more " +
    "than this dashboard's yield rule accepts. The price and the earnings " +
    "figure that WOULD change the answer are both stated below.",
  AVOID: "A gate failed outright — the company is losing money, or revenue is " +
    "shrinking while analysts cut. The specific reason is listed.",
    ...
    <div className="card inv-card">
      <div className="card-head">
        <div>
          <div className="card-title" title="A long-horizon view of the business
            behind the ticker: what it earns, whether that is growing, and what
            you are being asked to pay for it. Everything here comes from the
            company's own filings with the SEC unless a line says otherwise.">
            Investment · is this business worth owning
          </div>
          <div className="card-sub">
            Reported fundamentals from SEC EDGAR, priced against the live
            10-year Treasury yield. No score, no rating — one of five words
      ...
      {data && d.ok && (
        <div className="inv-body">
          ...
          <div className="inv-hero">
            <div className="inv-hero-top">
              <div className="inv-hero-name">
                <b title="Company name exactly as it registers with the SEC.">
                  {d.entity_name || sym}</b>
                <span className="muted" title={`SEC Central Index Key ${d.cik} — the
                  company's permanent identifier in EDGAR.`}> · {sym}</span>
              </div>
            ...
            {(v.what_would_change || []).length > 0 && (
              <div className="inv-change" title="Every WAIT or AVOID says what
                would have to change. The price is simply the price at which the
                earnings yield reaches the threshold in the configuration; the
                earnings figure is the same sum solved the other way round.">
          ...
          {/* ── 1. the business ── */}
          <div className="inv-sechead" title="What the company says it does, in
            its own words, taken from Item 1 of its most recent annual report.
            No language model is involved at any point — this is quoted, and the
            filing it came from is one click away.">
            The business
          </div>
          {d.profile ? (
            <div className="inv-desc">
              {d.profile.description}
              <div className="inv-src" title={`Quoted from Item 1, Business, of the
            ...
            <div className="research-empty" title="The business description is
              quoted from Item 1 of a 10-K. When no annual report can be read for
              this filer, nothing is substituted.">
              No annual report business section could be read for {sym}.
            </div>
          )}

          {/* ── core snapshot ── */}
          <div className="inv-sechead" title="The numbers that answer the first
            three questions. Each carries its source, its as-of date and its
            basis; hover any of them.">
            Core snapshot
          </div>
          <div className="inv-grid">
            <InvStat label="Share price" value={invPrice(d.price)}
                     prov={prov.price}
            ...
            <InvStat label="Market value of the company" value={invMoney(d.market_cap)}
                     ...
                     tip="Share price multiplied by the share count on the cover
                       page of the latest filing. Computed here from two sourced
                       numbers rather than taken from a vendor, so both halves
                       can be checked." />
            <InvStat label="Price to earnings, trailing" value={invRatio(d.trailing_pe)}
                     ...
                     tip="Share price divided by the last twelve months of
                       reported GAAP diluted earnings per share. Shown because
                       everyone reads it; the earnings YIELD below is what the
                       verdict actually uses, because it stays meaningful when
            ...
            <InvStat label="Price to earnings, forward" value={invRatio(d.forward_pe)}
                     ...
                     tip="Share price divided by the analyst consensus estimate
                       for this fiscal year. Analyst estimates are on an adjusted
                       (non-GAAP) basis and are NEVER mixed with the GAAP
                       trailing figures in the same ratio." />
            <InvStat label="Revenue, last twelve months" value={invMoney(d.revenue_ttm, 0)}
                     ...
                     tip="Total revenue over the last four reported quarters,
                       rebuilt from the company's own quarterly filings." />
            <InvStat label="Revenue growth, year over year"
                     ...
                     tip="This twelve-month revenue against the same twelve
                       months a year earlier. When the year-ago figure was zero
                       or negative, no percentage is shown — a percentage change
                       from a loss is not a meaningful number, and the words
                       describing what happened appear instead." />
            <InvStat label="Earnings per share, last twelve months"
                     value={invPrice(d.eps_ttm)}
                     prov={prov.fundamentals} basis={(md.eps || {}).basis}
                     asOf={(md.eps || {}).period_end} reason={reasonOf("eps")}
                     tone={d.eps_ttm == null ? "" : (d.eps_ttm >= 0 ? "" : "down")}
                     tip="Reported GAAP diluted earnings per share, summed over
                       the last four quarters. This is what the company signed
                       and filed, not an adjusted figure." />
            ...
            <InvStat label="This year's earnings estimate" value={invPrice(d.eps_forward)}
                     prov={prov.estimates}
                     reason={d.estimates_available ? "" : (prov.estimates || {}).reason || d.estimates_reason}
                     tip="What analysts currently expect the company to earn per
                       share this fiscal year. Adjusted (non-GAAP) basis." />
            <InvStat label="Next year's earnings estimate" value={invPrice(d.eps_next_year)}
                     prov={prov.estimates}
                     reason={d.estimates_available ? "" : (prov.estimates || {}).reason || d.estimates_reason}
                     tip="What analysts expect for the following fiscal year." />
            <InvStat label="Forward earnings growth"
                     ...
                     tip="Next year's estimate against this year's — both on the
                       same analyst basis, so the comparison is like for like." />
            <InvStat label="Estimate revisions, last 30 days"
                     value={invSignedPct(d.estimate_change_30d_pct)}
                     tone={d.estimate_change_30d_pct == null ? "" : (d.estimate_change_30d_pct >= 0 ? "up" : "down")}
                     prov={prov.estimates}
                     reason={d.estimates_available ? "" : (prov.estimates || {}).reason || d.estimates_reason}
                     tip="The net share of covering analysts who RAISED this
                       year's estimate rather than cut it, over the last 30 days.
                       This is revision breadth, not a percentage change in the
                       estimate itself: the free provider publishes revision
                       counts, not an archive of past estimates, and this tab
                       will not present one as the other." />
            <InvStat label="Earnings yield" value={invPct(d.earnings_yield_pct)}
                     prov={prov.fundamentals}
                     reason={reasonOf("eps")}
                     tone={d.earnings_yield_pct == null || d.treasury_10y_pct == null ? ""
                       : (d.earnings_yield_pct >= d.treasury_10y_pct ? "up" : "down")}
                     tip="Trailing earnings per share divided by the share price
                       — the P/E turned upside down. This is the number the
                       verdict compares against the Treasury yield, because a
                       yield can be set beside a bond yield and a multiple
                       cannot." />
            <InvStat label="Free cash flow yield" value={invPct(d.fcf_yield_pct)}
                     ...
                     tone={d.fcf_yield_pct == null || d.fcf_yield_pct >= 0 ? ""
                       : "down"}
                     tip="Cash from operations minus capital spending over the
                       last twelve months, divided by the market value of the
                       company. Cash is harder to shape than accounting
                       earnings, which is why this sits next to the earnings
                       yield rather than instead of it. Banks and some property
                       companies do not report capital spending in a comparable
                       way, and show N/A rather than a made-up number." />
            <InvStat label="10-year Treasury yield" value={invPct(d.treasury_10y_pct, 2)}
                     prov={prov.treasury_10y}
                     tip="The yield on a 10-year US Treasury note — what you can
                       earn without owning a business at all. Every valuation
                       judgement on this tab is made relative to it, from the
                       same official daily curve the Treasuries tab draws." />
          </div>
          ...
          <div className="inv-subgrid">
            <InvStat label="Net profit margin" value={invPct(d.net_margin_pct)}
                     ...
                     tip="What share of each dollar of revenue survives as
                       profit, over the last twelve months." />
            <InvStat label="Net income, last twelve months" value={invMoney(d.net_income_ttm, 0)}
                     prov={prov.fundamentals} reason={reasonOf("net_income")}
                     tip="Reported GAAP net income over the last four quarters." />
            <InvStat label="Free cash flow, last twelve months"
                     value={invMoney(d.free_cash_flow_ttm, 0)}
                     prov={prov.fundamentals}
                     reason={(d.free_cash_flow_detail || {}).reason}
                     tip="Cash from operations minus capital spending. Quarterly
                       cash-flow figures are reported year-to-date rather than
                       per quarter, so each quarter is recovered by subtracting
                       the previous one before the four are added up." />
            <InvStat label="Shares outstanding" value={invCount(d.shares_outstanding)}
                     prov={prov.fundamentals}
                     basis={(d.shares_detail || {}).basis}
                     reason={(d.shares_detail || {}).reason}
                     tip="The share count from the cover page of the most recent
                       filing. Companies with several share classes report it per
                       class, which the SEC's machine-readable feed does not
                       carry; those fall back to the average diluted count and
                       say so." />
            ...
            <InvStat label="Latest reported period"
                     value={d.period_end ? invDate(d.period_end) : invNA}
                     prov={prov.fundamentals}
                     tip="The end of the most recent quarter included in every
                       trailing figure above, and the date its filing reached the
                       SEC." asOf={d.last_filed} />
          </div>

          {/* ── 2. what moved earnings ── */}
          <InvDrivers drivers={d.drivers} />

          {/* ── 3. price against earnings ── */}
          <InvHistoryChart history={d.history} years={years} onYears={setYears}
                           symbol={sym} />

          <div className="inv-foot" title={`This tab stores one snapshot of these
