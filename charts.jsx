...
function PriceChart({ daily, expHigh, expLow, emHigh, emLow, callStrike, putStrike, currentPrice, chartStyle = "candles", colors, earnings, showMA50 = false, showMA200 = false, showEMA21 = false, showRSI = false, showProbCone = false, ivAnnualized = null, dteToExp = null, fullDailyLength = null, visibleStart = 0, onViewRangeChange = null }) {
  ...
  useEffect(() => { setHover(null); }, [daily.length, daily[0]?.date]);
  ...
  const onWheelImpl = (e) => {
    ...
    if (newEnd > fullDailyLength) { newStart -= (newEnd - fullDailyLength); newEnd = fullDailyLength; }
  ...
  useEffect(() => {
    ...
    if (!node) return;
    node.addEventListener("wheel", onWheelImpl, { passive: false });
  ...
  const xTicks = useMemo(() => {
    ...
    for (let i = 0; i <= n; i++) {
      ...
      if (daily[idx]) out.push({ idx, date: daily[idx].date });
    }
    return out;
  }, [daily]);
    ...
    return (
      <div className="chart-wrap">
        <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg" >
          <text x={W/2} y={H/2} textAnchor="middle" fill={colors?.fg3 || "#888"} fontSize="12" fontFamily="ui-monospace, monospace">
  ...
  return (
    <div className="chart-wrap" onMouseLeave={() => setHover(null)}>
      <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} className={`chart-svg ${pan ? "is-panning" : ""}`}  onMouseDown={onChartMouseDown}>
        {ticks.map((t, i) => (
          <g key={i}>
            <line x1={padL} x2={W - padR} y1={yScale(t)} y2={yScale(t)} className="grid" />
            <text x={padL - 8} y={yScale(t) + 4} className="axis-text" textAnchor="end">${t.toFixed(2)}</text>
          </g>
        ))}
        {xTicks.map((t, i) => (
          <text key={i} x={xScale(t.idx)} y={H - 10} className="axis-text" textAnchor="middle">{fmtDate(t.date)}</text>
        ))}

        {expHigh && expLow && (
          <g>
            <rect x={padL} y={yScale(expHigh)} width={innerW} height={yScale(expLow) - yScale(expHigh)}
        ...
        {chartStyle === "area" && (() => {
          ...
          return (
            <g>
              <path d={fill} fill={colors.up} opacity="0.12" />
              <path d={path} fill="none" stroke={colors.up} strokeWidth="1.8" />
            </g>
          );
        })()}
        {chartStyle === "candles" && daily.map((d, i) => {
          ...
          return (
            <g key={i}>
              <line x1={x} x2={x} y1={yScale(d.high)} y2={yScale(d.low)} stroke={up ? colors.up : colors.down} strokeWidth="1" />
        ...
        {chartStyle === "ohlc" && daily.map((d, i) => {
          ...
          const w = candleW * 0.6;
          return (
            <g key={i} stroke={up ? colors.up : colors.down} strokeWidth="1.2">
              <line x1={x} x2={x} y1={yScale(d.high)} y2={yScale(d.low)} />
              <line x1={x - w} x2={x} y1={yO} y2={yO} />
              <line x1={x} x2={x + w} y1={yC} y2={yC} />
            </g>
          );
        })}

        {/* Moving averages — render after candles so they sit on top */}
        {(showEMA21 || showMA50 || showMA200) && (() => {
          ...
          if (showMA50 && showMA200) {
            for (let i = 1; i < daily.length; i++) {
              ...
              const wasBelow = a50 < a200, nowAbove = b50 > b200;
              const wasAbove = a50 > a200, nowBelow = b50 < b200;
              if (wasBelow && nowAbove) crosses.push({ idx: i, type: "golden", price: daily[i].close });
              else if (wasAbove && nowBelow) crosses.push({ idx: i, type: "death", price: daily[i].close });
            }
          }
          return (
            <g>
              {showEMA21 && (
                <path d={buildPath("ema21")} fill="none" stroke="rgb(80, 160, 255)"
                      strokeWidth="1.5" opacity="0.9" />
              )}
              {showMA50 && (
                <path d={buildPath("ma50")} fill="none" stroke={colors.warn}
                      strokeWidth="1.6" opacity="0.85" />
              )}
              {showMA200 && (
                <path d={buildPath("ma200")} fill="none" stroke={colors.accent}
              ...
              {crosses.map((c, k) => {
                ...
                return (
                  <g key={`mx${k}`}>
                    <circle cx={cx} cy={cy} r="6" fill={fill} stroke="white" strokeWidth="1.5" opacity="0.95" />
                    <rect x={cx + 8} y={cy - 14} width="58" height="14" rx="3"
                          fill={fill} opacity="0.18" stroke={fill} strokeOpacity="0.55" />
                    <text x={cx + 12} y={cy - 4} fontSize="9" fontWeight="700"
                          fill={fill} fontFamily="ui-monospace, monospace"
        ...
        {/* Probability cone — Black-Scholes-derived ±1σ / ±2σ price fan
            projecting from the most recent bar to the chart's right edge.
            Width grows with sqrt(t/T_exp). Useful for visualizing where
            the suggested strikes sit relative to expected dispersion. */}
        {showProbCone && ivAnnualized && dteToExp > 0 && currentPrice > 0 && daily.length > 0 && (() => {
          ...
          const sample = (sigmaMult) => {
            ...
            const path =
              ...
              upper.slice(1).map(p => `L ${p[0]} ${p[1]}`).join(" ") + " " +
              lower.slice().reverse().map(p => `L ${p[0]} ${p[1]}`).join(" ") +
              " Z";
            ...
            return { fill: path, upperLine, lowerLine, lastUpper: upper[upper.length - 1], lastLower: lower[lower.length - 1] };
          };
          const s2 = sample(2);
          const s1 = sample(1);
          ...
          return (
            <g>
              <line x1={xStart} y1={padT} x2={xStart} y2={padT + priceH}
                    stroke={colors.accent} strokeWidth="1" strokeDasharray="2 4" opacity="0.3" />
              <path d={s2.fill} fill={colors.accent} fillOpacity="0.07" stroke="none" />
              <path d={s1.fill} fill={colors.accent} fillOpacity="0.13" stroke="none" />
              <path d={s2.upperLine} fill="none" stroke={colors.accent} strokeWidth="1" strokeDasharray="3 3" opacity="0.55" />
              <path d={s2.lowerLine} fill="none" stroke={colors.accent} strokeWidth="1" strokeDasharray="3 3" opacity="0.55" />
              <path d={s1.upperLine} fill="none" stroke={colors.accent} strokeWidth="1.4" opacity="0.85" />
              <path d={s1.lowerLine} fill="none" stroke={colors.accent} strokeWidth="1.4" opacity="0.85" />
              <line x1={xStart} y1={yScale(currentPrice)} x2={xEnd} y2={yScale(currentPrice)}
                    stroke={colors.accent} strokeWidth="1" strokeDasharray="2 3" opacity="0.4" />
              {/* Labels at the right edge */}
              <text x={s2.lastUpper[0] - 4} y={s2.lastUpper[1] - 3} fontSize="10" textAnchor="end"
                    fill={colors.accent} fontFamily="ui-monospace, monospace" opacity="0.7">+2σ</text>
              <text x={s1.lastUpper[0] - 4} y={s1.lastUpper[1] - 3} fontSize="10" textAnchor="end"
                    fill={colors.accent} fontFamily="ui-monospace, monospace" opacity="0.85">+1σ</text>
              <text x={s1.lastLower[0] - 4} y={s1.lastLower[1] + 11} fontSize="10" textAnchor="end"
                    fill={colors.accent} fontFamily="ui-monospace, monospace" opacity="0.85">-1σ</text>
              <text x={s2.lastLower[0] - 4} y={s2.lastLower[1] + 11} fontSize="10" textAnchor="end"
                    fill={colors.accent} fontFamily="ui-monospace, monospace" opacity="0.7">-2σ</text>
            </g>
          );
        })()}
        ...
        {callStrike > 0 && (
          <g>
            <line x1={padL} x2={W - padR} y1={yScale(callStrike)} y2={yScale(callStrike)}
                  stroke={colors.up} strokeWidth="1.5" strokeDasharray="6 4" opacity="0.85" />
            <text x={W - padR - 6} y={yScale(callStrike) - 6} className="strike-label" textAnchor="end" fill={colors.up}>
              CALL ${callStrike.toFixed(2)}
            </text>
          </g>
        )}
        {putStrike > 0 && (
          <g>
            <line x1={padL} x2={W - padR} y1={yScale(putStrike)} y2={yScale(putStrike)}
                  stroke={colors.down} strokeWidth="1.5" strokeDasharray="6 4" opacity="0.85" />
            <text x={W - padR - 6} y={yScale(putStrike) + 14} className="strike-label" textAnchor="end" fill={colors.down}>
              PUT ${putStrike.toFixed(2)}
            </text>
          </g>
        )}
        ...
        {currentPrice && (
          <g>
            <line x1={padL} x2={W - padR} y1={yScale(currentPrice)} y2={yScale(currentPrice)}
                  stroke={colors.accent} strokeWidth="1" opacity="0.5" />
            <rect x={W - padR - 60} y={yScale(currentPrice) - 9} width="58" height="18" fill={colors.accent} rx="3" />
            <text x={W - padR - 31} y={yScale(currentPrice) + 4} className="strike-label" textAnchor="middle" fill={colors.accentText}>
        ...
        {earningsMarkers.map((m, i) => (
          <g key={`erm${i}`}>
            <line x1={xScale(m.idx)} x2={xScale(m.idx)} y1={padT} y2={padT + innerH}
                  stroke={colors.warn} strokeOpacity="0.35" strokeDasharray="2 4" strokeWidth="1" />
            <g transform={`translate(${xScale(m.idx)}, ${padT + 4})`}>
              <circle r="9" fill={colors.warn} opacity="0.95" />
              <text x="0" y="4" textAnchor="middle" fontSize="11" fontWeight="700"
                    fill="white" fontFamily="ui-sans-serif, system-ui">E</text>
            </g>
          </g>
        ))}

        {/* Next earnings marker — placed in the projection zone to the
            right of the last bar. Positioned proportionally to days
            until the event, capped at the right edge if the event is
            beyond visible projection space. */}
        {(() => {
          ...
          return (
            <g>
              <line x1={xE} x2={xE} y1={padT} y2={padT + innerH}
                    stroke={colors.warn} strokeWidth="1.4"
                    strokeDasharray={isClose ? "4 3" : "2 4"}
                    opacity={isClose ? 0.85 : 0.55} />
              <g transform={`translate(${xE}, ${padT + 4})`}>
                <circle r="11" fill={colors.warn} stroke="white" strokeWidth="1.5" opacity="0.95" />
                <text x="0" y="4" textAnchor="middle" fontSize="11" fontWeight="800"
                      fill="white" fontFamily="ui-sans-serif, system-ui">E</text>
              </g>
              <g transform={`translate(${xE}, ${padT + innerH - 12})`}>
                <rect x="-22" y="-9" width="44" height="18" rx="3"
                      fill={colors.warn} opacity="0.9" />
                <text x="0" y="4" textAnchor="middle" fontSize="11" fontWeight="700"
                      fill="white" fontFamily="ui-monospace, monospace">{dteLabel}</text>
              </g>
            </g>
          );
        })()}
        ...
        {hover != null && daily[hover.idx] && (() => {
          ...
          return (
            <g pointerEvents="none">
              ...
              <line x1={cx} x2={cx} y1={padT} y2={padT + priceH}
                    stroke="currentColor" opacity="0.35" strokeDasharray="2 3" />
              {/* Horizontal line follows mouse Y */}
              <line x1={padL} x2={padL + innerW} y1={cy} y2={cy}
                    stroke="currentColor" opacity="0.35" strokeDasharray="2 3" />
              {/* Price chip at mouse Y on the y-axis */}
              <g transform={`translate(${padL - 1}, ${cy})`}>
                <rect x="-50" y="-9" width="49" height="18" rx="3"
                      fill={colors.accent} opacity="0.95" />
                <text x="-25" y="4" textAnchor="middle" fontSize="11" fontWeight="700"
                      fill={colors.accentText} fontFamily="ui-monospace, monospace">
                  ${priceAtY.toFixed(2)}
                </text>
              </g>
              {/* Date chip at mouse X on the x-axis */}
              <g transform={`translate(${cx}, ${padT + priceH + 1})`}>
                <rect x="-36" y="0" width="72" height="16" rx="3"
                      fill={colors.fg2 || "#666"} opacity="0.95" />
                <text x="0" y="11" textAnchor="middle" fontSize="10" fontWeight="700"
                      fill="white" fontFamily="ui-monospace, monospace">
                  {fmtDate(daily[hover.idx].date, { month: "short", day: "numeric" })}
                </text>
              </g>
              {/* Bar dot — stays on the actual close at the hovered bar */}
              <circle cx={xScale(hover.idx)} cy={yScale(daily[hover.idx].close)} r="4"
                      fill={colors.accent} stroke="white" strokeWidth="1.5" />
            </g>
          );
        })()}

        {/* ─ MACD subchart (12, 26, 9) ─ */}
        {macdData && macdScale && (() => {
          const { mMin, mMax, yScaleM } = macdScale;
          ...
          return (
            <g>
              {/* Panel divider hint — subtle line above the panel */}
              <line x1={padL} x2={W - padR} y1={macdTop - gapH / 2} y2={macdTop - gapH / 2}
                    stroke="currentColor" opacity="0.08" />
              {/* Y-axis labels for MACD scale */}
              <text x={padL - 8} y={yScaleM(macdLabel) + 4} className="axis-text" textAnchor="end">
                {macdLabel >= 1 ? `+${macdLabel.toFixed(1)}` : `+${macdLabel.toFixed(2)}`}
              </text>
              <text x={padL - 8} y={zeroY + 4} className="axis-text" textAnchor="end">0</text>
              <text x={padL - 8} y={yScaleM(-macdLabel) + 4} className="axis-text" textAnchor="end">
                {macdLabel >= 1 ? `-${macdLabel.toFixed(1)}` : `-${macdLabel.toFixed(2)}`}
              </text>
              {/* Zero baseline */}
              <line x1={padL} x2={W - padR} y1={zeroY} y2={zeroY}
                    stroke="currentColor" opacity="0.25" strokeWidth="1" />
              {/* Histogram bars */}
              {macdData.hist.map((h, i) => {
                ...
                return <rect key={`h${i}`} x={x} y={y} width={barW} height={height} fill={fillC} opacity={opacity} />;
              })}
              {/* MACD line (cyan / accent) */}
              <path d={macdPath} fill="none" stroke={colors.accent} strokeWidth="1.5" opacity="0.9" />
              {/* Signal line (warn / orange) */}
              <path d={signalPath} fill="none" stroke={colors.warn} strokeWidth="1.5"
                    strokeDasharray="4 3" opacity="0.9" />
              {/* Panel label */}
              <text x={padL + 6} y={macdTop + 12} className="axis-text"
        ...
        {/* ─ RSI14 subchart (Wilder) ─ */}
        {showRSI && rsiData && rsiScale && (() => {
          const { yScaleR } = rsiScale;
          ...
          const lineColor = lastVal == null ? colors.fg2
            : lastVal >= 70 ? colors.down
            : lastVal <= 30 ? colors.up
            : "rgb(150, 130, 220)";
          return (
            <g>
              {/* Pane background band */}
              <rect x={padL} y={rsiTop} width={innerW} height={rsiH}
                    fill="none" stroke={colors.line} strokeOpacity="0.3" />
              {/* Overbought zone (70-100) tinted red */}
              <rect x={padL} y={rsiTop} width={innerW} height={yScaleR(70) - rsiTop}
                    fill={colors.down} fillOpacity="0.06" />
              {/* Oversold zone (0-30) tinted green */}
              <rect x={padL} y={yScaleR(30)} width={innerW} height={(rsiTop + rsiH) - yScaleR(30)}
                    fill={colors.up} fillOpacity="0.06" />
              {/* Reference lines: 70 / 50 / 30 */}
              {[70, 50, 30].map(level => (
                <line key={level}
                      x1={padL} x2={padL + innerW}
                      y1={yScaleR(level)} y2={yScaleR(level)}
              ...
              {[70, 30].map(level => (
                <text key={level}
                      x={padL - 6} y={yScaleR(level)}
                      textAnchor="end" dominantBaseline="middle"
                      fontFamily="ui-monospace, monospace" fontSize="10"
                      fill={level === 70 ? colors.down : colors.up} opacity="0.85">
                  {level}
                </text>
              ))}
              {/* RSI line */}
              <path d={d} fill="none" stroke={lineColor} strokeWidth="1.6" opacity="0.92" />
              {/* Latest value pill (right side) */}
              {lastVal != null && (() => {
                const x = padL + innerW + 4;
                const y = yScaleR(lastVal);
                return (
                  <g>
                    <rect x={x} y={y - 8} width={32} height={16} rx="3"
      ...
      {hover != null && daily[hover.idx] && (
        <div className="chart-tooltip" style={{ left: `${(xScale(hover.idx) / W) * 100}%` }}>
          <div className="tt-date">{fmtDate(daily[hover.idx].date, { weekday: "short", month: "short", day: "numeric" })}</div>
          <div className="tt-row"><span>Open</span><b>{fmt$(daily[hover.idx].open)}</b></div>
          <div className="tt-row"><span>High</span><b>{fmt$(daily[hover.idx].high)}</b></div>
          <div className="tt-row"><span>Low</span><b>{fmt$(daily[hover.idx].low)}</b></div>
          <div className="tt-row"><span>Close</span><b>{fmt$(daily[hover.idx].close)}</b></div>
        </div>
      )}
    </div>
  );
...
function _weekOrder(a, b) {
  const av = a.week_start, bv = b.week_start;
...
function ReturnsChart({ rows, medianHigh, medianLow, medianClose, currentReturn, colors, earnings }) {
  ...
  const earningsByWeek = useMemo(() => {
    ...
    const earnTimes = earnings.past
      .map(s => +new Date(s + "T16:00:00"))
  ...
  if (!data.length) {
    return <div className="chart-wrap" style={{padding: 40, textAlign: "center", color: "var(--fg-3)"}}>No history yet.</div>;
  }
  ...
  return (
    <div className="chart-wrap" onMouseLeave={() => setHover(null)}>
      <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg" >
        ...
        <line x1={padL} x2={W - padR} y1={yScale(medianHigh)} y2={yScale(medianHigh)}
              stroke={colors.up} strokeDasharray="4 3" strokeWidth="1" opacity="0.65" />
        <line x1={padL} x2={W - padR} y1={yScale(medianLow)} y2={yScale(medianLow)}
              stroke={colors.down} strokeDasharray="4 3" strokeWidth="1" opacity="0.65" />
        <line x1={padL} x2={W - padR} y1={yScale(medianClose)} y2={yScale(medianClose)}
              stroke="var(--fg-3)" strokeDasharray="2 4" strokeWidth="1" opacity="0.7" />

        {data.map((d, i) => {
          const x = xCenter(i);
          const yHi = yScale(d.high_return), yLo = yScale(d.low_return);
          ...
          return (
            <g key={i} onMouseEnter={() => setHover({ kind: "row", i })}>
              ...
              <line x1={x} x2={x} y1={yHi} y2={yLo}
                    stroke={fill} strokeWidth="1.5" opacity="0.45" />
              <line x1={x - barW / 2} x2={x + barW / 2} y1={yHi} y2={yHi}
                    stroke={colors.up} strokeWidth="2" />
              <line x1={x - barW / 2} x2={x + barW / 2} y1={yLo} y2={yLo}
                    stroke={colors.down} strokeWidth="2" />
              {/* Open marker: short horizontal tick on the LEFT of the
                  vertical bar line, mirroring how OHLC bars render in
                  TradingView. The Close stays as the centered ring. */}
              {yOp != null && (
                <line x1={x - barW / 2} x2={x} y1={yOp} y2={yOp}
                      stroke="var(--fg-2)" strokeWidth="2" />
              )}
              {/* Close marker: hollow ring in neutral fg color so it reads
                  as distinct from the green high cap and red low cap. */}
              <circle cx={x} cy={yCl} r="3.6" fill="var(--bg-2)"
                      stroke="var(--fg)" strokeWidth="2" />
              {earningsByWeek[i] && (
                <g transform={`translate(${x}, ${padT - 2})`}>
                  <circle r="6.5" fill={colors.warn} />
        ...
        {data.map((d, i) => {
          ...
          return (
            <text key={i} x={xCenter(i)} y={H - 10} className="axis-text" textAnchor="middle">
              {fmtDate(d.week_start, { month: "short", day: "numeric" })}
            </text>
          );
        })}
      ...
      {hover && hover.kind === "row" && data[hover.i] && (
        <div className="chart-tooltip small"
             style={{ left: `${(xCenter(hover.i) / W) * 100}%` }}>
          <div className="tt-date">{fmtDate(data[hover.i].week_start, { month: "short", day: "numeric" })}</div>
          <div className="tt-row"><span>High</span><b style={{color: colors.up}}>{fmtPct(data[hover.i].high_return, 2)}</b></div>
          <div className="tt-row"><span>Low</span><b style={{color: colors.down}}>{fmtPct(data[hover.i].low_return, 2)}</b></div>
          {data[hover.i].open_return != null && (
            <div className="tt-row"><span>Open</span><b>{fmtPct(data[hover.i].open_return, 2)}</b></div>
          )}
          <div className="tt-row"><span>Close</span><b>{fmtPct(data[hover.i].close_return, 2)}</b></div>
          {earningsByWeek[hover.i] && (
            <div className="tt-row"><span>Earnings</span><b style={{color: colors.warn}}>this week</b></div>
          )}
        </div>
      )}
...
// day_low / prior_close.
function DayBarChart({ rows, colors, mode = "high" }) {
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg dow-chart" >
        ...
        {days.map((d, i) => {
          ...
          const labelStyle = avg == null
            ...
            : inDirection
              ? { fill: fillColor, opacity: top ? 1 : 0.85, fontWeight: top ? 700 : 600 }
              : { fill: "var(--fg-3)", fontWeight: 500 };
          return (
            <g key={d}>
              {h > 0 && (
              ...
              <text x={x + bw / 2} y={H - 26} textAnchor="middle"
                    fontSize="15"
                    style={{fontWeight: top ? 700 : 500, fill: top ? fillColor : "var(--fg-2)"}}>
                {d}
              </text>
...
function PLChart({ legs, currentPrice, expectedMove, colors, strategyName = "" }) {
  const W = 1000, H = 400;
  ...
  const curve = useMemo(
    () => window.OptionStrats.pnlCurve(legs, lower, upper, 240),
    [legs, lower, upper]
  );

  const { min: plMin, max: plMax } = useMemo(
    () => window.OptionStrats.pnlBounds(curve), [curve]
  );
  ...
  const hoverIdx = hoverX == null ? null
    : Math.max(0, Math.min(curve.length - 1, Math.round(((hoverX - padL) / innerW) * (curve.length - 1))));
  ...
  const pathSegments = useMemo(() => {
    ...
    let current = { sign: curve[0].pl >= 0, pts: [[curve[0].s, curve[0].pl]] };
    for (let i = 1; i < curve.length; i++) {
      const prev = curve[i - 1], pt = curve[i];
      const sign = pt.pl >= 0;
      if (sign !== current.sign && prev.pl !== pt.pl) {
        ...
        segs.push(current);
        current = { sign, pts: [[sCross, 0], [pt.s, pt.pl]] };
      } else {
    ...
    <div className="chart-wrap pl-3d">
      <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg"
        ...
        <defs>
          ...
          <radialGradient id={`${uid}-prob`} cx="50%" cy="100%" r="80%">
            <stop offset="0%" stopColor="#3b6cd9" stopOpacity="0.55" />
            <stop offset="60%" stopColor="#1d3a7a" stopOpacity="0.32" />
            <stop offset="100%" stopColor="#1d3a7a" stopOpacity="0" />
          </radialGradient>
          <linearGradient id={`${uid}-profit`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#22e07a" stopOpacity="0.50" />
            <stop offset="100%" stopColor="#22e07a" stopOpacity="0.05" />
          </linearGradient>
          <linearGradient id={`${uid}-loss`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#ff3b6b" stopOpacity="0.05" />
            <stop offset="100%" stopColor="#7a1230" stopOpacity="0.55" />
          </linearGradient>
        ...
        <rect x="0" y="0" width={W} height={H} fill={`url(#${uid}-bg)`} rx="10" />

        {ticks.map((t, i) => (
          <line key={`g${i}`} x1={padL} x2={W - padR} y1={yScale(t)} y2={yScale(t)}
                stroke="#3a5a99" strokeOpacity="0.18" strokeWidth="1" />
        ))}
        {xTicks.map((t, i) => (
          <line key={`gx${i}`} x1={xScale(t)} x2={xScale(t)} y1={padT} y2={padT + innerH}
                stroke="#3a5a99" strokeOpacity="0.10" strokeWidth="1" />
        ))}

        <path d={`M ${probPath[0][0]} ${zeroY} ${probPath.map(p => `L ${p[0]} ${p[1]}`).join(" ")} L ${probPath[probPath.length - 1][0]} ${zeroY} Z`}
              fill={`url(#${uid}-prob)`} />
        <path d={probPath.map((p, i) => `${i === 0 ? "M" : "L"} ${p[0]} ${p[1]}`).join(" ")}
              fill="none" stroke="#6aa8ff" strokeOpacity="0.7" strokeWidth="1.4" />
        ...
        {pathSegments.map((seg, i) => {
          ...
          const d = `M ${xScale(first[0])} ${zeroY} ` +
                    seg.pts.map(p => `L ${xScale(p[0])} ${yScale(p[1])}`).join(" ") +
                    ` L ${xScale(last[0])} ${zeroY} Z`;
          return <path key={i} d={d} fill={`url(#${grad})`} />;
        })}

        <line x1={padL} x2={W - padR} y1={zeroY} y2={zeroY}
              stroke="white" strokeOpacity="0.85" strokeWidth="1.2" />

        <line x1={xScale(currentPrice)} x2={xScale(currentPrice)} y1={padT} y2={padT + innerH}
              stroke="white" strokeOpacity="0.55" strokeDasharray="3 4" strokeWidth="1" />
        <g transform={`translate(${xScale(currentPrice)}, ${padT + innerH + 18})`}>
          <rect x="-44" y="-11" width="88" height="18" rx="9" fill="#0a1228" stroke="white" strokeOpacity="0.4" />
          <text x="0" y="3" textAnchor="middle"
                fill="white" opacity="0.95" fontSize="11" fontFamily="ui-monospace, monospace">
        ...
        {pathSegments.map((seg, i) => {
          ...
          return <path key={`l${i}`} d={d} fill="none" stroke={stroke}
        ...
        {legs.filter(l => l.type !== "stock").map((leg, i) => {
          ...
          return (
            <g key={`s${i}`}>
              <line x1={xScale(leg.strike)} x2={xScale(leg.strike)} y1={padT} y2={padT + innerH}
                    stroke={color} strokeWidth={isShort ? "1.6" : "1.0"}
                    strokeDasharray={isShort ? null : "4 3"}
                    strokeOpacity={isShort ? "0.85" : "0.55"} />
              <text x={xScale(leg.strike)} y={labelY} textAnchor="middle"
                    fill={color} fontFamily="ui-monospace, monospace"
                    fontSize="11" fontWeight="600">
                {isShort ? "−" : "+"}{leg.type === "call" ? "C" : "P"} ${leg.strike.toFixed(2)}
                {leg.dte > 7 ? `  ${leg.dte}d` : ""}
              </text>
            </g>
          );
        ...
        {bes.map((be, i) => {
          ...
          return (
            <g key={`b${i}`}>
              <circle cx={xScale(be)} cy={zeroY} r="4" fill="white" />
              <text x={xScale(be)} y={zeroY + dy} textAnchor="middle"
                    fill="white" opacity="0.92" fontSize="11" fontFamily="ui-monospace, monospace">
                B/E ${be.toFixed(2)}
              </text>
            </g>
          );
        ...
        {ticks.map((t, i) => (
          <text key={`yt${i}`} x={padL - 10} y={yScale(t) + 4}
                fill="#9fb6e0" fontSize="11" fontFamily="ui-monospace, monospace" textAnchor="end">
        ...
        {xTicks.map((t, i) => {
          ...
          return (
            <text key={`xt${i}`} x={xScale(t)} y={padT + innerH + 22}
                  fill="#9fb6e0" fontSize="11" fontFamily="ui-monospace, monospace" textAnchor="middle">
              ${t.toFixed(2)}
            </text>
          );
        })}
        ...
        {hoverPt && (
          <g>
            <line x1={xScale(hoverPt.s)} x2={xScale(hoverPt.s)}
                  y1={padT} y2={padT + innerH}
                  stroke="white" strokeOpacity="0.4" strokeDasharray="2 3" />
            <circle cx={xScale(hoverPt.s)} cy={yScale(hoverPt.pl)} r="4.5"
                    fill={hoverPt.pl >= 0 ? "#22e07a" : "#ff3057"} stroke="white" strokeWidth="1.5" />
          </g>
        )}

        <g transform={`translate(${padL + 10}, ${padT + 10})`}>
          <rect x="0" y="0" width="240" height="22" rx="11" fill="#0a1228" stroke="#2a4a85" strokeOpacity="0.6" />
          <polygon points="10,16 17,7 24,16" fill="#6aa8ff" />
          <text x="32" y="15" fill="#bcd0f5" fontSize="11" fontFamily="ui-sans-serif, system-ui">
            {strategyName ? strategyName + " · " : ""}±${sigma.toFixed(2)} 1σ
          </text>
        </g>
      </svg>

      {hoverPt && (
        <div className="chart-tooltip pl-tip" style={{ left: `${(xScale(hoverPt.s) / W) * 100}%` }}>
          <div className="tt-row"><span>Stock at expiry</span><b>${hoverPt.s.toFixed(2)}</b></div>
          <div className="tt-row"><span>P/L</span>
            <b style={{color: hoverPt.pl >= 0 ? "#22e07a" : "#ff3057"}}>
...
function ThetaPanel({
  rows, sugCall, sugPut, callIv, putIv, side,
  currentPrice, baselinePrice, expDate, FRONT_DTE, typicalHighDay, typicalLowDay,
  medianHigh, medianLow, colors,
}) {
  const bs = window.OptionStrats?.bsPrice;
  if (!bs) return <div className="muted" style={{padding: 12}}>BS engine not loaded.</div>;
  ...
  const expIntrinsic = isCall
    ? Math.max(expDay.spot - strike, 0)
    : Math.max(strike - expDay.spot, 0);
  const scenarioHold = {
  ...
  for (let i = days.length - 1; i >= 1; i--) {
    if (days[i].isTargetDay) { preTargetIdx = i - 1; break; }
  }
    ...
    <div>
      ...
      <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10}}>
        <div className="muted" style={{fontSize: 12}}>
          Modeling the {isCall ? "call" : "put"} at <b style={{color: "var(--fg)"}}>${strike.toFixed(2)}</b>
          {" · "}IV <b style={{color: "var(--fg)"}}>{(iv * 100).toFixed(1)}%</b>
          {" · "}target day <b style={{color: isCall ? "var(--up)" : "var(--down)"}}>{targetDay}</b>
        </div>
      </div>
      ...
      <div className="chart-wrap">
        <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg" >
          ...
          {/* Highlight target rally day vertical band */}
          {days.map((d, i) => d.isTargetDay && (
            <rect key={`hl${i}`} x={xScale(i) - 16} y={padT} width="32" height={innerH}
                  fill={isCall ? colors.up : colors.down} opacity="0.10" rx="3" />
          ))}
          {/* Premium line */}
          ...
          {days.map((d, i) => (
            <g key={i}>
              <circle cx={xScale(i)} cy={yScale(d.price)} r="4"
                      fill={d.isTargetDay ? (isCall ? colors.up : colors.down) : colors.accent}
                      stroke="var(--bg-2)" strokeWidth="1.5" />
              <text x={xScale(i)} y={yScale(d.price) - 10} textAnchor="middle"
                    fontSize="10" fontFamily="ui-monospace, monospace" fill="var(--fg-2)">
                ${d.price.toFixed(2)}
              </text>
              <text x={xScale(i)} y={H - 16} textAnchor="middle" className="axis-text"
                    style={{fontWeight: d.isTargetDay ? 700 : 500,
                            fill: d.isTargetDay ? (isCall ? colors.up : colors.down) : "currentColor"}}>
                {d.dowName}
              </text>
              <text x={xScale(i)} y={H - 4} textAnchor="middle"
                    fontSize="9" fill="var(--fg-3)" fontFamily="ui-monospace, monospace">
                {d.dte}d
              </text>
            </g>
          ))}
          ...
          {[minPrice, (minPrice + maxPrice) / 2, maxPrice].map((v, i) => (
            <text key={`yt${i}`} x={padL - 6} y={yScale(v) + 3}
                  textAnchor="end" className="axis-text">${v.toFixed(2)}</text>
          ))}
        </svg>
      </div>

      {/* ── Scenario comparison ─────────────────────────────────────────── */}
      <div className="theta-scenarios">
        {scenarios.map((s, i) => {
          const isWinner = s.label === winner.label;
          return (
            <div key={i} className={`theta-scenario ${isWinner ? "winner" : ""}`}>
              <div className="theta-scenario-name">{s.label}</div>
              <div className="theta-scenario-stats">
                <span>Entry <b>${s.entry.toFixed(2)}</b></span>
                <span>Exit <b>${s.exit.toFixed(2)}</b></span>
                <span>Days <b>{s.daysAtRisk}</b></span>
              </div>
              <div className="theta-scenario-cap"
                   style={{color: s.captured >= 0 ? "var(--up)" : "var(--down)"}}>
                {s.captured >= 0 ? "+" : ""}${s.captured.toFixed(2)}/sh captured
              </div>
              {isWinner && <div className="theta-scenario-badge">Best expected capture</div>}
            </div>
          );
        })}
      </div>
      ...
      <div className="muted" style={{fontSize: 12, lineHeight: 1.55, marginTop: 10}}>
        <b style={{color: "var(--fg)"}}>Read.</b>{" "}
        Premium at expiration is intrinsic only, ${expIntrinsic.toFixed(2)}.
        {" "}Today's mid is ${entry.price.toFixed(2)}.
        {" "}Theta from today through expiry totals ${(entry.price - expIntrinsic).toFixed(2)} if the spot path follows history.
        {scenarioClose && (
          <> {" "}Closing {scenarioClose.exitDay.dowName} EOD captures
          <b style={{color: "var(--fg)"}}> ${scenarioClose.captured.toFixed(2)}</b>{" "}
          and removes the {targetDay} gamma exposure.</>
        )}
        {" "}IV used is the chain's marked IV, not a forward forecast.
        {" "}Spot path is purely historical median behavior, actuals will deviate.
      </div>
    </div>
  );
}
