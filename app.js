...
const idleRun = fn => typeof window !== "undefined" && window.requestIdleCallback ? window.requestIdleCallback(fn, {
  timeout: 1500
}) : setTimeout(fn, 600);
...
const _PERF = (() => {
  let on = false;
  try {
    on = localStorage.getItem("jerryDebug") === "1" || /[?&]debug\b/.test(location.search);
  } catch (_) {}
  ...
  const api = {
    ...
    mark(label) {
      if (on) console.log(`%c[perf] ${label} @ ${Math.round(since())}ms`, "color:#22c55e");
    },
    resetCounter(which) {
      if (!on) return;
      const v = ctr[which];
    ...
    report(label) {
      ...
      const rows = Object.entries(ep).map(([k, v]) => ({
        ...
        calls: v.n,
        cacheHits: v.cached,
        avgMs: Math.round(v.ms / v.n),
        totalKB: +(v.bytes / 1024).toFixed(1)
...
function App() {
  ...
  const apiFetch = React.useCallback((path, opts = {}) => {
    ...
    if (hit && now - hit.ts < API_CACHE_TTL) {
      if (_PERF.on) hit.promise.then(({
        text
      }) => _PERF.record(ep, (performance.now() || Date.now()) - t0, (text || "").length, true));
  ...
  const tweaks = {
    values: tweakVals,
    setValue: setTweakVal
  };

  // Persist tweak values to localStorage so theme, accent, etc. survive a reload.
  useEffect(() => {
    try {
      localStorage.setItem(TWEAK_KEY, JSON.stringify(tweakVals));
    } catch {}
  }, [tweakVals]);
  ...
  const [oiChartMetric, setOiChartMetric] = useState(() => {
    try {
      return localStorage.getItem("weeklyOptionsTimer.oiChart.metric.v1") || "volume";
    } catch {
      return "volume";
    }
  });
  useEffect(() => {
    try {
      localStorage.setItem("weeklyOptionsTimer.oiChart.metric.v1", oiChartMetric);
    } catch {}
  }, [oiChartMetric]);
  ...
  useEffect(() => {
    if (!window.__JT_EMBED) return;
    try {
      document.body.classList.add("jt-embed");
    } catch {}
    try {
  ...
  const scoreSnapshot = snap => {
    ...
    const iv = snap.iv30_avg,
      hv = snap.hv20;
    const richness = snap.richness;
    const callSafe = snap.call_safe_pct,
      putSafe = snap.put_safe_pct;
    ...
    if (safeBoth && !earnSoon) {
      ...
      if (isRich) {
        s += 18;
        reasons.push("premium rich");
      }
      if (isNeutral) {
        s += 8;
        reasons.push("neutral bias");
      }
      if (callSafe >= 5 && putSafe >= 5) {
        s += 8;
        reasons.push("wide expected range");
      }
      candidates.push({
        key: "short_strangle",
        name: "Short Strangle",
      ...
      try {
        ...
        if (!r.ok) {
          ...
          if (r.status === 409) {
            ...
            try {
              const sr = await apiFetch("/api/watchlist", {
                noCache: true
              });
              if (sr.ok) {
  ...
  useEffect(() => {
    ...
    const onVis = () => {
      if (document.visibilityState === "hidden") flushWatchlist();
    };
  ...
  useEffect(() => {
    ...
    const refetch = async () => {
      ...
      try {
        ...
        const local = latestWatchlistRef.current || {
          symbols: []
        };
  ...
  const filteredWatchlistSymbols = watchlistTagFilter ? watchlistData.symbols.filter(s => (s.tags || []).includes(watchlistTagFilter)).map(s => s.symbol) : watchlistData.symbols.map(s => s.symbol);
  ...
  const wlToggleStar = sym => {
    ...
    setWatchlistData(prev => ({
      ...prev,
      symbols: prev.symbols.map(s => s.symbol === sym ? {
        ...s,
        starred: !s.starred
      } : s)
    }));
    ...
    setWatchlistData(prev => {
      ...
      if (mode === "replace") {
        ...
        for (const s of prev.symbols) {
          ...
          if (r) {
            out.push(applyCsv(s, r));
            bySym.delete(s.symbol);
            count++;
          } else out.push(s);
        }
        for (const [symbol, r] of bySym) {
  ...
  useEffect(() => {
    const NOSWIPE = ".scan-table-wrap,.oc-table-scroll,.tab-bar,.sidebar,.mobile-overlay," + "canvas,input,select,textarea,.swing-levels,.news-srcnav,.swing-histnav," + ".screener-subnav,.ab-chips,.pcalc-panel,.mobile-bottombar";
    let sx = 0,
      sy = 0,
      st = 0,
      tracking = false;
    const onStart = e => {
      if (window.innerWidth > 900 || e.touches.length !== 1) {
        tracking = false;
        return;
      }
      const t = e.target;
      if (t && t.closest && t.closest(NOSWIPE)) {
        tracking = false;
        return;
      }
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const id = setTimeout(() => {
      apiFetch(`/api/search?q=${encodeURIComponent(q)}`).then(r => r.ok ? r.json() : Promise.reject(r)).then(data => {
        if (cancelled) return;
        ...
        const matches = presets.filter(t => t.startsWith(q.toUpperCase())).map(sym => ({
          symbol: sym,
          name: window.MockData.PRESETS[sym].name || sym,
          type: "EQUITY",
          exchange: ""
        }));
  ...
  const dataset = useMemo(() => {
    ...
    const {
      rows,
      current
    } = window.MockData.buildWeekly(ticker, weeks);
  ...
  const calls = chain.calls,
    puts = chain.puts;
  const nearest = (arr, target) => arr.length ? arr.reduce((a, b) => Math.abs(a.strike - target) < Math.abs(b.strike - target) ? a : b) : {
    strike: target,
    bid: 0,
    ask: 0,
    last: 0,
  ...
  useEffect(() => {
    try {
      localStorage.setItem("jerryDash.sizing.v1", JSON.stringify(sizingConfig));
    } catch (_) {}
  }, [sizingConfig]);
    ...
    setPositions(prev => {
      ...
      if (closed && (closed.type === "call" || closed.type === "put" || closed.kind === "call" || closed.kind === "put")) {
        ...
        try {
          ...
          }).then(() => window.dispatchEvent(new CustomEvent("jerry:position-closed", {
            detail: journalEntry
  ...
  const [weeklyRangeProgress, setWeeklyRangeProgress] = useState({
    done: 0,
    total: 0
  });
  ...
  const [pullbackScanProgress, setPullbackScanProgress] = useState({
    done: 0,
    total: 0
  });
  // Pullback scanner sort — default rank by score (verdict attractiveness)
  const [pbSort, setPbSort] = useState({
    key: "_score",
    dir: "desc"
  });
  ...
  const [pbScanDir, setPbScanDir] = useState(() => {
    try {
      return localStorage.getItem(PBSCAN_DIR_KEY) || "short";
    } catch {
      return "short";
    }
  });
  useEffect(() => {
    try {
      localStorage.setItem(PBSCAN_DIR_KEY, pbScanDir);
    } catch {}
  }, [pbScanDir]);
  ...
  const [richnessProgress, setRichnessProgress] = useState({
    done: 0,
    total: 0
  });
  const [richnessAt, setRichnessAt] = useState(null);
  const [richnessSort, setRichnessSort] = useState({
    key: "score",
    dir: "desc"
  });
  ...
  const [momentumProgress, setMomentumProgress] = useState({
    done: 0,
    total: 0
  });
  const [momentumAt, setMomentumAt] = useState(null);
  const [momentumSort, setMomentumSort] = useState({
    key: "score",
    dir: "desc"
  });
  ...
  const [marketScanSort, setMarketScanSort] = useState({
    key: "unusual_pct",
    dir: "desc"
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(EMA_PARAMS_KEY, JSON.stringify({
        fast: emaFast
      }));
    } catch {}
  }, [emaFast]);
  ...
  const [emaScanProgress, setEmaScanProgress] = useState({
    done: 0,
    total: 0
  });
  ...
  useEffect(() => {
    ...
    setManualPutStrike(null);
    setManualCallWing(null);
  ...
  if (strikeMode === "delta" && calls.length && calls.some(c => c.delta != null)) {
    ...
    callAtSug = callPool.length ? callPool.reduce((a, b) => Math.abs(a.delta - targetDelta) < Math.abs(b.delta - targetDelta) ? a : b) : nearest(calls, currentPrice * 1.02);
    putAtSug = putPool.length ? putPool.reduce((a, b) => Math.abs(a.delta + targetDelta) < Math.abs(b.delta + targetDelta) ? a : b) : nearest(puts, currentPrice * 0.98);
  } else {
  ...
  const liveExpirations = useMemo(() => window.MockData.getLiveExpirations ? window.MockData.getLiveExpirations(ticker) : [], [ticker, dataVersion]);

  // Active expiration date object — drives FRONT_DTE and label rendering.
  const activeExpDate = useMemo(() => {
    if (expiration) {
      try {
        return new Date(expiration + "T16:00:00");
      } catch {/* fall through */}
    }
  ...
  const callSafePct = rows.length ? rows.filter(r => r.high_return < (sugCall / baselinePrice - 1) * 100).length / rows.length * 100 : 0;
  const putSafePct = rows.length ? rows.filter(r => r.low_return > (sugPut / baselinePrice - 1) * 100).length / rows.length * 100 : 0;
  const bothSafePct = rows.length ? rows.filter(r => r.high_return < (sugCall / baselinePrice - 1) * 100 && r.low_return > (sugPut / baselinePrice - 1) * 100).length / rows.length * 100 : 0;
  ...
  const _recPair = window.RecEngine && window.RecEngine.buildBoth ? window.RecEngine.buildBoth({
  ...
  const rec = Object.assign({}, _recPair.cc, {
    cc: _recPair.cc,
    csp: _recPair.csp
  });
  ...
  const expFrontLabel = activeExpDate.toLocaleDateString("en-US", {
    month: "short",
    day: "numeric"
  });
  // For calendar/diagonal: a back month roughly 30 days after front
  const _back = new Date(activeExpDate);
  _back.setDate(_back.getDate() + 28);
  const expBackLabel = _back.toLocaleDateString("en-US", {
    month: "short",
    day: "numeric"
  });
  const stratCtx = {
  ...
  function scoreFor(key, c) {
    ...
    switch (key) {
      case "short_strangle":
        if (isRich) {
          s += 18;
          why.push("premium rich");
        }
        if (isVeryRich) s += 8;
        if (isHighProb) {
          s += 12;
          why.push("history holds");
        }
        if (isCheap) {
          s -= 25;
          why.push("premium cheap");
        }
        if (earn) {
          s -= 30;
          why.push("earnings risk");
        }
        break;
      case "iron_condor":
        s = 55;
      ...
      case "short_straddle":
        s = 30;
        if (isVeryRich && isVeryHighProb) {
          s += 35;
          why.push("max premium env");
        } else if (isVeryRich) {
          s += 15;
          why.push("very rich");
      ...
      case "long_risk_reversal":
        ...
        if (c.richness > 1.0 && c.putSafePct < c.callSafePct) {
          s += 8;
          why.push("put skew rich");
        }
        if (isBearish) s -= 22;
        break;
      ...
      case "call_ratio_backspread":
        ...
        if (isBullish && isCheap) {
          s += 25;
          why.push("cheap upside");
      ...
      case "put_ratio_backspread":
        ...
        if (isBearish && isCheap) {
          s += 25;
          why.push("cheap downside");
  ...
  const liveEarnings = useMemo(() => window.MockData.getLiveEarnings ? window.MockData.getLiveEarnings(ticker) : {
    past: [],
    next: null
  }, [ticker, dataVersion]);
  ...
  const plRange = useMemo(() => {
    ...
    return {
      lower: Math.max(0.5, currentPrice - half),
      upper: currentPrice + half
    };
  }, [plLegs, currentPrice]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "ab-icon ab-menu",
    onClick: () => setNavOpen(true),
    "aria-label": "Ticker, watchlist and settings",
    title: "Ticker, watchlist, presets and settings."
  }, "\u2630"), /*#__PURE__*/React.createElement("button", {
    ...
    className: "sb-status"
  }, loading ? "Loading…" : window.__LIVE ? /*#__PURE__*/React.createElement(LiveClock, null) : "Static snapshot"), loadError && /*#__PURE__*/React.createElement("div", {
    className: "sb-status err",
    title: retryIn != null ? THROTTLE_MSG : loadError
  }, retryIn != null ? `${THROTTLE_SHORT} (${retryIn}s)` : loadError), /*#__PURE__*/React.createElement("div", {
    ...
    if (last === "schwab") {
      ...
      return /*#__PURE__*/React.createElement("div", {
        className: `src-badge src-badge-schwab${sw.needs_refresh_soon ? " src-badge-warn" : ""}`,
    ...
    if (!uwHealth.configured) {
      return /*#__PURE__*/React.createElement("div", {
        className: "src-badge src-badge-uw-off",
    ...
    if (!uwHealth.connected) {
      return /*#__PURE__*/React.createElement("div", {
        className: "src-badge src-badge-uw-err",
    ...
    const dailyTxt = typeof daily === "number" && typeof limit === "number" ? `${daily.toLocaleString()}/${limit.toLocaleString()}` : typeof daily === "number" ? `${daily.toLocaleString()}` : "—";
    const remTxt = typeof rem === "number" ? `${rem}/min left` : "min quota unknown";
    return /*#__PURE__*/React.createElement("div", {
      className: `src-badge src-badge-uw${lowQuota ? "-warn" : ""}`,
    ...
    className: "sb-icon-btn sb-theme-btn",
    ...
    cx: tweaks.values.theme === "dark" ? "17" : "7",
    cy: "12",
    r: "3.2",
    fill: "currentColor"
  }))), /*#__PURE__*/React.createElement("button", {
    className: "sb-icon-btn sb-ref-btn",
    onClick: () => setShowRef(true),
    title: "Strategy reference",
    "aria-label": "Strategy cheat sheet"
  }, /*#__PURE__*/React.createElement("svg", {
    viewBox: "0 0 24 24",
    width: "18",
    height: "18",
    ...
    d: "M2 4 C 6 3, 10 3.5, 12 5 C 14 3.5, 18 3, 22 4 L 22 20 C 18 19, 14 19.5, 12 21 C 10 19.5, 6 19, 2 20 Z"
    ...
    className: "sb-icon-btn sb-help-btn",
    ...
    className: "sb-ticker-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "sb-ticker-left"
  }, /*#__PURE__*/React.createElement("div", {
    className: "sb-ticker-wrap sb-ticker-wrap-compact"
  }, /*#__PURE__*/React.createElement("input", {
    ref: tickerInputRef,
    className: "sb-ticker-input",
    value: tickerInput,
    onChange: e => {
      setTickerInput(e.target.value.toUpperCase().slice(0, 6));
      setSearchOpen(true);
    },
    onKeyDown: e => {
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setSearchActive(i => Math.min(searchResults.length - 1, i + 1));
        setSearchOpen(true);
      } else if (e.key === "ArrowUp") {
        e.preventDefault();
        setSearchActive(i => Math.max(-1, i - 1));
    ...
    className: "sb-search-dropdown"
  }, searchResults.map((r, i) => /*#__PURE__*/React.createElement("div", {
    key: r.symbol,
    className: `sb-search-item ${i === searchActive ? "active" : ""}`,
    onMouseDown: e => {
      e.preventDefault();
      setTicker(r.symbol);
      setTickerInput(r.symbol);
      setSearchOpen(false);
    },
    onMouseEnter: () => setSearchActive(i)
  }, /*#__PURE__*/React.createElement("span", {
    className: "sb-search-sym"
  }, r.symbol), /*#__PURE__*/React.createElement("span", {
    className: "sb-search-name"
  }, r.name), r.exchange && /*#__PURE__*/React.createElement("span", {
  ...
  React.createElement("span", {
    ...
    const staleTip = staleSec != null ? staleSec < 60 ? `Live · ${staleSec}s since last trade` : staleSec < 3600 ? `Last trade ${Math.round(staleSec / 60)}m ago` : `Last trade ${Math.round(staleSec / 3600)}h ago — illiquid or session boundary` : null;
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(FlashOnChange, {
      value: currentPrice,
      className: `sb-price${isStale ? " sb-price-stale" : ""}`
    }, /*#__PURE__*/React.createElement("span", {
    ...
    className: "sb-wl-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "sb-pin-btn",
    title: watchlistData.symbols.find(s => s.symbol === ticker)?.starred ? "Unstar this ticker" : watchlist.includes(ticker) ? "Star this ticker" : "Add and star this ticker",
    onClick: () => {
  ...
  }, watchlistData.symbols.find(s => s.symbol === ticker)?.starred ? "★ starred" : "☆ star"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "No starred tickers. Click \u2606 to star, or Manage to add."), starredSymbols.map(t => {
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      title: y == null ? "Click to switch · right-click to unstar" : `${t} is ${y >= 0 ? "up" : "down"} ${Math.abs(y).toFixed(1)}% this year · click to switch · right-click to unstar`
    }, t, y != null && /*#__PURE__*/React.createElement("span", {
      className: `pp-ytd ${y >= 0 ? "up" : "down"}`
    }, y >= 0 ? "+" : "", y.toFixed(1), "%"));
  }))), /*#__PURE__*/React.createElement("div", {
    className: "sb-section"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "sb-label"
  }, "Strike picker"), /*#__PURE__*/React.createElement("div", {
    className: "seg",
    style: {
      marginBottom: 8
    }
  }, /*#__PURE__*/React.createElement("button", {
    className: strikeMode === "delta" ? "active" : "",
    onClick: () => setStrikeMode("delta")
    ...
    k: "baseline",
    className: "sb-label"
  }, "Return baseline"), /*#__PURE__*/React.createElement("select", {
    className: "sb-select",
    value: baseline,
    onChange: e => setBaseline(e.target.value)
  }, /*#__PURE__*/React.createElement("option", {
    value: "monday"
  }, "Monday Open"), /*#__PURE__*/React.createElement("option", {
    value: "friday"
  }, "Previous Friday Close"))), /*#__PURE__*/React.createElement("div", {
    className: "sb-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "sb-label"
  }, "Expiration"), liveExpirations.length > 0 ? /*#__PURE__*/React.createElement("select", {
    className: "sb-select",
    ...
    className: "sb-expiry"
  }, activeExpDate.toLocaleDateString("en-US", {
    weekday: "short",
    month: "short",
    day: "numeric"
  }), /*#__PURE__*/React.createElement("span", {
  ...
  React.createElement("details", {
    ...
    label: "Watchlist analyst actions"
  }, /*#__PURE__*/React.createElement(WatchlistAnalystCard, {
    apiFetch: apiFetch,
    onSwitchTicker: switchTicker
  })), /*#__PURE__*/React.createElement(CardErrorBoundary, {
    ...
    label: "Streaks"
  }, /*#__PURE__*/React.createElement(WatchlistStreaksCard, {
    apiFetch: apiFetch,
    onSwitchTicker: switchTicker
  }))), /*#__PURE__*/React.createElement(TabPanel, {
    tab: "calendar",
    ...
    className: "hk-row"
  }, /*#__PURE__*/React.createElement("kbd", null, "Esc"), /*#__PURE__*/React.createElement("span", null, "Close help / exit search dropdown"))), /*#__PURE__*/React.createElement("div", {
    className: "hk-foot"
  }, "Hot keys ignore inputs, so you can type freely in any text field."))), /*#__PURE__*/React.createElement("div", {
    className: `load-bar ${loading ? "loading" : ""}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "load-bar-fill"
  })), /*#__PURE__*/React.createElement(PercentCalc, {
    activeTicker: ticker,
    ...
    const evaluated = open.filter(p => p.ticker === ticker && currentPrice).map(p => {
      let worst = null; // {leg, distance, tier}
      for (const leg of p.legs) {
        if (leg.qty >= 0) continue; // long legs don't get assigned
        const itm = leg.type === "call" && currentPrice > leg.strike || leg.type === "put" && currentPrice < leg.strike;
        const distPct = Math.abs(currentPrice - leg.strike) / leg.strike;
        let tier = null;
        if (itm) tier = "itm";else if (distPct < 0.01) tier = "very-close";else if (distPct < 0.03) tier = "close";
        if (tier && (!worst || ["close", "very-close", "itm"].indexOf(tier) > ["close", "very-close", "itm"].indexOf(worst.tier))) {
          worst = {
            leg,
            distPct,
            tier
    ...
    const topTier = evaluated.reduce((best, e) => order[e.tier] > order[best] ? e.tier : best, "close");
    const tierLabel = topTier === "itm" ? "IN THE MONEY" : topTier === "very-close" ? "VERY CLOSE" : "CLOSE";
    return /*#__PURE__*/React.createElement("div", {
      className: `assign-banner assign-${topTier}`,
      onClick: () => {
    ...
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement(RollManagerCard, {
    ticker: ticker,
    positions: positions,
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: `chart-perf-val ${cls(ytd)}`
    }, fmt(ytd))));
  })(), /*#__PURE__*/React.createElement("div", {
    className: "card-head"
    ...
    title: "Toggle EMA21 (TradingView ta.ema, length 21)"
  }, "EMA21"), /*#__PURE__*/React.createElement("button", {
    className: showMA50 ? "active ma50" : "ma50",
    onClick: () => setShowMA50(v => !v),
    title: "Toggle 50-day moving average"
  }, "MA50"), /*#__PURE__*/React.createElement("button", {
    className: showMA200 ? "active ma200" : "ma200",
    onClick: () => setShowMA200(v => !v),
    title: "Toggle 200-day moving average"
  }, "MA200"), /*#__PURE__*/React.createElement("button", {
    className: showRSI ? "active rsi" : "rsi",
    onClick: () => setShowRSI(v => !v),
    title: "Toggle RSI14 panel below the chart. Wilder smoothing, standard 14-period."
  ...
  }, [[30, "30D"], [60, "60D"], [120, "120D"], [250, "250D"]].map(([n, l]) => /*#__PURE__*/React.createElement("button", {
    key: n,
    className: !viewRange && chartDays === n ? "active" : "",
    onClick: () => {
      setChartDays(n);
      setViewRange(null);
    },
    title: `Show last ${n} trading days`
  }, l)), viewRange && /*#__PURE__*/React.createElement("button", {
    className: "zoom-reset",
    onClick: () => setViewRange(null),
    title: "Reset zoom and pan"
  }, "RESET")), /*#__PURE__*/React.createElement("div", {
    className: "seg"
  }, [["candles", "Candles"], ["area", "Area"], ["ohlc", "OHLC"]].map(([v, l]) => /*#__PURE__*/React.createElement("button", {
    key: v,
    ...
    return /*#__PURE__*/React.createElement(CardErrorBoundary, {
      label: "Price chart"
    }, window.LightweightCharts ? /*#__PURE__*/React.createElement(TVPriceChart, {
      daily: visibleDaily,
      expHigh: expHigh,
      expLow: expLow,
  ...
  }), "Suggested call"), /*#__PURE__*/React.createElement("span", {
    className: "item"
  }, /*#__PURE__*/React.createElement("span", {
    className: "swatch dashed",
    style: {
  ...
  }), "Suggested put"), /*#__PURE__*/React.createElement("span", {
    className: "item"
  }, /*#__PURE__*/React.createElement("span", {
    className: "swatch",
    style: {
  ...
  }), "Current price"), showEMA21 && /*#__PURE__*/React.createElement("span", {
    className: "item"
  }, /*#__PURE__*/React.createElement("span", {
    className: "swatch",
    style: {
  ...
  }), "EMA 21"), showMA50 && /*#__PURE__*/React.createElement("span", {
    className: "item"
  }, /*#__PURE__*/React.createElement("span", {
    className: "swatch",
    style: {
  ...
  }), "MA 50"), showMA200 && /*#__PURE__*/React.createElement("span", {
    className: "item"
  }, /*#__PURE__*/React.createElement("span", {
    className: "swatch",
    style: {
  ...
  }), "MA 200"), showProbCone && /*#__PURE__*/React.createElement("span", {
    className: "item"
  }, /*#__PURE__*/React.createElement("span", {
    className: "swatch",
    style: {
    ...
    for (let i = daily.length - 1; i > 0; i--) {
      ...
      if (a50 < a200 && b50 > b200 || a50 > a200 && b50 < b200) {
        lastCross = {
          idx: i,
          daysAgo: daily.length - 1 - i,
          type: a50 < a200 ? "golden" : "death",
          date: daily[i].date
        };
        break;
      }
    ...
    return /*#__PURE__*/React.createElement("span", {
      className: "item ma-regime"
    }, /*#__PURE__*/React.createElement("span", {
      className: "ma-pip",
      style: {
        background: isAbove ? "var(--up)" : "var(--down)"
      }
    }), /*#__PURE__*/React.createElement("b", {
      style: {
        color: isAbove ? "var(--up)" : "var(--down)"
      }
    }, isAbove ? "Bull regime (50 above 200)" : "Bear regime (50 below 200)"), lastCross && /*#__PURE__*/React.createElement("span", {
      style: {
        color: "var(--fg-3)",
        marginLeft: 6
      }
  ...
  }), "Current price"), liveEarnings.past?.length > 0 && /*#__PURE__*/React.createElement("span", {
    className: "item"
  }, /*#__PURE__*/React.createElement("span", {
  ...
  }), "Past earnings"), liveEarnings.next && /*#__PURE__*/React.createElement("span", {
    className: "item earn-next-legend"
  }, /*#__PURE__*/React.createElement("span", {
    className: "swatch dot",
    style: {
      background: chartColors.warn,
    ...
    let hi = -Infinity,
      ...
      loIdx = winStart;
    win.forEach((d, i) => {
      if (d.high != null && d.high > hi) {
        hi = d.high;
        hiIdx = winStart + i;
      }
      if (d.low != null && d.low < lo) {
        lo = d.low;
        loIdx = winStart + i;
      }
    });
    ...
    const fmtDate = d => Number.isNaN(d?.getTime()) ? "—" : d.toLocaleDateString("en-US", {
      month: "short",
      day: "numeric"
    });
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "chart-stats-grid",
      title: "Price action context for the active ticker. Helps gauge where price sits in its recent range and how stretched it is from key averages."
    }, /*#__PURE__*/React.createElement("div", {
      className: "chart-stat",
      title: "Highest price in the last 252 trading days (~1 year)"
    }, /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-lbl"
    }, "52W HIGH"), /*#__PURE__*/React.createElement("div", {
      ...
      className: "chart-stat-lbl"
    }, "52W LOW"), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-val"
      ...
      className: "chart-stat-lbl"
    }, "EMA21"), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-val"
    }, fmt$(last.ema21)), /*#__PURE__*/React.createElement("div", {
      className: `chart-stat-sub ${cls(distEMA21)}`
    }, fmtPct(distEMA21))), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat",
      title: "MA50 value and price distance. A common short-term trend reference."
    }, /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-lbl"
    }, "MA50"), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-val"
    }, fmt$(last.ma50)), /*#__PURE__*/React.createElement("div", {
      ...
      className: "chart-stat-val"
    }, fmt$(last.ma200)), /*#__PURE__*/React.createElement("div", {
      className: `chart-stat-sub ${cls(distMA200)}`
    }, fmtPct(distMA200))), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat",
      title: "14-day Relative Strength Index. Below 30 = oversold (green), above 70 = overbought (red)."
    }, /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-lbl"
    }, "RSI14"), /*#__PURE__*/React.createElement("div", {
      className: `chart-stat-val ${rsiCls}`
    }, rsi == null ? "—" : rsi.toFixed(1)), /*#__PURE__*/React.createElement("div", {
      ...
      className: "chart-stat-val"
    }, fmt$(atr)), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-sub"
    }, atr != null && lastClose > 0 ? (atr / lastClose * 100).toFixed(2) + "% of price" : "")), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat",
      title: "Average daily range as a percentage of close, last 20 days. Useful for sizing same-day trades."
    }, /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-lbl"
    }, "ADR 20D"), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-val"
    }, adr == null ? "—" : adr.toFixed(2) + "%"), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-sub"
    }, "avg high-low range")), /*#__PURE__*/React.createElement("div", {
      className: "chart-stat",
      title: ivRankInfo?.iv_rank != null ? `IV Rank from ${ivRankInfo.iv_rank_days} days of local history. Current ATM IV30 is at the ${ivRankInfo.iv_rank.toFixed(0)}% mark of its observed range. Above 70 = rich premium territory, below 30 = cheap. IV Pct ${ivRankInfo.iv_pct != null ? ivRankInfo.iv_pct.toFixed(0) + "%" : "n/a"} of days had lower IV.` : ivRankInfo?.iv_rank_days > 0 ? `Building IV history. ${ivRankInfo.iv_rank_days} of 20 days collected before rank can be computed.` : "Loading IV history…"
    }, /*#__PURE__*/React.createElement("div", {
      className: "chart-stat-lbl"
    }, "IV RANK"), /*#__PURE__*/React.createElement("div", {
      className: `chart-stat-val ${ivRankInfo?.iv_rank == null ? "" : ivRankInfo.iv_rank >= 70 ? "up" : ivRankInfo.iv_rank <= 30 ? "down" : ""}`
    ...
    className: "strikes-head-row"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    className: "kicker"
  }, "This week's setup"), /*#__PURE__*/React.createElement("div", {
    className: "card-title"
  }, "Suggested strikes")), /*#__PURE__*/React.createElement("div", {
    className: "strikes-head-right"
    ...
    const label = days < 0 ? `Past · ${dateStr}` : days === 0 ? `Today · ${dateStr}` : days <= 7 ? `This week · ${dateStr}` : days <= 14 ? `Next week · ${dateStr}` : `${dateStr} · ${days}d`;
    return /*#__PURE__*/React.createElement("div", {
    ...
    className: "strategy-mode-toggle",
    ...
    className: "strategy-mode-kicker"
  }, "Focus"), /*#__PURE__*/React.createElement("div", {
    className: "strategy-mode-pills"
  }, /*#__PURE__*/React.createElement("button", {
  ...
  }, "CSP"))))), /*#__PURE__*/React.createElement("div", {
    className: `strike-pair ${strategyMode !== "both" ? "single" : ""}`
  }, (() => {
    ...
    const flowFor = (side, strike) => {
      ...
      const matches = strikeFlow.filter(r => r.side === side && Math.abs(r.strike - strike) <= tol);
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, (strategyMode === "both" || strategyMode === "cc") && /*#__PURE__*/React.createElement("div", {
      className: `strike-card call ${manualCallStrike != null ? "manual" : ""}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "label"
    }, /*#__PURE__*/React.createElement("span", {
      className: "dot"
    }), "SELL CALL", manualCallStrike != null && /*#__PURE__*/React.createElement("span", {
      className: "manual-tag"
    }, "manual")), /*#__PURE__*/React.createElement("div", {
      className: "price-big"
    }, /*#__PURE__*/React.createElement("small", null, "$"), sugCall.toFixed(2)), /*#__PURE__*/React.createElement("div", {
      className: "meta"
      ...
      k: "oi"
    }, "OI"), " ", /*#__PURE__*/React.createElement("b", null, callAtSug.openInterest.toLocaleString())), /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement(Term, {
      k: "iv"
    }, "IV"), " ", /*#__PURE__*/React.createElement("b", null, (callAtSug.iv * 100).toFixed(1), "%"))), uwHealth?.connected && callFlow && /*#__PURE__*/React.createElement("div", {
      className: "strike-flow-row",
      title: `Unusual Whales activity at $${sugCall.toFixed(2)} call today: ${callFlow.trade_count} unusual trades. Ask-side premium = $${callFlow.ask_premium.toFixed(0)}. Volume / OI max = ${callFlow.vol_oi_max.toFixed(2)}. ${callFlow.sweep_count} sweep(s).`
    }, /*#__PURE__*/React.createElement("span", {
      ...
      className: "dot"
    }), "SELL PUT", manualPutStrike != null && /*#__PURE__*/React.createElement("span", {
      className: "manual-tag"
    }, "manual")), /*#__PURE__*/React.createElement("div", {
      className: "price-big"
    }, /*#__PURE__*/React.createElement("small", null, "$"), sugPut.toFixed(2)), /*#__PURE__*/React.createElement("div", {
      className: "meta"
    }, /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement(Term, {
      k: "premium"
      ...
      k: "oi"
    }, "OI"), " ", /*#__PURE__*/React.createElement("b", null, putAtSug.openInterest.toLocaleString())), /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement(Term, {
      k: "iv"
    }, "IV"), " ", /*#__PURE__*/React.createElement("b", null, (putAtSug.iv * 100).toFixed(1), "%"))), uwHealth?.connected && putFlow && /*#__PURE__*/React.createElement("div", {
      className: "strike-flow-row",
      title: `Unusual Whales activity at $${sugPut.toFixed(2)} put today: ${putFlow.trade_count} unusual trades. Ask-side premium = $${putFlow.ask_premium.toFixed(0)}.`
    }, /*#__PURE__*/React.createElement("span", {
      className: "strike-flow-lbl",
      title: "Today's volume in unusual trades at this strike"
    ...
    if (earnDateStr) {
      try {
        ...
        if (!isNaN(d) && d <= activeExpDate) {
          earnInside = true;
          earnLabel = `Earnings ${d.toLocaleDateString("en-US", {
            month: "short",
            day: "numeric"
          })} inside exp · CAUTION`;
          earnCls = "cat-caution";
    ...
    const otmCalls = (calls || []).filter(c => c && c.strike > currentPrice && c.delta != null && c.delta > 0 && c.bid != null && c.ask != null && c.bid > 0 && c.openInterest > 10);
    const callMidOf = c => {
      const b = c.bid || 0,
        a = c.ask || 0;
    ...
    if (otmCalls.length > 0 && currentPrice > 0) {
      const withYield = otmCalls.filter(c => callMidOf(c) >= 0.05).map(c => ({
        ...c,
        _mid: callMidOf(c),
        _yieldPct: callMidOf(c) / currentPrice * 100
      }));
      if (withYield.length > 0) {
        pickIncome = withYield.reduce((best, c) => c._yieldPct > (best?._yieldPct || 0) ? c : best, null);
    ...
    const formatPick = (label, pick, popOverride) => {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "pick-strike"
      }, "$", pick.strike.toFixed(2)), /*#__PURE__*/React.createElement("div", {
        className: "pick-row"
      }, /*#__PURE__*/React.createElement("span", null, "Mid"), /*#__PURE__*/React.createElement("b", null, "$", (pick._mid || 0).toFixed(2))), /*#__PURE__*/React.createElement("div", {
        className: "pick-row"
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: `catalyst-badge ${earnCls}`,
      title: "Earnings inside the active expiration adds risk of large gap moves and IV crush"
    }, earnLabel), /*#__PURE__*/React.createElement("div", {
      className: "pop-pot"
    }, /*#__PURE__*/React.createElement("span", {
      className: "pp-pair",
      title: "Probability the short call expires OUT-of-the-money. Higher = safer. Approx 1 - |delta|."
    }, /*#__PURE__*/React.createElement("span", {
      className: "pp-key"
      ...
      className: "pp-val"
    }, potPct != null ? potPct.toFixed(0) + "%" : "—")))), /*#__PURE__*/React.createElement("div", {
      className: "picks-row"
    }, formatPick("Best income", pickIncome), formatPick("Best balanced", pickBalanced), formatPick("Best safety", pickSafety)));
  })(), /*#__PURE__*/React.createElement("div", {
    ...
    k: baseline === "monday" ? "monday_open" : "prev_friday_close"
  }, baseline === "monday" ? "Monday Open" : "Prev Fri Close")), /*#__PURE__*/React.createElement("div", {
    className: "val"
  }, "$", baselinePrice.toFixed(2))), /*#__PURE__*/React.createElement("div", {
    className: "metric compact"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lbl"
  }, "Current"), /*#__PURE__*/React.createElement("div", {
    className: "val"
  }, "$", currentPrice.toFixed(2), /*#__PURE__*/React.createElement("span", {
    className: "delta-tiny",
    style: {
      color: stockDelta >= 0 ? "var(--up)" : "var(--down)"
    }
  }, fmtPct(stockDeltaPct, 2)))), /*#__PURE__*/React.createElement("div", {
    className: "metric compact"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "expected_high"
  }, "Expected high")), /*#__PURE__*/React.createElement("div", {
    className: "val",
    style: {
      color: "var(--up)"
    }
  }, "$", expHigh.toFixed(2))), /*#__PURE__*/React.createElement("div", {
    className: "metric compact"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "expected_low"
  }, "Expected low")), /*#__PURE__*/React.createElement("div", {
    className: "val",
    style: {
      color: "var(--down)"
    }
  }, "$", expLow.toFixed(2)))), /*#__PURE__*/React.createElement(RecommendationPair, {
    rec: rec,
    ...
    className: "jump-anchor",
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement(PositionsCard, {
    positions: positions,
    ...
    className: "jump-anchor",
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement(WinRateCard, {
    apiFetch: apiFetch
  }), /*#__PURE__*/React.createElement(PushSettingsCard, {
    apiFetch: apiFetch
  }), /*#__PURE__*/React.createElement(BrokerImportCard, {
    apiFetch: apiFetch,
    positions: positions,
    setPositions: setPositions
  })), /*#__PURE__*/React.createElement(TabPanel, {
    ...
    k: "median_high"
  }, "Median high")), /*#__PURE__*/React.createElement("div", {
    className: "val",
    style: {
      color: "var(--up)"
    }
  }, fmtPct(medianHigh)), /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      fontSize: 11,
      marginTop: 2
    }
  }, "typical rise")), /*#__PURE__*/React.createElement("div", {
    className: "metric"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "median_low"
  }, "Median low")), /*#__PURE__*/React.createElement("div", {
    className: "val",
    style: {
      color: "var(--down)"
    }
  }, fmtPct(medianLow)), /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      fontSize: 11,
      marginTop: 2
    }
  }, "typical drop")), /*#__PURE__*/React.createElement("div", {
    className: "metric"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "median_close"
  }, "Median close")), /*#__PURE__*/React.createElement("div", {
    className: "val"
  }, fmtPct(medianClose)), /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      fontSize: 11,
      marginTop: 2
    }
  }, "where Fri lands")), /*#__PURE__*/React.createElement("div", {
    className: "metric"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "typical_high_day"
  }, "Typical high day")), /*#__PURE__*/React.createElement("div", {
    className: "val",
    style: {
      color: "var(--up)"
    }
  }, typicalHighDay), /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      fontSize: 11,
      marginTop: 2
    }
  }, "most common peak")), /*#__PURE__*/React.createElement("div", {
    className: "metric"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "typical_low_day"
  }, "Typical low day")), /*#__PURE__*/React.createElement("div", {
    className: "val",
    style: {
      color: "var(--down)"
    }
  }, typicalLowDay), /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      fontSize: 11,
      marginTop: 2
  ...
  }), "Earnings week"))), /*#__PURE__*/React.createElement(ReturnsChart, {
    rows: rows,
    medianHigh: medianHigh,
    medianLow: medianLow,
    medianClose: medianClose,
    ...
    className: "kicker"
  }, "Day of week"), /*#__PURE__*/React.createElement("div", {
    className: "card-title"
  }, "When highs and lows land"))), /*#__PURE__*/React.createElement("div", {
    className: "dow-seclabel dow-seclabel--up"
  }, "High"), /*#__PURE__*/React.createElement(DayBarChart, {
    rows: rows,
    colors: chartColors,
    mode: "high"
  }), /*#__PURE__*/React.createElement("div", {
    className: "dow-seclabel dow-seclabel--down",
    style: {
      marginTop: 14
    }
  }, "Low"), /*#__PURE__*/React.createElement(DayBarChart, {
    rows: rows,
    colors: chartColors,
    mode: "low"
  }), (() => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "dow-extremes"
    }, /*#__PURE__*/React.createElement("div", {
      className: "dow-extremes-head"
    }, /*#__PURE__*/React.createElement("span", {
      className: "dow-extremes-corner"
    }), days.map(d => /*#__PURE__*/React.createElement("span", {
      key: d,
      className: `dow-extremes-daycol${d === _todayShort ? " today" : ""}`
    }, d))), /*#__PURE__*/React.createElement("div", {
      className: "dow-extremes-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "dow-extremes-label up"
    }, "Best high"), extremes.map(e => /*#__PURE__*/React.createElement("span", {
      key: e.day,
      className: `dow-extremes-cell up${e.day === _todayShort ? " today" : ""}`
    }, fmt(e.bestHigh)))), /*#__PURE__*/React.createElement("div", {
      className: "dow-extremes-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "dow-extremes-label down"
    }, "Worst low"), extremes.map(e => /*#__PURE__*/React.createElement("span", {
      key: e.day,
      className: `dow-extremes-cell down${e.day === _todayShort ? " today" : ""}`
    }, fmt(e.worstLow)))));
  })(), /*#__PURE__*/React.createElement("div", {
    className: "dow-footer"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    className: "jump-anchor",
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement(BasingCard, {
    ticker: ticker,
    weeks: weeks,
    apiFetch: apiFetch,
    ...
    className: "jump-anchor",
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement(FlowScoreCard, {
    ticker: ticker,
    ...
    label: "Big Money Map"
  }, /*#__PURE__*/React.createElement(MoneyMapCard, {
    ticker: ticker,
    currentPrice: getLivePrice(ticker) ?? currentPrice,
    apiFetch: apiFetch,
    uwHealth: uwHealth
    ...
    className: "jump-anchor",
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement(AnalystCard, {
    ticker: ticker,
    currentPrice: getLivePrice(ticker) ?? currentPrice,
    apiFetch: apiFetch,
    onData: setAnalystData,
    strategyMode: strategyMode
  }), /*#__PURE__*/React.createElement(CardErrorBoundary, {
    label: "Priced for Perfection"
  }, /*#__PURE__*/React.createElement(PerfectionCard, {
    apiFetch: apiFetch,
    ticker: ticker
  })), /*#__PURE__*/React.createElement(CardErrorBoundary, {
    label: "Credit risk"
  }, /*#__PURE__*/React.createElement(CreditRiskCard, {
    apiFetch: apiFetch,
    ticker: ticker
  }))), /*#__PURE__*/React.createElement(TabPanel, {
    tab: "trade",
    ...
    className: "jump-anchor",
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement(TradeBuilderCard, {
    ticker: ticker,
    currentPrice: currentPrice,
    callAtSug: callAtSug,
    putAtSug: putAtSug,
    ...
    apiFetch: apiFetch,
    strategyMode: strategyMode
  }), /*#__PURE__*/React.createElement(LevelRepriceCard, {
    ticker: ticker,
    currentPrice: currentPrice,
    calls: calls,
    puts: puts,
    ...
    k: "historical_range"
  }, "Avg historical range")), /*#__PURE__*/React.createElement("div", {
    className: "val"
    ...
    style: {
      ...
      border: `1px solid ${ivMove > histMove * 1.2 ? "color-mix(in oklch, var(--up), transparent 70%)" : ivMove < histMove * 0.8 ? "color-mix(in oklch, var(--warn), transparent 70%)" : "color-mix(in oklch, var(--accent), transparent 70%)"}`
    }
  }, ivMove > histMove * 1.2 ? /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("b", {
    style: {
      color: "var(--up)"
    }
  }, "Premium is rich."), " Options imply a ", (ivMove / histMove * 100 - 100).toFixed(0), "% larger move than typical. Favorable for selling.") : ivMove < histMove * 0.8 ? /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("b", {
    style: {
      color: "var(--warn)"
    }
  }, "Premium is cheap."), " The market expects a calmer week. Selling premium pays less than usual.") : /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("b", null, "In line with history."), " Implied and historical moves are within 20% of each other."))), /*#__PURE__*/React.createElement("div", {
    className: "card"
  }, /*#__PURE__*/React.createElement("div", {
    className: "card-head"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    k: "prob_profit"
  }, "Probability of profit")), /*#__PURE__*/React.createElement("div", {
    className: "card-title"
  }, "How often each side has expired worthless")), /*#__PURE__*/React.createElement("div", {
    className: "card-sub"
  }, "last ", weeks, " weeks")), /*#__PURE__*/React.createElement("div", {
    className: "prob-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "label-col"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    k: "prob_call_safe"
  }, "Stock stays ", /*#__PURE__*/React.createElement("b", null, "below $", sugCall.toFixed(2))))), /*#__PURE__*/React.createElement("div", {
    className: "bar-track"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bar-fill",
    style: {
  ...
  }, callSafePct.toFixed(0), "%")), /*#__PURE__*/React.createElement("div", {
    className: "prob-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "label-col"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    k: "prob_put_safe"
  }, "Stock stays ", /*#__PURE__*/React.createElement("b", null, "above $", sugPut.toFixed(2))))), /*#__PURE__*/React.createElement("div", {
    className: "bar-track"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bar-fill",
    style: {
    ...
    k: "prob_both"
  }, /*#__PURE__*/React.createElement("b", null, "Both"), " sides expire worthless"))), /*#__PURE__*/React.createElement("div", {
    className: "bar-track"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bar-fill",
    style: {
    ...
    className: "pct"
  }, bothSafePct.toFixed(0), "%")), /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      fontSize: 12,
      marginTop: 12,
  ...
  }, "Based on actual weekly highs and lows over the lookback window. Not implied probabilities."))), /*#__PURE__*/React.createElement("div", {
    className: "row two"
  }, (() => {
    if (!calls.length && !puts.length) return null;
    const sumVol = arr => arr.reduce((a, b) => a + (b.volume || 0), 0);
    const sumOI = arr => arr.reduce((a, b) => a + (b.openInterest || 0), 0);
    const totalCallVol = sumVol(calls),
      totalPutVol = sumVol(puts);
    const totalCallOI = sumOI(calls),
      totalPutOI = sumOI(puts);
    const pcVol = totalCallVol > 0 ? totalPutVol / totalCallVol : null;
    const pcOI = totalCallOI > 0 ? totalPutOI / totalCallOI : null;
    const topBy = (arr, key, n = 5) => [...arr].filter(x => (x[key] || 0) > 0).sort((a, b) => b[key] - a[key]).slice(0, n);
    ...
    const allStrikesMP = Array.from(new Set([...calls.map(c => c.strike), ...puts.map(p => p.strike)])).sort((a, b) => a - b);
    let maxPainStrike = null,
      minTotalValue = Infinity;
    ...
    const maxPainPct = maxPainStrike != null ? (maxPainStrike - currentPrice) / currentPrice * 100 : null;
    const expDateLabel = activeExpDate.toLocaleDateString("en-US", {
      weekday: "short",
      month: "short",
      day: "numeric"
    });
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "mp-lbl"
    }, "Max pain"), /*#__PURE__*/React.createElement("span", {
      className: "mp-strike"
      ...
      if (byStrike.size === 0) {
        return /*#__PURE__*/React.createElement("div", {
          className: "oi-chart-empty muted"
      ...
      let rows = Array.from(byStrike.entries()).map(([strike, e]) => ({
        strike,
        call: e.call,
        put: e.put,
        total: e.call + e.put
      ...
      const base = within.length >= 6 ? within : rows.sort((a, b) => b.total - a.total).slice(0, 20);
      base.sort((a, b) => b.strike - a.strike); // high strike at top
      const maxSide = Math.max(1, ...base.map(r => Math.max(r.call, r.put)));
      ...
      return /*#__PURE__*/React.createElement("div", {
        className: "oi-chart-wrap"
      }, /*#__PURE__*/React.createElement("div", {
        className: "oi-chart-controls"
      }, /*#__PURE__*/React.createElement("div", {
        className: "oi-chart-legend"
      }, /*#__PURE__*/React.createElement("span", {
        className: "oi-leg-item"
      }, /*#__PURE__*/React.createElement("span", {
        className: "oi-leg-dot",
        style: {
          background: "var(--up)"
        }
        ...
        className: `oi-metric-pill ${metric === "volume" ? "active" : ""}`,
        onClick: () => setOiChartMetric("volume"),
        title: "Today's traded contracts per strike."
      }, "Volume"), /*#__PURE__*/React.createElement("button", {
        className: `oi-metric-pill ${metric === "oi" ? "active" : ""}`,
        onClick: () => setOiChartMetric("oi"),
        title: "Total open contracts per strike."
      ...
      className: "oi-total-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "oi-label"
    }, "Total ", /*#__PURE__*/React.createElement(Term, {
      k: "volume"
    ...
    }, fmtN(totalCallVol)), /*#__PURE__*/React.createElement("span", {
      className: "oi-sep"
    }, "calls"), /*#__PURE__*/React.createElement("b", {
      style: {
        color: "var(--down)"
      }
    }, fmtN(totalPutVol)), /*#__PURE__*/React.createElement("span", {
      className: "oi-sep"
    }, "puts"), pcVol != null && /*#__PURE__*/React.createElement("span", {
      className: "oi-ratio"
    }, "P/C ", pcVol.toFixed(2)))), /*#__PURE__*/React.createElement("div", {
      className: "oi-total-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "oi-label"
    }, "Total ", /*#__PURE__*/React.createElement(Term, {
    ...
    }, fmtN(totalCallOI)), /*#__PURE__*/React.createElement("span", {
      className: "oi-sep"
    }, "calls"), /*#__PURE__*/React.createElement("b", {
      style: {
        color: "var(--down)"
      }
    }, fmtN(totalPutOI)), /*#__PURE__*/React.createElement("span", {
      className: "oi-sep"
    }, "puts"), pcOI != null && /*#__PURE__*/React.createElement("span", {
      className: "oi-ratio"
    }, "P/C ", pcOI.toFixed(2))))), /*#__PURE__*/React.createElement("div", {
      className: "oi-tops"
    }, /*#__PURE__*/React.createElement("div", {
      className: "oi-col"
    }, /*#__PURE__*/React.createElement("div", {
      className: "oi-col-head",
      style: {
        color: "var(--up)"
      }
    }, "Top call OI"), topCallOI.length ? topCallOI.map(c => /*#__PURE__*/React.createElement("div", {
      key: `coi${c.strike}`,
      className: "oi-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "oi-strike"
    }, "$", c.strike.toFixed(2)), /*#__PURE__*/React.createElement("span", {
      className: "oi-num"
    }, fmtN(c.openInterest)))) : /*#__PURE__*/React.createElement("div", {
      className: "oi-row muted"
    ...
    }, "Top put OI"), topPutOI.length ? topPutOI.map(p => /*#__PURE__*/React.createElement("div", {
      key: `poi${p.strike}`,
      className: "oi-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "oi-strike"
    }, "$", p.strike.toFixed(2)), /*#__PURE__*/React.createElement("span", {
      className: "oi-num"
    }, fmtN(p.openInterest)))) : /*#__PURE__*/React.createElement("div", {
      className: "oi-row muted"
    }, "no data")), /*#__PURE__*/React.createElement("div", {
      className: "oi-col"
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "Top call volume"), topCallVol.length ? topCallVol.map(c => /*#__PURE__*/React.createElement("div", {
      key: `cv${c.strike}`,
      className: "oi-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "oi-strike"
      ...
      className: "oi-num"
    }, fmtN(c.volume)))) : /*#__PURE__*/React.createElement("div", {
      className: "oi-row muted"
    }, "no data")), /*#__PURE__*/React.createElement("div", {
      className: "oi-col"
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "Top put volume"), topPutVol.length ? topPutVol.map(p => /*#__PURE__*/React.createElement("div", {
      key: `pv${p.strike}`,
      className: "oi-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "oi-strike"
      ...
      className: "oi-num"
    }, fmtN(p.volume)))) : /*#__PURE__*/React.createElement("div", {
      className: "oi-row muted"
    }, "no data")))));
  })())), /*#__PURE__*/React.createElement(TabPanel, {
    tab: "flow",
    ...
    className: "card market-dashboard-card",
    style: {
      marginBottom: "var(--row-gap)"
    }
  }, /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "market-tide-summary",
      title: "Current market regime read from net call vs put premium"
    }, /*#__PURE__*/React.createElement("span", {
      className: "muted"
    }, "Net call - put:"), /*#__PURE__*/React.createElement("b", {
      className: regimeCls
    }, fmt$M(net)));
  })()), marketDashOpen && /*#__PURE__*/React.createElement("div", {
    className: "market-dashboard-body"
  }, !marketDashboard && /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      padding: "12px 0"
    }
  }, "Loading market flow."), marketDashboard && /*#__PURE__*/React.createElement(React.Fragment, null, marketDashboard.sector && (() => {
    const sectors = Array.isArray(marketDashboard.sector) ? marketDashboard.sector : marketDashboard.sector.data || [];
    if (!sectors.length) return null;
    // Build sortable list with derived net premium
    const enriched = sectors.map(s => {
      const net = (s.net_call_premium != null ? s.net_call_premium : s.call_premium || 0) - (s.net_put_premium != null ? s.net_put_premium : s.put_premium || 0);
      return {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      title: "Net call premium minus net put premium per sector ETF. Bigger absolute value = bigger institutional positioning."
    }, /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "market-section",
      title: "Tickers UW flagged as having sudden volume or premium spikes."
    }, /*#__PURE__*/React.createElement("div", {
      className: "market-section-title"
    }, "Recent spikes"), /*#__PURE__*/React.createElement("div", {
      className: "spike-list"
  ...
  })(), !marketDashboard.tide && !marketDashboard.sector && !marketDashboard.spike && /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      padding: "12px 0"
    }
  ...
  }, marketScanAt && /*#__PURE__*/React.createElement("span", {
    className: "scan-stale",
    title: "When the last scan finished"
  }, "scanned ", Math.round((Date.now() - marketScanAt) / 60000), " min ago"), /*#__PURE__*/React.createElement("button", {
  ...
  }, marketScanRunning ? `Scanning. ${marketScanProgress.done}/${marketScanProgress.total}` : marketScanRows.length > 0 ? "Re-scan" : "Scan market"))), marketScanError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", marketScanError), marketScanRows.length === 0 && !marketScanRunning && !marketScanError && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Click ", /*#__PURE__*/React.createElement("b", null, "Scan market"), " to find tickers off your watchlist with unusual options flow today."), marketScanRows.length > 0 && (() => {
    // Filter out: errors, missing flow data, and "Noisy flow" verdicts
    // (low-conviction flow that doesn't deserve attention).
    const usable = marketScanRows.filter(r => !r.error && r.flow_score?.data_available && !/noisy/i.test(r.flow_score?.verdict || ""));
    const hiddenCount = marketScanRows.filter(r => !r.error && r.flow_score?.data_available && /noisy/i.test(r.flow_score?.verdict || "")).length;
    ...
    sortable.sort((a, b) => {
      ...
      switch (marketScanSort.key) {
        case "symbol":
          ...
          break;
        case "flow_score":
          av = a.flow_score?.overall;
          bv = b.flow_score?.overall;
          break;
        case "direction":
          ...
          break;
        case "verdict":
          av = a.flow_score?.verdict;
          bv = b.flow_score?.verdict;
          break;
        case "total_premium_today":
          av = a.total_premium_today;
          bv = b.total_premium_today;
          break;
        case "net_premium_today":
          av = a.net_premium_today;
          bv = b.net_premium_today;
          break;
        case "total_volume_today":
          av = a.total_volume_today;
          bv = b.total_volume_today;
          break;
        case "premium":
          av = a.total_premium;
          bv = b.total_premium;
        ...
        case "days_to_earnings":
          av = a.days_to_earnings;
          bv = b.days_to_earnings;
          break;
        case "analyst_signal":
          ...
          break;
        default:
          av = a[marketScanSort.key];
          bv = b[marketScanSort.key];
      }
    ...
    const SortHeader = ({
      ...
      return /*#__PURE__*/React.createElement("span", {
        className: `pb-th${active ? " active" : ""}`,
        title: tip,
        onClick: () => {
          if (marketScanSort.key === k) {
            setMarketScanSort({
              key: k,
              dir: marketScanSort.dir === "desc" ? "asc" : "desc"
            });
          } else {
            setMarketScanSort({
              key: k,
    ...
    const fmtEarn = (iso, days, cls) => {
      ...
      return /*#__PURE__*/React.createElement("span", {
        className: pillCls,
        title: `Next earnings on ${iso}`
      }, lbl);
    };
    return /*#__PURE__*/React.createElement("div", {
      className: "market-scan-table"
    }, /*#__PURE__*/React.createElement("div", {
      className: "market-scan-head"
    }, /*#__PURE__*/React.createElement(SortHeader, {
      label: "Ticker",
      k: "symbol",
      tip: "Click to sort alphabetically"
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Score",
      k: "flow_score",
      tip: "UW flow score 0-100. Above 65 = bullish lean. Below 35 = bearish lean. IV rank shown on hover of each cell."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Dir",
      k: "direction",
      tip: "Direction read from flow sub-scores. Bullish = ask-side calls dominating. Bearish = ask-side puts dominating."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Verdict",
      k: "verdict",
      tip: "UW decision-engine read on covered-call viability."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Total $",
      ...
      tip: "Total options contracts traded today. Hover to see call/put split."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Price",
      k: "price",
      tip: "Live price (Schwab if available)."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Chg%",
      k: "change_pct",
      tip: "Today's change vs prior close."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Analyst",
      k: "analyst_signal",
      ...
      label: "Earnings",
      k: "days_to_earnings",
      tip: "Days to next earnings. Red = within 7 days. Yellow = within 14 days."
    })), sortable.map(r => {
      const overall = r.flow_score?.overall ?? null;
      const cls = overall == null ? "fair" : overall >= 70 ? "rich" : overall >= 60 ? "moderate" : overall <= 30 ? "thin" : overall <= 40 ? "bear-mod" : "fair";
      const isActive = r.symbol === ticker;
      const chgCls = r.change_pct == null ? "" : r.change_pct > 0 ? "up" : "down";
      const direction = dirOf(r);
      const dirCls = direction === "bullish" ? "dir-bull" : direction === "bearish" ? "dir-bear" : "dir-neutral";
      const dirLabel = direction === "bullish" ? "BULL" : direction === "bearish" ? "BEAR" : "—";
      const netPrem = r.net_premium_today;
      const netCls = netPrem == null ? "" : netPrem > 0 ? "up" : netPrem < 0 ? "down" : "";
      const totalTip = r.total_premium != null && r.total_premium_today ? `Total: ${fmt$M(r.total_premium_today)} · Unusual flow: ${fmt$M(r.total_premium)} (${(r.total_premium / r.total_premium_today * 100).toFixed(0)}% of total)` : `Total premium today`;
      const volTip = r.call_volume_today != null || r.put_volume_today != null ? `Calls: ${fmtVol(r.call_volume_today)} · Puts: ${fmtVol(r.put_volume_today)}${r.put_call_ratio != null ? ` · P/C: ${r.put_call_ratio.toFixed(2)}` : ""}` : "Total options volume today";
      const scoreTip = r.iv_rank != null ? `Score ${overall ?? "—"} · IV rank ${Number(r.iv_rank).toFixed(0)}%` : `Score ${overall ?? "—"}`;
      const netTip = r.call_premium_today != null && r.put_premium_today != null ? `Calls: ${fmt$M(r.call_premium_today)} · Puts: ${fmt$M(r.put_premium_today)} · Net: ${fmt$M(netPrem)}` : "Net premium = call − put";
      ...
      const unusPct = r.total_premium != null && r.total_premium_today && r.total_premium_today > 0 ? r.total_premium / r.total_premium_today * 100 : null;
      const unusPctCls = unusPct == null ? "" : unusPct >= 50 ? "unus-high" : unusPct >= 20 ? "unus-mid" : "unus-low";
      const unusTip = `${r.alert_count ?? 0} unusual flow alert${(r.alert_count ?? 0) === 1 ? "" : "s"} today` + (unusPct != null ? ` · ${unusPct.toFixed(1)}% of total premium` : "");
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: `richness-score ${cls}`,
        title: scoreTip
      }, overall != null ? overall : "—"), /*#__PURE__*/React.createElement("span", {
        className: `scan-dir-pill ${dirCls}`,
        title: `Bullish ${r.flow_score?.bullish ?? 0} · Bearish ${r.flow_score?.bearish ?? 0}`
      }, dirLabel), /*#__PURE__*/React.createElement("span", {
        className: `richness-verdict ${r.flow_score?.verdict_class || ""}`
      }, r.flow_score?.verdict || "—"), /*#__PURE__*/React.createElement("span", {
        className: "num-strong",
        title: totalTip
      }, fmt$M(r.total_premium_today)), /*#__PURE__*/React.createElement("span", {
        title: unusTip
      }, fmt$M(r.total_premium)), /*#__PURE__*/React.createElement("span", {
        className: `unus-pct ${unusPctCls}`,
        title: unusTip
      }, unusPct != null ? unusPct.toFixed(1) + "%" : "—"), /*#__PURE__*/React.createElement("span", {
        className: netCls,
        title: netTip
      }, fmt$M(netPrem)), /*#__PURE__*/React.createElement("span", {
        title: volTip
      }, fmtVol(r.total_volume_today)), /*#__PURE__*/React.createElement("span", null, r.last_price != null ? "$" + Number(r.last_price).toFixed(2) : "—"), /*#__PURE__*/React.createElement("span", {
        className: chgCls
      }, fmtPct(r.change_pct)), /*#__PURE__*/React.createElement("span", {
        className: `analyst-cell ${analystCls}`,
        title: analystTip
      }, analystIcon), /*#__PURE__*/React.createElement("span", null, fmtEarn(r.next_earnings, r.days_to_earnings, r.earnings_class)));
    }), sortable.length === 0 && /*#__PURE__*/React.createElement("div", {
      className: "research-empty"
    }, "No usable rows. Try re-scanning when markets are open."), hiddenCount > 0 && /*#__PURE__*/React.createElement("div", {
      className: "muted",
      style: {
        padding: "8px 10px",
        fontSize: 11,
    ...
    className: "ema-dir-toggle",
    title: "Long = uptrend pullback to fast EMA. Short = downtrend bounce to fast EMA."
  }, /*#__PURE__*/React.createElement("button", {
    className: emaDirection === "long" ? "active up" : "",
    onClick: () => {
      setEmaDirection("long");
      setEmaBacktest(null);
      setEmaScan({});
    }
  }, "LONG"), /*#__PURE__*/React.createElement("button", {
    className: emaDirection === "short" ? "active down" : "",
    onClick: () => {
      setEmaDirection("short");
      setEmaBacktest(null);
      setEmaScan({});
    }
    ...
    value: emaFast,
    onChange: e => {
      const v = Math.max(2, Math.min(50, parseInt(e.target.value) || 9));
      setEmaFast(v);
      setEmaBacktest(null);
    }
  }))), /*#__PURE__*/React.createElement("div", {
  ...
  }, emaBacktestRunning ? "Running…" : emaBacktest ? "Re-run" : "Run backtest")), emaBacktestError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", emaBacktestError), !emaBacktest && !emaBacktestRunning && !emaBacktestError && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Click ", /*#__PURE__*/React.createElement("b", null, "Run backtest"), " to test the ", emaDirection, " EMA pullback strategy on ", ticker, " over the last year."), emaBacktest && !emaBacktest.error && (() => {
    const t = emaBacktest;
    // Top stats grid
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "ema-stats-grid"
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-stat",
      title: "Total trades the strategy generated over the lookback period."
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-lbl"
    }, "Trades"), /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-val"
    }, t.n_trades)), /*#__PURE__*/React.createElement("div", {
      className: "ema-stat",
      title: "Percentage of trades that were profitable."
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-lbl"
      ...
      className: `ema-stat-val ${totalRCls}`
    }, t.total_R > 0 ? "+" : "", t.total_R, "R")), /*#__PURE__*/React.createElement("div", {
      className: "ema-stat",
      title: "Expected R per trade. Above 0.10R per trade = positive edge. Above 0.30R = excellent. Negative means the strategy loses money over time."
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-lbl"
    }, "Expectancy"), /*#__PURE__*/React.createElement("div", {
      className: `ema-stat-val ${expRCls}`
    }, t.expectancy_R > 0 ? "+" : "", t.expectancy_R, "R")), /*#__PURE__*/React.createElement("div", {
      className: "ema-stat",
      title: "Average R on winning trades."
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-lbl"
    }, "Avg win"), /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-val up"
    }, "+", t.win_R_avg, "R")), /*#__PURE__*/React.createElement("div", {
      className: "ema-stat",
      title: "Average R on losing trades. Should be near -1R if stops are honored. More negative means slippage or gap-down losses beyond the stop."
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-lbl"
    }, "Avg loss"), /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-val down"
    }, t.loss_R_avg, "R")), /*#__PURE__*/React.createElement("div", {
      className: "ema-stat",
      title: "Profit factor: gross wins / gross losses. Above 1.5 = solid. Below 1.0 = losing."
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-stat-lbl"
    }, "P factor"), /*#__PURE__*/React.createElement("div", {
      ...
      className: `ema-stat-val ${retCls}`
    }, t.total_return_pct >= 0 ? "+" : "", t.total_return_pct, "%"))), t.n_trades === 0 && /*#__PURE__*/React.createElement("div", {
      className: "research-empty"
    }, "No qualifying setups in the last ", t.lookback_days, " days. The trend filter rejects choppy / range-bound stocks."), t.trades && t.trades.length > 0 && /*#__PURE__*/React.createElement("div", {
      className: "ema-trades"
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-trades-title"
    }, "Trade history (most recent first)"), /*#__PURE__*/React.createElement("div", {
      className: "ema-trades-table"
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-trades-head"
    }, /*#__PURE__*/React.createElement("span", null, "Entry"), /*#__PURE__*/React.createElement("span", null, "Exit"), /*#__PURE__*/React.createElement("span", null, "Entry $"), /*#__PURE__*/React.createElement("span", null, "Exit $"), /*#__PURE__*/React.createElement("span", null, "P&L %"), /*#__PURE__*/React.createElement("span", null, "R"), /*#__PURE__*/React.createElement("span", null, "Bars"), /*#__PURE__*/React.createElement("span", null, "Reason")), [...t.trades].reverse().slice(0, 30).map((tr, i) => {
      const pnlCls = tr.pnl_pct > 0 ? "up" : tr.pnl_pct < 0 ? "down" : "";
      return /*#__PURE__*/React.createElement("div", {
        key: i,
        className: "ema-trades-row"
      }, /*#__PURE__*/React.createElement("span", {
        className: "muted"
      }, tr.entry_date), /*#__PURE__*/React.createElement("span", {
        className: "muted"
      }, tr.exit_date), /*#__PURE__*/React.createElement("span", null, "$", tr.entry_price.toFixed(2)), /*#__PURE__*/React.createElement("span", null, "$", tr.exit_price.toFixed(2)), /*#__PURE__*/React.createElement("span", {
        className: pnlCls
      }, tr.pnl_pct > 0 ? "+" : "", tr.pnl_pct, "%"), /*#__PURE__*/React.createElement("span", {
        className: pnlCls
      }, tr.r_multiple > 0 ? "+" : "", tr.r_multiple, "R"), /*#__PURE__*/React.createElement("span", {
        className: "muted"
      }, tr.bars_held), /*#__PURE__*/React.createElement("span", {
        className: "muted"
      }, tr.exit_reason));
    }))));
  })(), emaBacktest && emaBacktest.error && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", emaBacktest.error)), /*#__PURE__*/React.createElement("div", {
    className: "ema-section",
    style: {
      marginTop: 18,
      paddingTop: 14,
    ...
    title: "When the last scan finished"
  }, "scanned ", Math.round((Date.now() - emaScanAt) / 60000), " min ago")), emaScanError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", emaScanError), !filteredWatchlistSymbols.length && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "No tickers in watchlist."), filteredWatchlistSymbols.length > 0 && Object.keys(emaScan).length === 0 && !emaScanRunning && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Click ", /*#__PURE__*/React.createElement("b", null, "Scan"), " to find which ", filteredWatchlistSymbols.length, " watchlist ", filteredWatchlistSymbols.length === 1 ? "ticker is" : "tickers are", " currently in ", emaDirection, " setup."), Object.keys(emaScan).length > 0 && (() => {
    // Group by state
    const rows = filteredWatchlistSymbols.map(sym => emaScan[sym] || {
      symbol: sym,
      state: "pending"
    }).filter(r => !r.error);
    const order = {
    ...
    const sorted = [...rows].sort((a, b) => (order[a.state] ?? 99) - (order[b.state] ?? 99) || a.symbol.localeCompare(b.symbol));
    const stateCounts = {};
    sorted.forEach(r => {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "ema-state-summary"
    }, ["confirmed", "pulled_back", "in_trend", "no_trend"].map(s => stateCounts[s] > 0 && /*#__PURE__*/React.createElement("span", {
      key: s,
      className: `ema-state-pill ${stateCls(s)}`,
      title: stateLabel(s)
    }, stateCounts[s], " ", s.replace("_", " ")))), /*#__PURE__*/React.createElement("div", {
      className: "ema-scan-table"
    }, /*#__PURE__*/React.createElement("div", {
      className: "ema-scan-head"
    }, /*#__PURE__*/React.createElement("span", null, "Ticker"), /*#__PURE__*/React.createElement("span", null, "State"), /*#__PURE__*/React.createElement("span", null, "Close"), /*#__PURE__*/React.createElement("span", null, emaFast, " EMA"), /*#__PURE__*/React.createElement("span", null, "21 EMA"), /*#__PURE__*/React.createElement("span", null, "50 EMA"), /*#__PURE__*/React.createElement("span", null, "RSI"), /*#__PURE__*/React.createElement("span", null, "Stop")), sorted.filter(r => r.state === "confirmed" || r.state === "pulled_back" || r.state === "in_trend").map(r => {
      const isActive = r.symbol === ticker;
      return /*#__PURE__*/React.createElement("div", {
        key: r.symbol,
        ...
        className: "ema-state-text"
      }, r.state.replace("_", " ")), /*#__PURE__*/React.createElement("span", null, r.close != null ? "$" + r.close.toFixed(2) : "—"), /*#__PURE__*/React.createElement("span", null, r.ema9 != null ? "$" + r.ema9.toFixed(2) : "—"), /*#__PURE__*/React.createElement("span", null, r.ema21 != null ? "$" + r.ema21.toFixed(2) : "—"), /*#__PURE__*/React.createElement("span", null, r.ema50 != null ? "$" + r.ema50.toFixed(2) : "—"), /*#__PURE__*/React.createElement("span", null, r.rsi14 != null ? r.rsi14.toFixed(1) : "—"), /*#__PURE__*/React.createElement("span", {
        className: "num-strong"
      }, r.suggested_stop != null ? "$" + r.suggested_stop.toFixed(2) : "—"));
    })));
  })()))), /*#__PURE__*/React.createElement(CardErrorBoundary, {
    ...
    title: "Switch between short and long open-trade scanners"
  }, /*#__PURE__*/React.createElement("button", {
    className: pbScanDir === "short" ? "active" : "",
    onClick: () => setPbScanDir("short"),
    title: "Rank for short-the-open setups"
  }, "Short"), /*#__PURE__*/React.createElement("button", {
    className: pbScanDir === "long" ? "active" : "",
    onClick: () => setPbScanDir("long"),
    title: "Rank for buy-the-open setups"
  }, "Long")), pullbackScanAt && /*#__PURE__*/React.createElement("span", {
    className: "scan-stale"
  }, "scanned ", Math.round((Date.now() - pullbackScanAt) / 60000), " min ago"), /*#__PURE__*/React.createElement("button", {
    className: "research-run-btn",
    disabled: pullbackScanRunning || !filteredWatchlistSymbols.length,
    onClick: runPullbackScan
  }, pullbackScanRunning ? `Scanning. ${pullbackScanProgress.done}/${pullbackScanProgress.total}` : Object.keys(pullbackScan).length > 0 ? "Re-scan" : `Scan ${filteredWatchlistSymbols.length}`))), pullbackScanError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", pullbackScanError), !filteredWatchlistSymbols.length && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, watchlistData.symbols.length === 0 ? "No tickers in watchlist. Click Manage in the sidebar." : `No tickers match the "${watchlistTagFilter}" tag.`), filteredWatchlistSymbols.length > 0 && Object.keys(pullbackScan).length === 0 && !pullbackScanRunning && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Click ", /*#__PURE__*/React.createElement("b", null, "Scan"), " to compute open-to-", pbScanDir === "short" ? "low pullback" : "high pop", " stats for ", filteredWatchlistSymbols.length, " ", filteredWatchlistSymbols.length === 1 ? "ticker" : "tickers", ". Allow ~1-2 seconds per symbol."), Object.keys(pullbackScan).length > 0 && (() => {
    const isShortScan = pbScanDir === "short";
    const rows = filteredWatchlistSymbols.map(sym => pullbackScan[sym] || {
      symbol: sym,
      pending: true
    ...
    const medianTip = isShortScan ? "Median open-to-low pullback across all days. Click to sort." : "Median open-to-high pop across all days. Click to sort.";
    const p75Tip = isShortScan ? "75th percentile open-to-low pullback. 25% of historical days exceeded this. Click to sort." : "75th percentile open-to-high pop. 25% of historical days exceeded this. Click to sort.";
    const eqTip = isShortScan ? "Frequency the open was the day's low (gap-and-go rate). Higher = more risk for shorts. Click to sort." : "Frequency the open was the day's high (no pop). Higher = more risk for longs. Click to sort.";
    const gapMedianTip = isShortScan ? "Median open-to-low pullback on gap-up days only (≥1%). Click to sort." : "Median open-to-high pop on gap-down days only (≤-1%). Click to sort.";
    const gapEqTip = isShortScan ? "Frequency of gap-and-go behavior on gap-up days. Higher = more risk. Click to sort." : "Frequency of open=high on gap-down days. Higher = more risk. Click to sort.";
    return /*#__PURE__*/React.createElement("div", {
      className: "pullback-scan-table"
    }, /*#__PURE__*/React.createElement("div", {
      className: "pullback-scan-head"
    }, /*#__PURE__*/React.createElement(SortHeader, {
      label: "Ticker",
      k: "symbol",
      tip: "Click to sort alphabetically by ticker symbol"
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Median",
      k: "_median",
      tip: medianTip
      ...
      label: "Days history",
      k: "samples",
      tip: "Number of historical trading days in the sample. Click to sort."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Verdict",
      k: "_score",
      tip: "Open-trade verdict. Click to sort by attractiveness score."
    })), sortable.map(r => {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "ticker-cell"
      }, r.symbol), /*#__PURE__*/React.createElement("span", null, r._median != null ? r._median.toFixed(2) + "%" : "—"), /*#__PURE__*/React.createElement("span", null, r._p75 != null ? r._p75.toFixed(2) + "%" : "—"), /*#__PURE__*/React.createElement("span", null, r._eqRate != null ? r._eqRate.toFixed(2) + "%" : "—"), /*#__PURE__*/React.createElement("span", null, r._gapMedian != null ? r._gapMedian.toFixed(2) + "%" : "—"), /*#__PURE__*/React.createElement("span", null, r._gapEqRate != null ? r._gapEqRate.toFixed(2) + "%" : "—"), /*#__PURE__*/React.createElement("span", {
        className: "muted"
      }, r.samples), /*#__PURE__*/React.createElement("span", {
        className: `pullback-scan-verdict ${cls}`
      }, verdict));
    }), sortable.length === 0 && /*#__PURE__*/React.createElement("div", {
      className: "research-empty"
    }, "No usable rows yet."));
    ...
    className: "research-controls"
  }, momentumAt && /*#__PURE__*/React.createElement("span", {
    className: "scan-stale",
    title: "When the last scan finished"
  }, "scanned ", Math.round((Date.now() - momentumAt) / 60000), " min ago"), /*#__PURE__*/React.createElement("button", {
    className: "research-run-btn",
  ...
  }, momentumRunning ? `Scanning. ${momentumProgress.done}/${momentumProgress.total}` : Object.keys(momentumScan).length > 0 ? "Re-scan" : `Scan ${filteredWatchlistSymbols.length}`))), momentumError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", momentumError), !filteredWatchlistSymbols.length && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "No tickers in watchlist."), filteredWatchlistSymbols.length > 0 && Object.keys(momentumScan).length === 0 && !momentumRunning && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Click ", /*#__PURE__*/React.createElement("b", null, "Scan"), " to rank ", filteredWatchlistSymbols.length, " ", filteredWatchlistSymbols.length === 1 ? "ticker" : "tickers", " by intraday momentum + UW flow."), Object.keys(momentumScan).length > 0 && (() => {
    const rows = filteredWatchlistSymbols.map(sym => momentumScan[sym] || {
      symbol: sym,
      pending: true
      ...
      return /*#__PURE__*/React.createElement("span", {
        ...
        onClick: () => {
          if (momentumSort.key === k) {
            setMomentumSort({
              key: k,
              dir: momentumSort.dir === "desc" ? "asc" : "desc"
            });
          } else {
            setMomentumSort({
              key: k,
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "momentum-table"
    }, /*#__PURE__*/React.createElement("div", {
      className: "momentum-head"
    }, /*#__PURE__*/React.createElement(SortHeader, {
      label: "Ticker",
      k: "symbol",
      tip: "Click to sort alphabetically"
      ...
      label: "Score",
      k: "score",
      tip: "Momentum score 0-100. Above 65 = bullish setup. Below 35 = bearish setup."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Verdict",
      k: "verdict",
      tip: "Plain-English read combining price action and flow."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Gap",
      k: "gap_pct",
      tip: "Today's open vs prior close."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "From open",
      k: "from_open_pct",
      tip: "Current price vs today's open. Holding gains = bullish, fading = bearish."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "RVOL",
      k: "rvol",
      ...
      label: "Flow",
      k: "flow_overall",
      tip: "UW Flow Score (50 = neutral, >65 = bullish, <35 = bearish)."
    })), sortable.map(r => {
      const cls = r.score >= 70 ? "rich" : r.score >= 60 ? "moderate" : r.score <= 30 ? "thin" : r.score <= 40 ? "bear-mod" : "fair";
      const isActive = r.symbol === ticker;
      const fromOpenCls = r.stats?.from_open_pct == null ? "" : r.stats.from_open_pct > 0 ? "up" : "down";
      const gapCls = r.stats?.gap_pct == null ? "" : r.stats.gap_pct > 0 ? "up" : "down";
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: `richness-verdict ${r.verdict_class || ""}`
      }, r.verdict), /*#__PURE__*/React.createElement("span", {
        className: gapCls
      }, fmtPct(r.stats?.gap_pct)), /*#__PURE__*/React.createElement("span", {
        className: fromOpenCls
      }, fmtPct(r.stats?.from_open_pct)), /*#__PURE__*/React.createElement("span", {
        className: rvolCls
      }, r.stats?.rvol != null ? r.stats.rvol.toFixed(2) + "×" : "—"), /*#__PURE__*/React.createElement("span", null, r.stats?.flow_overall != null ? r.stats.flow_overall : "—"));
    }), sortable.length === 0 && /*#__PURE__*/React.createElement("div", {
      className: "research-empty"
    }, "No usable rows. Markets may be closed or data unavailable."));
  })())), uwHealth?.connected && /*#__PURE__*/React.createElement(CardErrorBoundary, {
    label: "Premium richness scanner"
  }, /*#__PURE__*/React.createElement("div", {
    className: "card",
    style: {
      marginBottom: "var(--row-gap)"
    ...
    className: "research-controls"
  }, richnessAt && /*#__PURE__*/React.createElement("span", {
    className: "scan-stale",
    title: "When the last scan finished"
  }, "scanned ", Math.round((Date.now() - richnessAt) / 60000), " min ago"), /*#__PURE__*/React.createElement("button", {
    className: "research-run-btn",
  ...
  }, richnessRunning ? `Scanning. ${richnessProgress.done}/${richnessProgress.total}` : Object.keys(richnessScan).length > 0 ? "Re-scan" : `Scan ${filteredWatchlistSymbols.length}`))), richnessError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", richnessError), !filteredWatchlistSymbols.length && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "No tickers in watchlist. Click Manage in the sidebar."), filteredWatchlistSymbols.length > 0 && Object.keys(richnessScan).length === 0 && !richnessRunning && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Click ", /*#__PURE__*/React.createElement("b", null, "Scan"), " to check premium richness for ", filteredWatchlistSymbols.length, " ", filteredWatchlistSymbols.length === 1 ? "ticker" : "tickers", "."), Object.keys(richnessScan).length > 0 && (() => {
    const rows = filteredWatchlistSymbols.map(sym => richnessScan[sym] || {
      symbol: sym,
      pending: true
    }).filter(r => !r.error && !r.pending && r.data_available);
    const sortable = [...rows];
      ...
      return /*#__PURE__*/React.createElement("span", {
        ...
        onClick: () => {
          if (richnessSort.key === k) {
            setRichnessSort({
              key: k,
              dir: richnessSort.dir === "desc" ? "asc" : "desc"
            });
          } else {
            setRichnessSort({
              key: k,
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "richness-table"
    }, /*#__PURE__*/React.createElement("div", {
      className: "richness-head"
    }, /*#__PURE__*/React.createElement(SortHeader, {
      label: "Ticker",
      k: "symbol",
      tip: "Click to sort alphabetically"
      ...
      label: "Score",
      k: "score",
      tip: "Premium Richness Score 0-100. Higher = better conditions for selling premium. Click to sort."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Verdict",
      k: "verdict",
      tip: "Plain-English read on whether premium is rich, fair, or thin."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "IV rank",
      k: "iv_rank",
      tip: "IV rank from UW (when available). High = premium rich vs the stock's own history."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "Volume",
      k: "total_volume",
      tip: "Today's total options volume from UW."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "vs Avg",
      k: "rel_vol",
      tip: "Today's volume divided by average. High relative volume usually means a catalyst is driving flow."
    }), /*#__PURE__*/React.createElement(SortHeader, {
      label: "P/C",
      k: "put_call_ratio",
      ...
      const isActive = r.symbol === ticker;
      const relVol = r.stats?.total_volume && r.stats?.avg_volume && r.stats.avg_volume > 0 ? r.stats.total_volume / r.stats.avg_volume : null;
      return /*#__PURE__*/React.createElement("div", {
        key: r.symbol,
        ...
        className: `richness-verdict ${r.verdict_class || ""}`
      }, r.verdict), /*#__PURE__*/React.createElement("span", null, r.stats?.iv_rank != null ? fmtPct(r.stats.iv_rank) : r.stats?.iv_percentile != null ? fmtPct(r.stats.iv_percentile) : "—"), /*#__PURE__*/React.createElement("span", null, fmtVol(r.stats?.total_volume)), /*#__PURE__*/React.createElement("span", {
        className: relVol != null && relVol >= 1.5 ? "warn" : ""
      }, relVol != null ? relVol.toFixed(2) + "×" : "—"), /*#__PURE__*/React.createElement("span", null, r.stats?.put_call_ratio != null ? fmt(r.stats.put_call_ratio) : "—"));
    }), sortable.length === 0 && /*#__PURE__*/React.createElement("div", {
      className: "research-empty"
    }, "No tickers returned usable data. UW basic plan may not include all metrics for every symbol."));
  })())), /*#__PURE__*/React.createElement(CardErrorBoundary, {
    ...
    const convictionFor = snap => {
      ...
      let cRich = 0;
      if (richness != null) {
        if (richness >= 1.5) cRich = 1;else if (richness >= 0.9) cRich = (richness - 0.9) / 0.6;else cRich = 0;
      }
      ...
      if (snap.dte_front != null) {
        ...
        if (d >= 30 && d <= 45) cDte = 1;else if (d >= 14 && d < 30) cDte = (d - 14) / 16;else if (d > 45 && d <= 75) cDte = 1 - (d - 45) / 30;else cDte = 0;
      }
      ...
      if (snap.change_pct != null) {
        ...
        if (m <= 1.5) cCalm = 1;else if (m <= 4) cCalm = 1 - (m - 1.5) / 2.5;else cCalm = 0;
      }
      const score = cRich * 0.30 + cRank * 0.25 + cDte * 0.15 + cEarn * 0.15 + cCalm * 0.15;
    ...
    const numKey = (row, key) => {
      ...
      switch (key) {
        case "ticker":
          return row.symbol;
        case "price":
          return row.snap.price;
        case "change":
          return row.snap.change_pct;
        case "iv30":
          return row.snap.iv30_avg;
        case "hv20":
          return row.snap.hv20;
        case "richness":
          return row.snap.richness;
        case "iv_rank":
          return row.snap.iv_rank;
        case "earn":
          return row.snap.earnings_in_days;
        case "strategy":
          return row.best?.label || "";
        case "conv":
          return row.conviction;
        case "score":
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, watchlistTagFilter && /*#__PURE__*/React.createElement("button", {
      ...
      return /*#__PURE__*/React.createElement("select", {
        ...
        value: ""
      }, "All tags (", watchlistData.symbols.length, ")"), allTags.map(t => {
    ...
    })(), scanAt && /*#__PURE__*/React.createElement("span", {
      className: "scan-stale"
    }, "scanned ", Math.round((Date.now() - scanAt) / 60000), " min ago"), /*#__PURE__*/React.createElement("button", {
      className: "scan-run-btn",
      disabled: scanRunning || !filteredWatchlistSymbols.length,
      onClick: runScan
    }, scanRunning ? "Scanning." : haveAny ? "Re-scan" : "Run scan", watchlistTagFilter && ` (${filteredWatchlistSymbols.length})`))), !filteredWatchlistSymbols.length && /*#__PURE__*/React.createElement("div", {
      className: "scan-empty"
    }, watchlistData.symbols.length === 0 ? "No tickers in watchlist. Click Manage in the sidebar to add some." : `No tickers match the "${watchlistTagFilter}" tag.`), filteredWatchlistSymbols.length > 0 && !haveAny && !scanRunning && /*#__PURE__*/React.createElement("div", {
      className: "scan-empty"
    }, "Click ", /*#__PURE__*/React.createElement("b", null, "Run scan"), " to score ", filteredWatchlistSymbols.length, " ", filteredWatchlistSymbols.length === 1 ? "ticker" : "tickers", ". Allow ~1 second per symbol."), haveAny && /*#__PURE__*/React.createElement("div", {
      className: "scan-table-wrap"
    }, /*#__PURE__*/React.createElement("table", {
      className: "scan-table"
    }, /*#__PURE__*/React.createElement("thead", null, /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement(SortableTh, {
      label: "Ticker",
      sortKey: "ticker",
      current: scanSort,
      ...
      onSort: k => setScanSort(prev => cycleSort(prev, k)),
      className: "scan-th-num"
    }))), /*#__PURE__*/React.createElement("tbody", null, sorted.map(({
      symbol: sym,
      snap,
      best,
      conviction
      ...
      if (!snap) {
        return /*#__PURE__*/React.createElement("tr", {
          key: sym,
          className: "scan-row scan-row-pending",
          onClick: () => {
            setTicker(sym);
            setTickerInput(sym);
          }
      ...
      if (snap.error) {
        return /*#__PURE__*/React.createElement("tr", {
          key: sym,
          className: "scan-row scan-row-err",
          onClick: () => {
            setTicker(sym);
            setTickerInput(sym);
          }
      ...
      const ivRkTip = ivRk == null ? ivRkDays > 0 ? `Building IV history. ${ivRkDays} of 20 days collected before rank can be computed.` : "No IV history yet. Rank is computed from local snapshots, growing as you scan." : `IV Rank from ${ivRkDays} days of local history. Current ATM IV30 is at the ${ivRk.toFixed(0)} percent mark of its observed range. IV Pct ${snap.iv_pct != null ? snap.iv_pct.toFixed(0) + "%" : "n/a"} of days had lower IV. Above 70 = rich, below 30 = cheap.`;
      const earnD = snap.earnings_in_days;
      ...
      return /*#__PURE__*/React.createElement("tr", {
        key: sym,
        className: `scan-row ${sym === ticker ? "scan-row-active" : ""}`,
        onClick: () => {
          setTicker(sym);
          setTickerInput(sym);
        },
        ...
        className: "scan-num"
      }, "$", (getLivePrice(sym) ?? snap.price).toFixed(2)), /*#__PURE__*/React.createElement("td", {
        className: `scan-num ${chg >= 0 ? "up" : "down"}`
      }, chg >= 0 ? "+" : "", chg.toFixed(2), "%"), /*#__PURE__*/React.createElement("td", {
        className: "scan-num"
      }, snap.iv30_avg != null ? `${(snap.iv30_avg * 100).toFixed(0)}%` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "scan-num"
      }, snap.hv20 != null ? `${(snap.hv20 * 100).toFixed(0)}%` : "—"), /*#__PURE__*/React.createElement("td", {
        className: `scan-num ${richClass}`
      }, richness != null ? richness.toFixed(2) : "—"), /*#__PURE__*/React.createElement("td", {
        className: `scan-num ${ivRkClass}`,
        title: ivRkTip
      }, ivRk != null ? `${ivRk.toFixed(0)}` : "—"), /*#__PURE__*/React.createElement("td", {
        className: `scan-num ${earnClass}`
      }, earnD != null ? `${earnD}d` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "scan-strat"
      }, best ? best.name : "—"), /*#__PURE__*/React.createElement("td", {
        className: `scan-num scan-conv ${convClass}`
      }, conviction != null ? conviction : "—"), /*#__PURE__*/React.createElement("td", {
        className: `scan-num ${scoreClass}`
      }, best ? best.score : "—"));
    ...
    className: "research-controls"
  }, weeklyRangeAt && /*#__PURE__*/React.createElement("span", {
    className: "scan-stale"
  }, "scanned ", Math.round((Date.now() - weeklyRangeAt) / 60000), " min ago"), /*#__PURE__*/React.createElement("button", {
    className: "research-run-btn",
    disabled: weeklyRangeRunning || !filteredWatchlistSymbols.length,
    onClick: runWeeklyRange
  }, weeklyRangeRunning ? `Scanning. ${weeklyRangeProgress.done}/${weeklyRangeProgress.total}` : Object.keys(weeklyRange).length > 0 ? "Re-scan" : `Scan ${filteredWatchlistSymbols.length}`))), weeklyRangeError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", weeklyRangeError), !filteredWatchlistSymbols.length && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, watchlistData.symbols.length === 0 ? "No tickers in watchlist. Click Manage in the sidebar." : `No tickers match the "${watchlistTagFilter}" tag.`), filteredWatchlistSymbols.length > 0 && Object.keys(weeklyRange).length === 0 && !weeklyRangeRunning && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Click ", /*#__PURE__*/React.createElement("b", null, "Scan"), " to compute implied weekly range and 0.20 delta strikes for ", filteredWatchlistSymbols.length, " ", filteredWatchlistSymbols.length === 1 ? "ticker" : "tickers", ". Allow ~1-3 seconds per symbol."), Object.keys(weeklyRange).length > 0 && (() => {
    // Build sorted rows — symbols in current filter order, missing ones at bottom
    const baseRows = filteredWatchlistSymbols.map(sym => weeklyRange[sym] || {
      symbol: sym,
      pending: true
    });
    const wrKey = (row, key) => {
      ...
      switch (key) {
        ...
        case "spot":
          return row.spot;
        case "move":
          return row.implied_move_pct;
        case "low":
          return row.implied_low;
        case "high":
          return row.implied_high;
        case "put_strike":
          return row.put_strike_20d;
        case "put_credit":
          return row.put_credit_20d;
        case "call_strike":
          return row.call_strike_20d;
        case "call_credit":
          return row.call_credit_20d;
        case "total":
          return row.total_credit_20d;
        default:
          return null;
      }
    };
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      if (row.pending) {
        return /*#__PURE__*/React.createElement("tr", {
          key: row.symbol,
          className: "wr-row-pending"
        }, /*#__PURE__*/React.createElement("td", null, /*#__PURE__*/React.createElement("span", {
          className: "wr-sym"
        }, row.symbol)), /*#__PURE__*/React.createElement("td", {
          colSpan: 9,
          className: "wr-pending-cell"
        }, "Queued."));
      }
      if (row.error) {
        return /*#__PURE__*/React.createElement("tr", {
          key: row.symbol,
          className: "wr-row-err"
        }, /*#__PURE__*/React.createElement("td", null, /*#__PURE__*/React.createElement("button", {
          className: "wr-sym-btn",
          onClick: () => {
            setTicker(row.symbol);
            setTickerInput(row.symbol);
          }
        }, row.symbol)), /*#__PURE__*/React.createElement("td", {
          colSpan: 9,
          className: "wr-err-cell"
        }, "Error: ", row.error));
      }
      return /*#__PURE__*/React.createElement("tr", {
        key: row.symbol
        ...
        className: "num"
      }, "$", row.spot != null ? row.spot.toFixed(2) : "—"), /*#__PURE__*/React.createElement("td", {
        className: "num"
      }, row.implied_move_pct != null ? `±${row.implied_move_pct.toFixed(2)}%` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "num down"
      }, row.implied_low != null ? `$${row.implied_low.toFixed(2)}` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "num up"
      }, row.implied_high != null ? `$${row.implied_high.toFixed(2)}` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "num"
      }, row.put_strike_20d != null ? `$${row.put_strike_20d.toFixed(0)}` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "num"
      }, row.put_credit_20d != null ? `$${row.put_credit_20d.toFixed(2)}` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "num"
      }, row.call_strike_20d != null ? `$${row.call_strike_20d.toFixed(0)}` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "num"
      }, row.call_credit_20d != null ? `$${row.call_credit_20d.toFixed(2)}` : "—"), /*#__PURE__*/React.createElement("td", {
        className: "num up"
    ...
    onClick: async () => {
      ...
      try {
        ...
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        const data = await r.json();
        if (data.error) setEarningsError(data.error);else setEarningsLadder(data);
      } catch (e) {
  ...
  }, earningsLoading ? "Loading…" : earningsLadder ? "Refresh" : "Run analysis")), !earningsLadder && !earningsError && !earningsLoading && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Click ", /*#__PURE__*/React.createElement("b", null, "Run analysis"), " to compute implied vs realized moves for the past 8 earnings reports on ", ticker, "."), earningsError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", earningsError), earningsLadder && earningsLadder.events && earningsLadder.events.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "ladder-summary"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat-lbl"
  }, "Sellers won"), /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat-val"
  }, earningsLadder.summary.sellers_pct, "% ", /*#__PURE__*/React.createElement("span", {
    className: "ladder-stat-sub"
  }, "of ", earningsLadder.summary.n))), /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat-lbl"
  }, "Avg implied move"), /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat-val"
  }, earningsLadder.summary.avg_implied, "%")), /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat-lbl"
  }, "Avg realized move"), /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat-val"
  }, earningsLadder.summary.avg_realized, "%")), /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ladder-stat-lbl"
    ...
    className: "num"
  }, e.iv_proxy_pct, "%"), /*#__PURE__*/React.createElement("td", {
    className: "num"
  }, e.implied_move_pct, "%"), /*#__PURE__*/React.createElement("td", {
    className: "num"
  }, e.realized_move_pct, "%"), /*#__PURE__*/React.createElement("td", {
    className: `num ${e.edge_pct >= 0 ? "up" : "down"}`
  }, e.edge_pct >= 0 ? "+" : "", e.edge_pct, "%"), /*#__PURE__*/React.createElement("td", {
    className: "ladder-winner"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    value: "covered_call"
  }, "Covered Call"), /*#__PURE__*/React.createElement("option", {
    value: "cash_secured_put"
  }, "Cash-Secured Put"), /*#__PURE__*/React.createElement("option", {
    value: "short_strangle"
  }, "Short Strangle"), /*#__PURE__*/React.createElement("option", {
    value: "iron_condor"
  }, "Iron Condor"), /*#__PURE__*/React.createElement("option", {
    value: "bull_put_spread"
  }, "Bull Put Spread"), /*#__PURE__*/React.createElement("option", {
    value: "jade_lizard"
  }, "Jade Lizard"), /*#__PURE__*/React.createElement("option", {
    value: "wheel"
  }, "Wheel (CSP only)")), /*#__PURE__*/React.createElement("select", {
    className: "research-select",
    value: backtestWeeks,
    onChange: e => setBacktestWeeks(Number(e.target.value))
  }, /*#__PURE__*/React.createElement("option", {
    value: 26
  }, "26 weeks"), /*#__PURE__*/React.createElement("option", {
    value: 52
  }, "52 weeks"), /*#__PURE__*/React.createElement("option", {
    value: 104
  }, "2 years")), /*#__PURE__*/React.createElement("button", {
    className: "research-run-btn",
    ...
    onClick: async () => {
      ...
      try {
        ...
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        const data = await r.json();
        if (data.error) setBacktestError(data.error);else setBacktest(data);
      } catch (e) {
  ...
  }, backtestLoading ? "Running…" : backtest ? "Refresh" : "Run backtest"))), !backtest && !backtestError && !backtestLoading && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
  }, "Pick a strategy and click ", /*#__PURE__*/React.createElement("b", null, "Run backtest"), ". Each Monday the system \"opens\" the strategy at 0.20 delta strikes, holds to Friday, and marks P/L from synthetic prices."), backtestError && /*#__PURE__*/React.createElement("div", {
    className: "research-error"
  }, "Error: ", backtestError), backtest && backtest.summary && backtest.trades && backtest.trades.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "bt-summary"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bt-stat"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, backtest.summary.win_rate_pct, "% ", /*#__PURE__*/React.createElement("span", {
    className: "bt-stat-sub"
  }, backtest.summary.wins, "/", backtest.summary.n_cycles))), /*#__PURE__*/React.createElement("div", {
    className: "bt-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bt-stat-lbl"
  }, "Total P/L per share"), /*#__PURE__*/React.createElement("div", {
    className: `bt-stat-val ${backtest.summary.total_pl >= 0 ? "up" : "down"}`
  }, backtest.summary.total_pl >= 0 ? "+" : "", "$", backtest.summary.total_pl)), /*#__PURE__*/React.createElement("div", {
    className: "bt-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bt-stat-lbl"
  }, "Annualized return"), /*#__PURE__*/React.createElement("div", {
    className: `bt-stat-val ${backtest.summary.annual_return_pct >= 0 ? "up" : "down"}`
  }, backtest.summary.annual_return_pct >= 0 ? "+" : "", backtest.summary.annual_return_pct, "%")), /*#__PURE__*/React.createElement("div", {
    className: "bt-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bt-stat-lbl"
  }, "Avg P/L per cycle"), /*#__PURE__*/React.createElement("div", {
    className: `bt-stat-val ${backtest.summary.avg_pl >= 0 ? "up" : "down"}`
  }, backtest.summary.avg_pl >= 0 ? "+" : "", "$", backtest.summary.avg_pl)), /*#__PURE__*/React.createElement("div", {
    className: "bt-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bt-stat-lbl"
    ...
    className: "bt-stat-lbl"
  }, "Sharpe (approx)"), /*#__PURE__*/React.createElement("div", {
    className: "bt-stat-val"
  }, backtest.summary.sharpe_approx))), (() => {
    const cum = [];
    let s = 0;
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      d: fillPath,
      fill: last >= 0 ? chartColors.up : chartColors.down,
      fillOpacity: "0.18"
    }), /*#__PURE__*/React.createElement("path", {
      d: `M ${path}`,
      fill: "none",
      stroke: last >= 0 ? chartColors.up : chartColors.down,
      strokeWidth: "2"
    }), /*#__PURE__*/React.createElement("text", {
      x: pL - 6,
      y: ys(hi) + 4,
      ...
      fill: chartColors.fg3,
      fontFamily: "ui-monospace, monospace"
    }, backtest.trades[backtest.trades.length - 1].friday), /*#__PURE__*/React.createElement("text", {
      x: pL,
      y: H - pB + 14,
      fontSize: "10",
      fill: chartColors.fg3,
      fontFamily: "ui-monospace, monospace"
    }, backtest.trades[0].monday)));
  ...
  }, /*#__PURE__*/React.createElement("td", null, t.monday), /*#__PURE__*/React.createElement("td", null, t.friday), /*#__PURE__*/React.createElement("td", {
    className: "num"
  }, "$", t.spot_open), /*#__PURE__*/React.createElement("td", {
    className: "num"
  }, "$", t.spot_close), /*#__PURE__*/React.createElement("td", {
    className: "num"
  }, (t.iv * 100).toFixed(0), "%"), /*#__PURE__*/React.createElement("td", {
    className: `num ${t.credit >= 0 ? "up" : "down"}`
  }, t.credit >= 0 ? "+" : "", "$", t.credit), /*#__PURE__*/React.createElement("td", {
    className: "num"
  }, "$", t.exp_value), /*#__PURE__*/React.createElement("td", {
    className: `num ${t.pl >= 0 ? "up" : "down"}`
  }, t.pl >= 0 ? "+" : "", "$", t.pl)))))), /*#__PURE__*/React.createElement("div", {
    className: "research-disclaimer"
  }, "Backtest uses synthetic option prices (Black-Scholes from HV20) and assumes hold-to-expiration. Real-world P/L will be ~3-8% lower per cycle due to bid/ask spread + slippage. Use this for relative strategy comparison and methodology validation, not absolute P/L expectations.")), backtest && backtest.trades && backtest.trades.length === 0 && /*#__PURE__*/React.createElement("div", {
    className: "research-empty"
    ...
    for (const K of allK) {
      ...
      const callContribution = c && c.gamma && c.openInterest ? c.gamma * c.openInterest * 100 * S * S * 0.01 : 0;
      const putContribution = p && p.gamma && p.openInterest ? -p.gamma * p.openInterest * 100 * S * S * 0.01 : 0;
    ...
    const topStrikes = [...gex].filter(g => g.value !== 0).sort((a, b) => Math.abs(b.value) - Math.abs(a.value)).slice(0, 5);
    const fmtGex = v => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: `gex-total ${totalGex >= 0 ? "pos" : "neg"}`
    }, /*#__PURE__*/React.createElement("span", {
      className: "gex-total-lbl"
    }, "Net GEX"), /*#__PURE__*/React.createElement("span", {
      className: "gex-total-val"
    }, fmtGex(totalGex)), /*#__PURE__*/React.createElement("span", {
      className: "gex-total-tag"
    }, "/ 1% move")), /*#__PURE__*/React.createElement("div", {
      className: "gex-refresh"
    }, /*#__PURE__*/React.createElement("button", {
      ...
      value: "60"
    }, "auto 1m"), /*#__PURE__*/React.createElement("option", {
      value: "300"
    }, "auto 5m"), /*#__PURE__*/React.createElement("option", {
      value: "900"
    }, "auto 15m")), lastFetched && /*#__PURE__*/React.createElement("span", {
      className: "gex-last"
    }, (() => {
      ...
      className: "gex-col-head"
    }, "Gamma flip"), flipStrike != null ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "gex-flip-strike"
    }, "$", flipStrike.toFixed(2)), /*#__PURE__*/React.createElement("div", {
      className: `gex-flip-side ${aboveFlip ? "above" : "below"}`
    }, "Spot $", S.toFixed(2), " is ", aboveFlip ? "above" : "below", " flip"), /*#__PURE__*/React.createElement("div", {
      className: "gex-flip-hint"
    }, aboveFlip ? "Dealers long gamma here. Expect dampening / mean reversion." : "Dealers short gamma here. Expect amplification / trending moves.")) : /*#__PURE__*/React.createElement("div", {
      className: "gex-flip-side"
    }, "No flip in this chain.")), /*#__PURE__*/React.createElement("div", {
      className: "gex-col"
    }, /*#__PURE__*/React.createElement("div", {
      className: "gex-col-head"
    }, "Largest GEX strikes"), topStrikes.map(g => {
      const pct = Math.abs(g.value) / maxAbsGex * 100;
      return /*#__PURE__*/React.createElement("div", {
        key: `gx${g.strike}`,
        className: "gex-row"
      }, /*#__PURE__*/React.createElement("span", {
        className: "gex-strike"
      }, "$", g.strike.toFixed(2)), /*#__PURE__*/React.createElement("span", {
        className: "gex-bar-wrap"
      }, /*#__PURE__*/React.createElement("span", {
        className: `gex-bar ${g.value >= 0 ? "pos" : "neg"}`,
        style: {
          width: `${pct}%`
        }
      })), /*#__PURE__*/React.createElement("span", {
        className: `gex-val ${g.value >= 0 ? "pos" : "neg"}`
      }, fmtGex(g.value)));
    }))), /*#__PURE__*/React.createElement("div", {
      className: "gex-disclaimer"
    }, "Convention: dealers short calls, long puts (typical). GEX values are estimates."));
  })()), /*#__PURE__*/React.createElement(CardErrorBoundary, {
    label: "Volatility skew"
  }, /*#__PURE__*/React.createElement(VolSkewCard, {
    calls: calls,
    puts: puts,
    currentPrice: currentPrice,
    ticker: ticker,
    ...
    const callWingRow = manualCallWing != null ? calls.find(c => skey(c.strike) === skey(manualCallWing)) || null : null;
    const putWingRow = manualPutWing != null ? puts.find(p => skey(p.strike) === skey(manualPutWing)) || null : null;
    const cd = callPrimaryRow.delta || 0,
      cg = callPrimaryRow.gamma || 0;
    const ct = callPrimaryRow.theta || 0,
      cv = callPrimaryRow.vega || 0;
    const pd = putPrimaryRow.delta || 0,
      pg = putPrimaryRow.gamma || 0;
    const pt = putPrimaryRow.theta || 0,
      pv = putPrimaryRow.vega || 0;
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "ng-credit-lbl"
    }, "Net credit"), /*#__PURE__*/React.createElement("span", {
    ...
    }, "Per contract pair"), /*#__PURE__*/React.createElement("div", {
      ...
      k: "vega"
    }, "V Vega")), /*#__PURE__*/React.createElement("span", {
    ...
    className: "kicker"
  }, "Theta vs gamma timing"), /*#__PURE__*/React.createElement("div", {
    className: "card-title"
  }, "When to sell this week")), /*#__PURE__*/React.createElement("div", {
    className: "seg"
  }, [["call", "Call"], ["put", "Put"]].map(([v, l]) => /*#__PURE__*/React.createElement("button", {
    key: v,
    className: thetaSide === v ? "active" : "",
    onClick: () => setThetaSide(v)
  }, l)))), /*#__PURE__*/React.createElement(ThetaPanel, {
    rows: rows,
    sugCall: sugCall,
    sugPut: sugPut,
    callIv: callAtSug.iv,
    ...
    const nextExpLabel = nextExpDate.toLocaleDateString("en-US", {
      weekday: "short",
      month: "short",
      day: "numeric"
    });
    ...
    const buildCallRolls = (currentStrike, currentIv) => {
      ...
      return [make(currentStrike, "Roll out (same strike)"), make(currentStrike + stepCalls, "Roll out + up 1 step"), make(currentStrike - stepCalls, "Roll out + down 1 step")];
    };
    const buildPutRolls = (currentStrike, currentIv) => {
      ...
      return [make(currentStrike, "Roll out (same strike)"), make(currentStrike - stepCalls, "Roll out + down 1 step"), make(currentStrike + stepCalls, "Roll out + up 1 step")];
    };
    const callRolls = buildCallRolls(sugCall, callAtSug.iv);
    const putRolls = buildPutRolls(sugPut, putAtSug.iv);
    const fmtCredit = v => v >= 0 ? /*#__PURE__*/React.createElement("span", {
      className: "up"
    }, "+$", v.toFixed(2), " credit") : /*#__PURE__*/React.createElement("span", {
      className: "down"
    }, "$", v.toFixed(2), " debit");
    return /*#__PURE__*/React.createElement("div", {
      className: "card",
      ...
      className: "card-sub"
    }, "Estimated via Black-Scholes. Verify against the next expiration's actual chain.")), /*#__PURE__*/React.createElement("div", {
      className: "roll-grid"
    }, /*#__PURE__*/React.createElement("div", {
      className: "roll-side"
    }, /*#__PURE__*/React.createElement("div", {
      className: "roll-side-head"
    }, /*#__PURE__*/React.createElement("span", {
      className: "up"
      ...
      className: "roll-k"
    }, "$", r.strike.toFixed(2)), /*#__PURE__*/React.createElement("span", {
      className: "roll-prem"
    }, "premium $", r.newPrem.toFixed(2)), /*#__PURE__*/React.createElement("span", {
      className: "roll-be"
    }, "B/E $", r.breakEven.toFixed(2))), /*#__PURE__*/React.createElement("div", {
      className: "roll-credit"
    }, fmtCredit(r.credit))))), /*#__PURE__*/React.createElement("div", {
      className: "roll-side"
    }, /*#__PURE__*/React.createElement("div", {
      className: "roll-side-head"
    }, /*#__PURE__*/React.createElement("span", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "card-sub"
    }, "Stays within your max risk per trade based on the strategy's defined loss.")), /*#__PURE__*/React.createElement("div", {
      className: "sizing-grid"
    }, /*#__PURE__*/React.createElement("div", {
      className: "sizing-input"
    }, /*#__PURE__*/React.createElement("label", {
      className: "sizing-label"
    }, "Account size"), /*#__PURE__*/React.createElement("div", {
      className: "sizing-input-wrap"
    }, /*#__PURE__*/React.createElement("span", {
      className: "sizing-prefix"
    }, "$"), /*#__PURE__*/React.createElement("input", {
      type: "number",
      min: "0",
      step: "1000",
      value: sizingConfig.accountSize,
      onChange: e => setSizingConfig(prev => ({
        ...prev,
      ...
      className: "sizing-label"
    }, "Max risk per trade"), /*#__PURE__*/React.createElement("div", {
      className: "sizing-input-wrap"
    }, /*#__PURE__*/React.createElement("input", {
      type: "number",
      min: "0.1",
      ...
      onChange: e => setSizingConfig(prev => ({
        ...prev,
        maxRiskPct: Number(e.target.value) || 0
      }))
    }), /*#__PURE__*/React.createElement("span", {
      className: "sizing-suffix"
    }, "%"))), /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat"
    }, /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat-lbl"
    }, "Max risk dollars"), /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat-val"
    }, "$", maxRiskDollars.toFixed(0))), /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat"
    }, /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat-lbl"
    }, "Risk per contract"), /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat-val"
    }, perContractLoss != null ? `$${perContractLoss.toFixed(0)}` : /*#__PURE__*/React.createElement("span", {
      className: "sizing-undef"
    }, "undefined"))), /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat sizing-result"
    }, /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat-lbl"
    }, "Suggested contracts"), /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat-val sizing-result-val"
    }, contracts != null ? contracts > 0 ? contracts : "0 (risk too low)" : "—")), /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat"
    }, /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat-lbl"
    }, "Actual risk at this size"), /*#__PURE__*/React.createElement("div", {
      className: "sizing-stat-val"
    }, totalRiskAtCount != null && contracts > 0 ? `$${totalRiskAtCount.toFixed(0)} (${(totalRiskAtCount / acct * 100).toFixed(2)}%)` : "—"))), perContractLoss == null && /*#__PURE__*/React.createElement("div", {
      className: "sizing-warn"
    ...
    if (positions.length === 0) {
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "card-sub"
      }, "Build a strategy in the chain, click Open as position, and it shows up here.")), /*#__PURE__*/React.createElement("div", {
        className: "pos-empty"
      }, "No positions yet."));
    }
    ...
    const renderRow = p => {
      ...
      if (livePrice) {
        for (const leg of p.legs) {
          ...
          const itm = leg.type === "call" && livePrice > leg.strike || leg.type === "put" && livePrice < leg.strike;
      ...
      return /*#__PURE__*/React.createElement("div", {
        key: p.id,
        className: `pos-row ${p.status === "closed" ? "pos-closed" : ""}`
      }, /*#__PURE__*/React.createElement("div", {
        className: "pos-row-head"
      }, /*#__PURE__*/React.createElement("span", {
        className: "pos-ticker"
      }, p.ticker), /*#__PURE__*/React.createElement("span", {
        className: "pos-meta"
      }, "Opened ", new Date(p.openedAt).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric"
      }), p.contracts > 1 ? ` · ×${p.contracts}` : ""), dte != null && p.status === "open" && /*#__PURE__*/React.createElement("span", {
        ...
        return /*#__PURE__*/React.createElement("span", {
          key: i,
          className: `pos-leg ${isLong ? "long" : "short"}`
        ...
        className: "pos-stat-lbl"
      }, "Entry credit/debit"), /*#__PURE__*/React.createElement("div", {
        className: `pos-stat-val ${netCredit >= 0 ? "up" : "down"}`
      }, netCredit >= 0 ? "+" : "", "$", netCredit.toFixed(2))), /*#__PURE__*/React.createElement("div", {
        className: "pos-stat"
      }, /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-lbl"
      }, "Entry price"), /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-val"
      }, p.entryPrice != null ? `$${p.entryPrice.toFixed(2)}` : "—")), p.status === "open" ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
        className: "pos-stat"
      }, /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-lbl"
      }, "Current price"), /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-val"
      }, livePrice != null ? `$${livePrice.toFixed(2)}` : /*#__PURE__*/React.createElement("span", {
        className: "pos-note"
      }, "switch ticker"))), /*#__PURE__*/React.createElement("div", {
        className: "pos-stat"
      }, /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-lbl"
      }, "P/L at exp (current)"), /*#__PURE__*/React.createElement("div", {
        className: `pos-stat-val ${livePl == null ? "" : livePl >= 0 ? "up" : "down"}`
      }, livePl != null ? `${livePl >= 0 ? "+" : ""}$${livePl.toFixed(2)}` : "—"))) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
        className: "pos-stat"
      }, /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-lbl"
      }, "Exit price"), /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-val"
      }, p.exitPrice != null ? `$${p.exitPrice.toFixed(2)}` : "—")), /*#__PURE__*/React.createElement("div", {
        className: "pos-stat"
      }, /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-lbl"
      }, "Closed"), /*#__PURE__*/React.createElement("div", {
        className: "pos-stat-val"
      }, p.closedAt ? new Date(p.closedAt).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric"
      }) : "—")))), /*#__PURE__*/React.createElement("div", {
        className: "pos-actions"
      }, p.ticker !== ticker && /*#__PURE__*/React.createElement("button", {
        className: "pos-btn pos-btn-ghost",
        onClick: () => {
          setTicker(p.ticker);
          setTickerInput(p.ticker);
        }
      }, "Switch to ", p.ticker), p.status === "open" && /*#__PURE__*/React.createElement("button", {
        className: "pos-btn pos-btn-close",
        onClick: () => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "card-sub"
    }, "Live P/L shown when the active ticker matches.")), open.length > 0 && /*#__PURE__*/React.createElement("div", {
      className: "pos-list"
    }, open.map(renderRow)), closed.length > 0 && /*#__PURE__*/React.createElement("details", {
      className: "pos-closed-block"
    }, /*#__PURE__*/React.createElement("summary", null, "Closed positions (", closed.length, ")"), /*#__PURE__*/React.createElement("div", {
      className: "pos-list"
    ...
    className: "card-sub"
  }, "All strikes derived from the suggested call and put. Premiums are mid market estimates.")), /*#__PURE__*/React.createElement("div", {
    className: "strat-grid"
  }, strategies.map(s => /*#__PURE__*/React.createElement(StrategyCard, {
    key: s.key,
    rank: s.rank,
    score: s.score,
    reason: s.reason,
    ...
    Term: Term
  })))), activeStrat && /*#__PURE__*/React.createElement("div", {
    className: "card",
    style: {
      marginBottom: "var(--row-gap)"
    }
    ...
    className: "card-sub"
  }, plNetCredit >= 0 ? /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement(Term, {
    k: "net_credit"
  }, "Net credit"), ": ", /*#__PURE__*/React.createElement("b", {
    className: "mono",
    style: {
      color: "var(--fg)"
    }
    ...
    k: "break_even"
  }, plBreakEvens.length > 1 ? "Break evens" : "Break even")), /*#__PURE__*/React.createElement("span", {
    className: "v"
  }, plBreakEvens.map(b => `$${b.toFixed(2)}`).join(" · "))), plBreakEvens.length >= 2 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
    className: "k"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "profit_zone"
  }, "Profit zone")), /*#__PURE__*/React.createElement("span", {
    className: "v"
  }, "$", plBreakEvens[0].toFixed(2), " to $", plBreakEvens[plBreakEvens.length - 1].toFixed(2)))), /*#__PURE__*/React.createElement("div", {
    className: "muted",
    style: {
      fontSize: 12,
      lineHeight: 1.5
    }
  }, activeStrat.note, isBackMonthStrat && " Back month value is a Black Scholes estimate at front month expiry.")), netGreeks && /*#__PURE__*/React.createElement("div", {
    className: "net-greeks"
  }, /*#__PURE__*/React.createElement("div", {
    className: "net-greeks-head"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    k: "delta"
  }, "Net Delta")), /*#__PURE__*/React.createElement("div", {
    className: `ng-val ${netGreeks.delta >= 0 ? "up" : "down"}`
  }, netGreeks.delta >= 0 ? "+" : "", netGreeks.delta.toFixed(2)), /*#__PURE__*/React.createElement("div", {
    className: "ng-sub"
  }, Math.abs(netGreeks.delta) < 0.10 ? "delta neutral" : netGreeks.delta > 0 ? "long bias" : "short bias")), /*#__PURE__*/React.createElement("div", {
    className: "ng-cell"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ng-lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "gamma"
  }, "Net Gamma")), /*#__PURE__*/React.createElement("div", {
    className: `ng-val ${netGreeks.gamma >= 0 ? "up" : "down"}`
  }, netGreeks.gamma >= 0 ? "+" : "", netGreeks.gamma.toFixed(3)), /*#__PURE__*/React.createElement("div", {
    className: "ng-sub"
  }, netGreeks.gamma >= 0 ? "long convexity" : "short convexity")), /*#__PURE__*/React.createElement("div", {
    className: "ng-cell"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ng-lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "theta"
  }, "Net Theta")), /*#__PURE__*/React.createElement("div", {
    className: `ng-val ${netGreeks.theta >= 0 ? "up" : "down"}`
  }, netGreeks.theta >= 0 ? "+" : "", "$", netGreeks.theta.toFixed(2), /*#__PURE__*/React.createElement("span", {
    className: "ng-unit"
  }, "/ day")), /*#__PURE__*/React.createElement("div", {
    className: "ng-sub"
  }, netGreeks.theta > 0 ? "collecting decay" : "paying decay")), /*#__PURE__*/React.createElement("div", {
    className: "ng-cell"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ng-lbl"
  }, /*#__PURE__*/React.createElement(Term, {
    k: "vega"
  }, "Net Vega")), /*#__PURE__*/React.createElement("div", {
    className: `ng-val ${netGreeks.vega >= 0 ? "up" : "down"}`
  }, netGreeks.vega >= 0 ? "+" : "", "$", netGreeks.vega.toFixed(2), /*#__PURE__*/React.createElement("span", {
    className: "ng-unit"
  }, "/ vol pt")), /*#__PURE__*/React.createElement("div", {
    className: "ng-sub"
  }, netGreeks.vega >= 0 ? "long vol" : "short vol")))), (calls.length > 0 || puts.length > 0) && (() => {
    ...
    const atmIdx = allStrikes.reduce((bi, s, i) => Math.abs(s - currentPrice) < Math.abs(allStrikes[bi] - currentPrice) ? i : bi, 0);
    const radius = 12;
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "Strategy builder"), /*#__PURE__*/React.createElement("span", {
      className: "builder-sub"
    }, customLegs.length ? `${customLegs.length} leg${customLegs.length === 1 ? "" : "s"} selected — click +/− buttons in the chain to add legs` : "Click +C/−C/+P/−P next to any strike to start building")), customLegs.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "builder-legs"
    }, customLegs.map((leg, i) => {
      const isLong = leg.qty > 0;
      const totalCost = (isLong ? -1 : 1) * Math.abs(leg.qty / 100) * leg.premium;
      return /*#__PURE__*/React.createElement("div", {
        key: i,
        className: `builder-leg ${isLong ? "long" : "short"}`
      }, /*#__PURE__*/React.createElement("span", {
        className: "builder-leg-tag"
      }, isLong ? "LONG" : "SHORT"), /*#__PURE__*/React.createElement("span", {
        className: "builder-leg-side"
      }, leg.type.toUpperCase()), /*#__PURE__*/React.createElement("span", {
      ...
      }, totalCost >= 0 ? "+" : "", "$", totalCost.toFixed(2)), /*#__PURE__*/React.createElement("button", {
        className: "builder-leg-flip",
        onClick: () => setCustomLegs(prev => prev.map((l, j) => j === i ? {
          ...l,
          qty: -l.qty
        } : l)),
      ...
      const totalCost = customLegs.reduce((sum, l) => sum + (l.qty > 0 ? -1 : 1) * Math.abs(l.qty / 100) * l.premium, 0);
      return /*#__PURE__*/React.createElement("span", {
        className: `builder-total ${totalCost >= 0 ? "credit" : "debit"}`
      }, totalCost >= 0 ? "Net credit: +" : "Net debit: ", "$", Math.abs(totalCost).toFixed(2));
    })(), /*#__PURE__*/React.createElement("div", {
      className: "builder-btn-row"
    }, /*#__PURE__*/React.createElement("button", {
      className: "builder-btn builder-btn-viz",
      onClick: () => setShowPayoff(v => !v)
    }, showPayoff ? "Hide P/L chart" : "Visualize P/L at expiration"), /*#__PURE__*/React.createElement("button", {
      className: "builder-btn builder-btn-track",
      onClick: () => {
    ...
    }, "Open as position"), /*#__PURE__*/React.createElement("button", {
      className: "builder-btn builder-btn-clear",
      onClick: () => {
        setCustomLegs([]);
        setShowPayoff(false);
      }
    }, "Clear all"))), showPayoff && (() => {
      ...
      const expDollarMove = callAtSug.iv && currentPrice && FRONT_DTE ? currentPrice * callAtSug.iv * Math.sqrt(FRONT_DTE / 365) : null;
      return /*#__PURE__*/React.createElement("div", {
        className: "card",
        style: {
          marginTop: 12,
        ...
        className: "card-sub"
      }, netCredit >= 0 ? /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement(Term, {
        k: "net_credit"
      }, "Net credit"), ": ", /*#__PURE__*/React.createElement("b", {
        className: "mono",
        style: {
        ...
        k: "break_even"
      }, bes.length > 1 ? "Break evens" : "Break even")), /*#__PURE__*/React.createElement("span", {
        className: "v"
      }, bes.map(b => `$${b.toFixed(2)}`).join(" · "))), bes.length >= 2 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
        className: "k"
      }, /*#__PURE__*/React.createElement(Term, {
        k: "profit_zone"
      }, "Profit zone")), /*#__PURE__*/React.createElement("span", {
        className: "v"
      }, "$", bes[0].toFixed(2), " to $", bes[bes.length - 1].toFixed(2)))), /*#__PURE__*/React.createElement("div", {
        className: "muted",
        style: {
          fontSize: 12,
          lineHeight: 1.5
      ...
      const inDragRange = drag && strike >= Math.min(drag.start, drag.end) && strike <= Math.max(drag.start, drag.end);
      ...
      const pickCall = e => {
        ...
        e.preventDefault();
        e.stopPropagation();
        if (e.shiftKey) setManualCallWing(prev => prev === strike ? null : strike);else setManualCallStrike(strike);
      };
      const pickPut = e => {
        e.preventDefault();
        e.stopPropagation();
        if (e.shiftKey) setManualPutWing(prev => prev === strike ? null : strike);else setManualPutStrike(strike);
      };
      const callClass = () => `oc-cell oc-call-cell ${itmCall ? "itm" : ""} ${isCallSel ? "oc-sel-call" : ""} ${isCallWing ? "oc-wing-call" : ""} ${isCallDrag ? "oc-drag-call" : ""}`.trim();
      const putClass = () => `oc-cell oc-put-cell ${itmPut ? "itm" : ""} ${isPutSel ? "oc-sel-put" : ""} ${isPutWing ? "oc-wing-put" : ""} ${isPutDrag ? "oc-drag-put" : ""}`.trim();
      ...
      return /*#__PURE__*/React.createElement("tr", {
        key: strike,
        className: `${isAtm ? "oc-atm" : ""} ${hasLegHere ? "oc-has-leg" : ""}`
      }, /*#__PURE__*/React.createElement("td", {
        className: "oc-build-cell oc-build-call"
      }, /*#__PURE__*/React.createElement("button", {
        className: "oc-build-btn oc-build-long",
        onClick: e => {
          e.stopPropagation();
          addLeg("call", 1);
        },
        title: "Add long call to builder"
      }, "+C"), /*#__PURE__*/React.createElement("button", {
        className: "oc-build-btn oc-build-short",
        onClick: e => {
          e.stopPropagation();
          addLeg("call", -1);
        },
      ...
      className: "oc-leg-swatch oc-leg-itm"
    }), " ITM shaded"), /*#__PURE__*/React.createElement("span", {
      className: "oc-leg-item"
    }, /*#__PURE__*/React.createElement("span", {
      className: "oc-leg-swatch oc-leg-atm"
    }), " ATM row highlighted"), /*#__PURE__*/React.createElement("span", {
      className: "oc-leg-item"
    ...
    onChange: v => tweaks.setValue("accent", v),
    ...
    label: "Pairing",
    value: tweaks.values.typeface,
    onChange: v => tweaks.setValue("typeface", v),
    options: [{
      value: "sans",
      label: "Sans"
    }, {
    ...
    label: "Style",
    value: tweaks.values.chartStyle,
    onChange: v => tweaks.setValue("chartStyle", v),
    options: [{
      value: "candles",
      label: "Candles"
    }, {
    ...
    onChange: v => tweaks.setValue("layout", v),
    options: [{
      value: "default",
      label: "Default"
    }, {
