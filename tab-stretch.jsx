...
const stBasedOn = (r) => (r.n == null ? "—"
  : `${r.n} similar ${r.horizon === "week" ? "weeks" : "days"} · ${r.grade === "MEASURED" ? "its own" : "borrowed"}`);

// label, key, tooltip, formatter, numeric
const ST_COLS = [
  ["Symbol", "symbol", "card", null, false],
  ["What it did", "move_pct", "move", stMoveWords, false],
  ["Its usual", "line_pct", "line", stUsualWords, false],
  ["Sell", "strike", "strike", stSellWords, false],
  ["Delta", "delta", "delta", (r) => (r.delta == null ? "—" : Math.abs(r.delta).toFixed(2)), true],
  ["Credit", "credit", "credit", (r) => stMoney(r.credit), true],
  ["Ends past strike", "itm_pct", "itm", (r) => stPct0(r.itm_pct), true],
  ["Touches strike", "touch_pct", "touch", (r) => stPct0(r.touch_pct), true],
  ["Based on", "grade", "grade", stBasedOn, false],
  ["Ready since", "first_seen", "since", (r) => stTime(r.first_seen), false],
];
    ...
    <div className="card sk-card st-card">
      ...
      <PanelMethod label="Method — what the line is, what the evidence is, and what is refused">
        ...
        <p title={ST_TIP.day_upper}>
          ...
          name with fewer than three such sellable strikes is skipped for five days. Names with thin records
                    ...
                    {ST_COLS.map(([label, ck, tipKey, f, numeric]) => (
                      <td key={ck} data-label={label} title={ST_TIP[tipKey]}
                        ...
                        {ck === "symbol" ? (
                          ...
                          <span>{f(r)}{r.is_new ? <span className="st-new-chip"> NEW</span> : null}</span>
                        ) : f(r)}
                      </td>
                    ))}
                  </tr>
      ...
      ) : (data && !data.no_trade && ready.length === 0 ? (
        <p className="sl-muted">Nothing matches the filters; {data.n_ready} ready on the other side or horizon.</p>
      ) : null)}

      {crossed.length ? (
        <div className="st-crossed" title={ST_TIP.crossed_list}>
          <div className="sl-block-title">Crossed a line, but nothing to sell ({crossed.length})</div>
          <ul>
            {crossed.map((r) => {
              const k = stRowKey(r);
              return (
                <li key={k} className={open === k ? "st-crossed-open" : ""} onClick={() => toggle(r)}
                    title="Click for the numbers behind this">
                  <button className="su-blink" onClick={(e) => { e.stopPropagation(); onPickTicker && onPickTicker(r.symbol); }}
                          title={`Load ${r.symbol}`}>{r.symbol}</button>
                  {" — "}{stMoveWords(r).toLowerCase()} ({stUsualWords(r)}) — no {r.side} worth selling
                  {r.why && r.why.length ? `: ${r.why[0]}` : ""}
                  {r.crossed_since ? <span className="sl-muted"> · since {stTime(r.crossed_since)}</span> : null}
                </li>
              );
            })}
          </ul>
