...
const PAGE_NOT_DATA_MSG =
  "The app answered with a page instead of data — usually the sign-in "
  + "screen after the session expires. Reload the page to sign back in.";
  ...
  try {
    ...
    return (
      <span className="lc-wrap">
        ...
        <span className={`lc-time${mktOpen ? " mkt-open" : ""}`}
              title={mktOpen ? "Market is OPEN (regular session, 9:30–4:00 ET)" : "Market is closed — outside regular trading hours"}>{timeFmt}</span>
      </span>
    );
  } catch {
...
const TRADE_SECTION_GROUPS = [
  ["Opportunities", ["at the line", "sold into strength", "best sales today", "best setup",
...
function App() {
  ...
  useEffect(() => {
    try { localStorage.setItem("weeklyOptionsTimer.marketDash.open.v2", marketDashOpen ? "1" : "0"); } catch {}
  }, [marketDashOpen]);
  ...
  const _defaultPresets = useMemo(
    () => Object.keys((window.MockData && window.MockData.PRESETS) || {}), []);
  ...
  const runAllScanners = async () => {
    ...
    try { await apiFetch("/api/scan_all", { noCache: true }); } catch (e) {}
    const steps = [
      ["EMA pullback", runEmaScan],
      ["Open-to-low pullback", runPullbackScan],
      ...(uwHealth?.connected ? [["Momentum × UW flow", runMomentumScan],
                                 ["Premium richness", runRichnessScan]] : []),
      ["Best setup per symbol", runScan],
      ["Implied move + strikes", runWeeklyRange],
      ...(uwHealth?.connected ? [["Market scanner", runMarketScan]] : []),
    ];
    try {
      for (const [name, fn] of steps) {
        setRunAllStep(name);
        try { await fn(); } catch (e) { console.warn(`run-all ${name} failed:`, e); }
      }
    } finally { setRunAllStep(null); }
  };
  ...
  useEffect(() => {
    ...
    if (_hit && Date.now() - _hit.t < _TICKER_LRU_TTL) {
      if (window.__installLive) window.__installLive(_hit.payload);
      else window.__bootstrapLive && window.__bootstrapLive(_hit.payload);
      setDataVersion(v => v + 1);
    ...
    apiFetch(url, { signal: ac.signal })
      .then(r => {
        ...
        if (r.ok) return r.json().catch(
          () => Promise.reject({ error: PAGE_NOT_DATA_MSG, status: r.status }));
        ...
        return r.json()
          .catch(() => ({ error: r.statusText || `HTTP ${r.status}` }))
          .then(e => Promise.reject(Object.assign({}, e, { status: r.status })));
      })
      .then(payload => {
        if (cancelled) return;
        _TICKER_LRU.delete(_lruKey);                 // re-insert = most recent
        _TICKER_LRU.set(_lruKey, { t: Date.now(), payload });
        if (_TICKER_LRU.size > _TICKER_LRU_MAX)
          _TICKER_LRU.delete(_TICKER_LRU.keys().next().value);
      ...
      .catch(err => {
        ...
        if (_hit && Date.now() - _hit.t < _TICKER_LRU_TTL) { setLoading(false); return; }
        ...
        if (offline && havePreset) {
          ...
          setLoadError(
            offline
              ...
              : throttled ? THROTTLE_GAVE_UP : msg
          );
  ...
  useEffect(() => {
    ...
    const load = () => sharedJson(apiFetch, `/api/intraday?symbol=${encodeURIComponent(ticker)}`, 25 * 1000)
      .then(d => { if (!stop) setIntradayData(d && !d.error ? d : { error: (d && d.error) || "no data" }); })
      .catch(e => { if (!stop) setIntradayData({ error: String((e && e.message) || e) }); });
    load();
  ...
  return (
    <div className="shell">
      ...
      <aside className={`sidebar${navOpen ? " nav-open" : ""}`}>
        ...
        <div className="sb-section sb-brand">
          ...
          <div className="sb-brand-text">
            ...
            {dataSource && (() => {
              ...
              const why =
                (sw.needs_reauth || sw.auth_error)
                  ? `Schwab rejected the last request${sw.auth_error ? ` (${sw.auth_error})` : ""}`
                    + " — re-authorize under Manage. Option chains will not load until you do."
                  : (typeof refr === "number" && refr <= 0)
                  ? "The Schwab sign-in has expired — they last 7 days. Re-authorize under"
                    + " Manage. Option chains will not load until you do."
                  : (typeof refr === "number" && refr < 1)
                  ? `The Schwab sign-in expires in about ${Math.round(refr * 24)} hours.`
                    + " Re-authorize under Manage before it does."
                  : "Schwab is connected but the last call fell back to yfinance —"
                    + " usually a timeout or a rate limit. Quotes are delayed;"
                    + " option chains may not load.";
              const urgent = !!(sw.needs_reauth || sw.auth_error
                                || (typeof refr === "number" && refr <= 0));
              return (
                <div className={`src-badge src-badge-yf${urgent ? " src-badge-warn" : ""}`}
                     title={why}>
        ...
        <div className="sb-section">
          <div className="sb-row h">
            <span className="sb-label">Presets</span>
            <button className="sb-preset-edit" onClick={() => setPresetsEditing(e => !e)}
                    title={presetsEditing ? "Done editing presets" : "Edit your preset tickers"}>
              {presetsEditing ? "Done" : "Edit"}
            </button>
          </div>
          <div className="sb-preset-row">
            {presets.map(t => (
              <span key={t} className={`preset-pill ${ticker === t ? "active" : ""}${presetsEditing ? " editing" : ""}`}>
                <button className="preset-pill-go"
                  onClick={() => { setTicker(t); setTickerInput(t); }}
                  onContextMenu={(e) => { e.preventDefault(); removePreset(t); }}
                  title={presetsEditing ? "Click × to remove" : "Click to switch · right-click to remove"}
                >{t}</button>
                {presetsEditing && (
                  <button className="preset-pill-x" onClick={() => removePreset(t)}
                          title={`Remove ${t} from presets`}>×</button>
                )}
              </span>
            ))}
            {presets.length === 0 && (
              <div className="sb-watchlist-empty">No presets. Add one below.</div>
            )}
          </div>
          {presetsEditing && (
            <div className="sb-preset-add">
              <input type="text" className="sb-preset-input"
                     placeholder="Add ticker (e.g. AMD)"
                     value={presetInput}
                     onChange={e => setPresetInput(e.target.value)}
                     onKeyDown={e => { if (e.key === "Enter") { addPreset(presetInput); setPresetInput(""); } }} />
              <button className="sb-preset-addbtn"
                      onClick={() => { addPreset(presetInput); setPresetInput(""); }}>Add</button>
            </div>
          )}
        </div>
      ...
      <main className="main">
        ...
        <TabPanel tab="watchlist" active={activeTab}>
          <CardErrorBoundary label="Watchlist analyst actions">
            <WatchlistAnalystCard apiFetch={apiFetch} onSwitchTicker={switchTicker} />
          </CardErrorBoundary>
          <CardErrorBoundary label="Watchlist table">
            <WatchlistTableCard apiFetch={apiFetch} onSwitchTicker={switchTicker} market={marketDashboard} onRemoveSymbol={removeFromWatchlist} watchlistSymbols={watchlist} />
          </CardErrorBoundary>
        </TabPanel>
        <TabPanel tab="streaks" active={activeTab}>
          <CardErrorBoundary label="Streaks">
            <WatchlistStreaksCard apiFetch={apiFetch} onSwitchTicker={switchTicker} />
          </CardErrorBoundary>
        </TabPanel>
        ...
        {/* Earnings Opportunities — MC-style earnings opportunity scanner
        ...
        <TabPanel tab="recovery" active={activeTab}>
          <CardErrorBoundary label="Prior High Recovery">
            <LazyTab chunk="tab-recovery" component="RecoveryTab" label="Prior High Recovery"
                     apiFetch={apiFetch}
                     onOpenTicker={(sym) => { switchTicker(sym); changeTab("trade"); }}
                     onMarkLevels={setRcvLevels} />
          </CardErrorBoundary>
        </TabPanel>
        <TabPanel tab="invest" active={activeTab}>
          <CardErrorBoundary label="Investment">
        ...
        <TabPanel tab="edge" active={activeTab}>
          <CardErrorBoundary label="Premium Edge">
            <LazyTab chunk="tab-edge" component="EdgeTab" label="Premium Edge"
                     apiFetch={apiFetch}
                     onOpenTicker={(sym) => { switchTicker(sym); changeTab("trade"); }} />
          </CardErrorBoundary>
        </TabPanel>
        <TabPanel tab="live" active={activeTab}>
          <CardErrorBoundary label="Live Scanner">
        ...
        <TabPanel tab="journal" active={activeTab}>
          <CardErrorBoundary label="Journal">
            <PicksJournalCard apiFetch={apiFetch} onSwitchTicker={switchTicker} />
            <CardErrorBoundary label="Radar performance">
              <RadarReportCard apiFetch={apiFetch} />
            </CardErrorBoundary>
          </CardErrorBoundary>
        </TabPanel>
        {showRef && (
        ...
        {loadError && !loading && (
          <div className="error-banner" title={retryIn != null
            ? "Your broker limits how many requests it will answer in a short "
              + "window. Switching between symbols quickly can cross that line. "
              + "It is a temporary refusal, not a problem with the symbol or "
              + "with your account, so the app is waiting it out and will load "
              + "the symbol itself. You can press Retry to try sooner."
            : "The symbol could not be loaded. The reason from the data source "
              + "is shown here. Press Retry to request it again."}>
            <span className="ico">⚠</span>
            <div>
              <b>Couldn't load {ticker}.</b>{" "}
              <span className="muted">
                {loadError}
                {retryIn != null && ` in ${retryIn} second${retryIn === 1 ? "" : "s"}…`}
              </span>
            </div>
        ...
        {/* At the line (v5.16) — the watchman. Names that have reached their
            ...
            this is where the reasoning lives, so it sits at the top. */}
        ...
        {/* Sold into strength (v4.82) — the time-critical one: a spike's
            premium decays by the minute, and this board is only worth
            anything while the move is live. */}
        <TabPanel tab="trade" active={activeTab} pending={dataPending} pendingLabel={ticker}>
          ...
          <div className="card chart-card">
            ...
            <div className="card-head">
              ...
              <div className="toolbar">
                <div className="seg" title="Chart timeframe: Daily candles, or today's 1-minute bars with session VWAP ± σ bands, premarket prints, the day-level map (prev day H/L/C, premarket, opening range, EM band) and Reversal-Radar signal markers.">
                  {[["daily","Daily"],["intraday","1-Min"]].map(([v, l]) => (
                    <button key={v} className={chartTF === v ? "active" : ""}
                            onClick={() => setChartTF(v)}
                            title={v === "daily" ? "Daily candles with moving averages and the expected weekly range" : "Today's 1-minute bars with VWAP bands and intraday levels — the reversal-trading view"}>{l}</button>
                  ))}
                </div>
            ...
            {chartTF === "intraday" ? (
              <CardErrorBoundary label="Intraday chart">
                {intradayData && intradayData.error ? (
                  <div className="ic-empty" title="Intraday bars come from Schwab minute data — they appear once today's session has printed.">{intradayData.error}</div>
                ) : intradayData ? (
                  <>
                    <IntradayChart data={intradayData} />
                    {intradayData.vwap && (
                      <div className="lvl-strip">
                        <span className="lvl-item" title="Session VWAP — the volume-weighted average price institutions benchmark against. T1 for radar mean-reversion trades.">
                          VWAP <b>{fmt$(intradayData.vwap.last)}</b>
                        </span>
                        <span className={`lvl-item ${(intradayData.vwap.stretch || 0) <= -2 ? "down" : (intradayData.vwap.stretch || 0) >= 2 ? "up" : ""}`}
                          ...
                          stretch <b>{intradayData.vwap.stretch > 0 ? "+" : ""}{intradayData.vwap.stretch}σ</b>
                        </span>
                        {(() => {
                          ...
                          return (
                            <>
                              {below.map((l, i) => (
                                <span key={`b${i}`} className="lvl-item down" title={`Nearest support below: ${l.label}. Reversal longs are strongest when the low formed at a level like this.`}>
                                  ↓ {l.label} <b>{fmt$(l.price)}</b>
                                </span>
                              ))}
                              {above.map((l, i) => (
                                <span key={`a${i}`} className="lvl-item up" title={`Nearest resistance above: ${l.label}. Bounces often stall here — a natural target or short location.`}>
                                  ↑ {l.label} <b>{fmt$(l.price)}</b>
                                </span>
                              ))}
                            </>
                  ...
                  <div className="ic-empty">Loading today's bars…</div>
                )}
              </CardErrorBoundary>
            ) : (() => {
            ...
            {chartTF === "intraday" && (
              <div className="legend" style={{marginTop: 12}}>
                <span className="item"><span className="swatch" style={{background: "#38bdf8", height: 3}}></span>VWAP ±1σ/±2σ</span>
                <span className="item"><span className="swatch dashed" style={{borderColor: "rgba(234,179,8,0.8)"}}></span>Prev day H/L/C</span>
                <span className="item"><span className="swatch dashed" style={{borderColor: "rgba(168,85,247,0.8)"}}></span>Premarket H/L</span>
                <span className="item"><span className="swatch dashed" style={{borderColor: "rgba(255,255,255,0.55)"}}></span>Opening range</span>
                <span className="item" title="Arrows mark where the Reversal Radar logged a signal (score ≥ 70) today, labeled with the score.">▲▼ Radar signals</span>
              </div>
            )}
            {chartTF === "daily" && <div className="legend" style={{marginTop: 12}}>
              ...
              {rcvLevels && rcvLevels.symbol === ticker && (
                <span className="item" role="button" style={{cursor: "pointer"}}
                  ...
                  <span className="swatch dashed" style={{borderColor: "#22c55e"}}></span>Recovery levels ✕
                </span>
              )}
        ...
        {/* After it reaches the line (v5.16) — the sentence the dashed
            median lines cannot say: how often the stock got there and what
            it did next, on its own record, with the count. */}
        <CardErrorBoundary label="After it reaches the line">
          <LazyTab chunk="tab-stretch" component="StretchLineCard" label="After it reaches the line"
                   apiFetch={apiFetch} ticker={ticker} />
        </CardErrorBoundary>
        ...
        {/* Run-everything strip (v3.98): one tap replaces clicking each
            scanner's own button. Server boards run sequentially with
            skip-if-fresh; the per-symbol scanners chain one at a time. */}
        <div className="card scanall-card">
          <div className="card-head">
            <div>
              <div className="kicker" title="Board scans (weekly range, trend, HV rank, movers, analyst) run on the server one at a time and are SKIPPED when their last scan is still fresh. The per-symbol scanners below then run one after another — never more than one request in flight, so rate limits are never burst. The single-ticker tools (earnings ladder, walk-forward) stay manual.">
                every scanner below · sequential · skips fresh boards · rate-limit safe
              </div>
              <div className="card-title">Scan everything</div>
            </div>
            <div className="tsy-ctrl rcv-ctrl">
              <button className="scan-run-btn" onClick={runAllScanners} disabled={!!runAllStep}
                      title="Run every scanner on this tab (and the Discover boards) in one sequential pass.">
                {runAllStep ? `Running: ${runAllStep}…` : "Run all scanners"}
              </button>
            </div>
          </div>
          {runAllStep && (
            <div className="ab-status">
              Working through the scanners one at a time — each card below fills in as its turn completes. Leave the tab open; nothing runs in parallel, so this is deliberately unhurried.
            </div>
          )}
        </div>
