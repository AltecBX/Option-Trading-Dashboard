...
function MarketOverview({
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, regime && /*#__PURE__*/React.createElement("div", {
    ...
    const srcNote = it.source === "schwab" ? "real-time (Schwab)" : it.source === "yahoo" ? "live (Yahoo)" : it.source === "yfinance" ? "delayed (yfinance)" : "";
...
function MarketPosture({
  ...
  const view = useMemo(() => {
    ...
    if (ivRows.length) {
      ivRows.forEach(r => {
        if (r.ticker) ivBySym[String(r.ticker).toUpperCase()] = r.rank;
      });
    ...
    const buildTicket = p => {
      ...
      if (!(medDays > 0) && p.swing_pct > 0 && p.swing_days > 0 && p.swing_med_pct > 0) medDays = p.swing_days * (p.swing_med_pct / p.swing_pct);
      ...
      if (buy) {
        ...
        const base = bull ? Math.max(p.swing_from || px * 0.9, px * 0.90) : Math.min(p.swing_from || px * 1.1, px * 1.10);
      ...
      return {
        ...
        why: `${p.symbol}: ${bull ? "long" : "short"} setup, ${Math.round(p.swing_pct || 0)}% into a typical ` + `${Math.round(p.swing_med_pct || 0)}% / ${Math.round(medDays)}d move (~${Math.round(remainDays)}d left). ` + (buy ? `IV ${ivr != null ? "rank " + ivr + " " : ""}still cheap — buy the ${bull ? "call" : "put"} ` + `near the money and let vega + delta expand as it accelerates. ${Math.round(dte)}d out (~2.5× the days left) so theta isn't the enemy.` : `IV rank ${ivr} is rich — sell the ${bull ? "put" : "call"} at ${fmtK(strike)} to harvest it, ${Math.round(dte)}d out. `) + ` Target ~$${fmtK(tgt)}.`
      };
    };
    ...
    const picks = earlyPool.map(r => ({
      ...r,
      _es: earlyScore(r)
    })).sort((a, b) => b._es - a._es).slice(0, 3).map(r => ({
      ...r,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await Promise.all(view.picks.map(async p => {
        ...
        try {
          const d = await sharedJson(apiFetch, `/api/pick_ticket?symbol=${encodeURIComponent(p.symbol)}&right=${tk.right}&buy=${tk.buy ? 1 : 0}&strike=${tk.strike}&dte=${tk.dte}`, 120000);
  ...
  const journalPick = async p => {
    ...
    const snap = {
      ...
      edge: p.edge,
      stage: p.swing_stage,
      ticket: ct ? `${tk.buy ? "Buy" : "Sell"} $${ct.strike}${ct.right} ${ct.expiration}${ct.mid != null ? ` ~$${ct.mid}` : ""}` : tk.text || sellSide(p),
      action: tk.buy === undefined ? null : tk.buy ? "buy" : "sell",
      right: ct && ct.right || tk.right,
      strike: (ct && ct.strike) != null ? ct.strike : tk.strike,
      dte: (ct && ct.dte) != null ? ct.dte : tk.dte,
      target: tk.tgt,
      expiration: ct ? ct.expiration : null,
      entry_mid: ct ? ct.mid : null,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    const tktText = ct ? `${tk && tk.buy ? "Buy" : "Sell"} $${fmtStrike(ct.strike)}${ct.right} ${fmtExp(ct.expiration)}${ct.mid != null ? ` ~$${ct.mid.toFixed(2)}` : ""}` : tk ? tk.text : sellSide(p);
    const tktWhy = (tk ? tk.why : "") + (ct ? ` — LISTED CONTRACT (validated against the live chain): $${fmtStrike(ct.strike)}${ct.right} exp ${ct.expiration} (${ct.dte}d)` + (ct.bid != null && ct.ask != null ? `, bid $${ct.bid} / ask $${ct.ask}` : "") + (ct.oi ? `, open interest ${ct.oi}` : "") + ". This is the exact order to place." : "");
    return /*#__PURE__*/React.createElement("div", {
      key: i,
      className: "pc-pick",
      role: "button",
    ...
    }, long ? "▲" : "▼", " ", p.swing_pct != null ? Math.round(p.swing_pct) : "?", "% in")), /*#__PURE__*/React.createElement("div", {
      className: "pc-pick-l2"
    }, /*#__PURE__*/React.createElement("span", {
      className: `pc-tkt ${tk && tk.buy ? "buy" : "sell"}`
    }, tktText), tk && /*#__PURE__*/React.createElement("span", {
    ...
    load().then(d => {
      if (d && d.status && d.status.scanning && !pollRef.current) {
        pollRef.current = setInterval(async () => {
          const dd = await load();
          if (!dd || !dd.status || !dd.status.scanning) {
...
function flowFreshness(ts) {
  ...
  if (min < 60) return {
    tone: "ok",
    label: `${Math.round(min)}m old`
  };
  const hr = min / 60;
  if (hr < 24) return {
    tone: hr >= 6 ? "stale" : "ok",
    label: `${Math.round(hr)}h old`
  };
  return {
    tone: "stale",
    label: `${Math.round(hr / 24)}d old`
  };
}
...
function edgesFor(rows) {
  ...
  if (!v) {
    v = computeWatchlistEdges(rows);
    _EDGE_MEMO.set(rows, v);
  }
  return v;
}
...
function makeSwingDatafeed(apiFetch) {
  return {
    ...
    getBars: (symbolInfo, _res, periodParams, onResult, onError) => {
      apiFetch(`/api/swings?symbol=${encodeURIComponent(symbolInfo.name)}&period=2y`).then(r => r.json()).then(d => {
...
function SwingChart({
  ...
  useEffect(() => {
    ...
    if (show.zones && rv && rv.ok && rv.zone) {
      ...
      if (rv.current && rv.current.extreme_price != null) mk2(rv.current.extreme_price, "rgba(148,163,184,0.75)", LC.LineStyle.Dashed);
  ...
  if (show.zones && rv && rv.ok && rv.zone) {
    ...
    if (rc.extreme_price != null) {
      ...
      legend.push({
        ...
        price: rc.extreme_price,
        color: "rgba(148,163,184,0.95)"
      });
    }
    if (rv.typical_zone) legend.push({
      name: `usual ${revName}s`,
      price: rv.typical_zone.band_low_price,
      price2: rv.typical_zone.band_high_price,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "ab-status muted"
  }, "Chart library didn't load (offline?). The swing table above has the full data."), !collapsed && LC && legend.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "swing-chart-legend"
  }, legend.map(l => /*#__PURE__*/React.createElement("span", {
    key: l.name,
    className: "swing-legend-item",
    title: l.tip || ""
  }, l.text ? /*#__PURE__*/React.createElement("b", null, l.text) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("i", {
    style: {
      background: l.color
    }
  }), l.name, " ", /*#__PURE__*/React.createElement("b", null, l.price2 != null ? `${fmtUsd(l.price, 2)}–${fmtUsd(l.price2, 2)}` : fmtUsd(l.price, 2)))))), !collapsed && LC && /*#__PURE__*/React.createElement("div", {
    ...
    className: "muted"
  }, "Vol ", fmtVol(ro.v)))), /*#__PURE__*/React.createElement("div", {
...
function SwingWhatIf({
  ...
  const setupKey = [ticker, sens, reversal && reversal.direction, cur.from_date, cur.extreme_abs_pct].join("|");
  useEffect(() => {
    setTgt(nxt && nxt.pct_median != null ? String(Math.round(nxt.pct_median)) : "");
    setStp(rem && rem.more_move_p75_pct != null ? String(Math.max(1, Math.round(rem.more_move_p75_pct))) : "");
    /* eslint-disable-next-line */
  }, [setupKey]);
  const run = async () => {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "muted"
  }, " \xB7 n=", wi.n, wi.median_days_to_target != null ? ` · ~${wi.median_days_to_target}d to target` : ""), wi.gapped_entries > 0 && /*#__PURE__*/React.createElement("small", {
    className: "muted",
    title: REV_TIP.gap
  }, " \xB7 ", wi.gapped_entries, " filled at the open after a gap"), wi.excluded_contaminated > 0 && /*#__PURE__*/React.createElement("small", {
    className: "muted",
    title: "Episodes whose follow-on leg contains a split or an incredible single-day print are dropped rather than raced through prices the engine does not trust."
  }, " ", "\xB7 ", wi.excluded_contaminated, " dropped")), wi && !wi.ok && wi.reason && /*#__PURE__*/React.createElement("span", {
...
const REV_STATUS_TONE = {
  "BOUNCING OFF ZONE": "up",
  "FADING OFF ZONE": "down",
  "IN ZONE": "warn",
  "APPROACHING": "muted",
  "BEYOND TYPICAL ZONE": "warn",
  "BEYOND HISTORY": "warn"
};
function SwingReversalBlock({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "rev-block",
    title: REV_TIP.block
  }, st.code && /*#__PURE__*/React.createElement("div", {
    className: `rev-status tone-${REV_STATUS_TONE[st.code] || "muted"}`,
    title: REV_TIP.status + (st.meaning ? " — " + st.meaning : "")
  }, /*#__PURE__*/React.createElement("span", {
    className: "rev-status-code"
  }, st.code), /*#__PURE__*/React.createElement("span", {
    className: "rev-status-why"
  }, st.meaning)), /*#__PURE__*/React.createElement("div", {
    ...
    label: down ? "Down from the swing high" : "Up from the swing low",
    tone: down ? "down" : "up",
    ...
    className: "muted"
  }, "\xB7 ", c.days, " day", c.days === 1 ? "" : "s", " \xB7 ", usd(c.price))), /*#__PURE__*/React.createElement(RevStat, {
    label: down ? "Deepest so far (running low)" : "Highest so far (running high)",
    tip: REV_TIP.extreme,
    tone: down ? "down" : "up"
  }, pctf(c.extreme_abs_pct), /*#__PURE__*/React.createElement("small", {
    className: "muted"
  }, " \xB7 ", usd(c.extreme_price), " on ", fmtLongDate(c.extreme_date))), /*#__PURE__*/React.createElement(RevStat, {
    label: down ? "Back off that low" : "Back off that high",
    tip: REV_TIP.extreme,
    tone: c.off_extreme_pct ? down ? "up" : "down" : ""
  }, c.off_extreme_pct == null ? "—" : c.off_extreme_pct === 0 ? "at the extreme" : `${down ? "+" : "−"}${c.off_extreme_pct}%`, c.off_extreme_dollars ? /*#__PURE__*/React.createElement("small", {
    className: "muted"
  }, " \xB7 ", usd(c.off_extreme_dollars)) : null), mat.code && /*#__PURE__*/React.createElement(RevStat, {
    label: "Stage of the move",
    tip: REV_TIP.maturity + (mat.note ? " — " + mat.note : ""),
    tone: mat.code === "EARLY IN THE MOVE" ? "muted" : ""
  }, mat.code === "EARLY IN THE MOVE" ? "Early" : mat.code === "AT ITS NORMAL SIZE" ? "Normal size" : mat.code === "BEYOND ITS NORMAL SIZE" ? "Beyond normal" : "—", /*#__PURE__*/React.createElement("small", {
    className: "muted"
  }, " \xB7 ", mat.ratio_pct == null ? "—" : `${mat.ratio_pct}% of a typical ${dirWord}`)), /*#__PURE__*/React.createElement(RevStat, {
    ...
    tip: REV_TIP.cohort + (co.floor_note ? " — " + co.floor_note : ""),
    ...
    className: "rev-box-title"
  }, "Historical ", revWord, co.insufficient ? " · thin sample" : ""), tz && /*#__PURE__*/React.createElement(RevStat, {
    label: `Where its ${dirWord}s usually end`,
    tip: REV_TIP.typical
  }, usd(tz.band_low_price), " \u2013 ", usd(tz.band_high_price), /*#__PURE__*/React.createElement("small", {
    className: "muted"
  }, " \xB7 ", pctf(tz.p25_abs_pct), "\u2013", pctf(tz.p75_abs_pct), " \xB7 all ", tz.n, " of them")), !z ? /*#__PURE__*/React.createElement("div", {
    className: "rev-empty"
    ...
    label: `Given it got this far, typical ${dirWord}`,
    tip: REV_TIP.zone
  ...
  }, l.within_more_pct, "%: ", /*#__PURE__*/React.createElement("b", null, l.rate_pct, "%"), /*#__PURE__*/React.createElement("small", null, " (", l.wilson_lo_pct, ")")))), co.earnings_note && /*#__PURE__*/React.createElement("div", {
    className: "rev-note",
    title: REV_TIP.earnA
  }, co.earnings_note))), /*#__PURE__*/React.createElement("div", {
    className: "rev-box",
    title: REV_TIP.next
  }, /*#__PURE__*/React.createElement("div", {
    className: "rev-box-title"
    ...
    className: "muted"
  }, " \xB7 zone ", usd(Math.min(nx.target_p25_price, nx.target_p75_price)), "\u2013", usd(Math.max(nx.target_p25_price, nx.target_p75_price)), nx.target_is_paired ? ` · ${(nx.paired || {}).n} paired episodes` : "")), /*#__PURE__*/React.createElement(RevStat, {
    label: "Typical duration",
    tip: REV_TIP.next
  }, nx.days_median == null ? "—" : `${nx.days_median} days`, /*#__PURE__*/React.createElement("small", {
    className: "muted"
  }, " \xB7 ", nx.days_p25, "\u2013", nx.days_p75)), (nx.horizon_touch || []).length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "rev-ladder rev-ladder-wide",
    title: REV_TIP.horizon + " " + (nx.horizon_note || "")
  }, /*#__PURE__*/React.createElement("span", null, "After the turn, ", down ? "rose" : "fell", "\u2026"), nx.horizon_touch.map(t => /*#__PURE__*/React.createElement("em", {
    key: `${t.pct}-${t.days}`
  }, down ? "+" : "−", t.pct, "% in ", t.days, "d: ", /*#__PURE__*/React.createElement("b", null, t.rate_pct, "%"), /*#__PURE__*/React.createElement("small", null, " (", t.wilson_lo_pct, ")", t.baseline_pct != null ? ` vs ${t.baseline_pct}% any day · ${t.edge_pp >= 0 ? "+" : ""}${t.edge_pp}pp` : "")))), nx.reclaim_20d && nx.reclaim_20d.rate_pct != null && /*#__PURE__*/React.createElement(RevStat, {
...
function SwingReversalScan({
  ...
  const lastScan = status && status.last_scan ? new Date(status.last_scan).toLocaleString(undefined, {
    month: "short",
    day: "numeric",
    hour: "numeric",
    minute: "2-digit"
  }) : null;
  const inMode = r => r.rz_median_price != null && (mode === "bounce" ? r.swing_dir === "short" : r.swing_dir === "long");
  const pick = (rows || []).filter(r => inMode(r) && (stageF === "all" || r.rz_stage !== "EARLY IN THE MOVE"));
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Pullback candidates (", (rows || []).filter(r => r.rz_median_price != null && r.swing_dir === "long").length, ")"), /*#__PURE__*/React.createElement("span", {
    className: "rev-scan-sep"
  }), /*#__PURE__*/React.createElement("button", {
    className: `rr-btn ${stageF === "all" ? "pd-f-on" : ""}`,
    onClick: () => setStageF("all"),
    title: REV_TIP.scanStage
  }, "All stages"), /*#__PURE__*/React.createElement("button", {
    className: `rr-btn ${stageF === "developed" ? "pd-f-on" : ""}`,
    onClick: () => setStageF("developed"),
    title: REV_TIP.scanStage
  }, "Normal size or beyond (", (rows || []).filter(r => inMode(r) && r.rz_stage && r.rz_stage !== "EARLY IN THE MOVE").length, ")"), /*#__PURE__*/React.createElement("button", {
    className: "scan-run-btn rev-scan-run",
    onClick: startScan,
    disabled: scanning,
    title: REV_TIP.scanRun
  }, scanning ? status && status.total ? `Scanning… ${status.scanned || 0}/${status.total}` : "Scanning…" : "Scan now")), /*#__PURE__*/React.createElement("div", {
    className: "rev-scan-meta",
    title: REV_TIP.scanRun
  }, lastScan ? /*#__PURE__*/React.createElement(React.Fragment, null, "Board last scanned ", /*#__PURE__*/React.createElement("b", null, lastScan), " \xB7 ", (rows || []).length, " names \xB7 auto-refreshes 9 AM & 6 PM ET") : /*#__PURE__*/React.createElement(React.Fragment, null, "No scan on file yet \u2014 the board this reads is filled by the watchlist scan."), status && status.error ? /*#__PURE__*/React.createElement("span", {
    className: "ab-err"
  }, " \xB7 ", status.error) : null), rows === null && /*#__PURE__*/React.createElement("div", {
    className: "rev-empty"
    ...
    k: "status",
    tip: REV_TIP.scanStatus
  }, "Reversal status"), /*#__PURE__*/React.createElement(TH, {
    k: "px",
    tip: "Last scanned price."
  }, "Price"), /*#__PURE__*/React.createElement(TH, {
    k: "ext",
    tip: `The running ${mode === "bounce" ? "low" : "high"} of the active swing — the deepest point it has reached, which is what the comparison set is conditioned on.`
  }, mode === "bounce" ? "Running low" : "Running high"), /*#__PURE__*/React.createElement(TH, {
    k: "cur",
    tip: `Current ${mode === "bounce" ? "decline" : "rally"} measured to the CURRENT price.`
  }, "Now"), /*#__PURE__*/React.createElement(TH, {
    k: "extpct",
    tip: `The same ${mode === "bounce" ? "decline" : "rally"} measured to its deepest point.`
  }, "Deepest"), /*#__PURE__*/React.createElement(TH, {
    k: "days",
    tip: "Trading days the swing has been running."
  }, "Days"), /*#__PURE__*/React.createElement(TH, {
    k: "off",
    tip: `How far price has already travelled back ${mode === "bounce" ? "up off the running low" : "down off the running high"}. The literal number — no confirmation threshold is applied.`
  }, "Off extreme"), /*#__PURE__*/React.createElement(TH, {
    k: "pen",
    tip: REV_TIP.penetration
  }, "Into zone"), /*#__PURE__*/React.createElement(TH, {
    k: "dist",
    ...
    k: "med",
    tip: `Median historical ${mode === "bounce" ? "bottom" : "top"} price, and the 25th-75th percentile zone around it.`
  }, "Zone"), /*#__PURE__*/React.createElement(TH, {
    k: "more",
    tip: "Median ADDITIONAL move comparable swings still had from this depth before they turned \u2014 measured from the price actually available at each historical crossing."
  }, "More to go"), /*#__PURE__*/React.createElement(TH, {
    k: "left",
    ...
    k: "tgt",
    tip: "The follow-on target: the median of one projection per comparable episode, each pairing that episode's own depth with its own follow-on move."
    ...
    k: "earn",
    tip: "Calendar days to the next known earnings report. Blank = none on file. The scan does not earnings-filter its history; the card does."
  }, "Earn"))), /*#__PURE__*/React.createElement("tbody", null, sorted.slice(0, 20).map(r => /*#__PURE__*/React.createElement("tr", {
    key: r.symbol,
    className: r.rz_extreme_in_zone ? "rev-inzone" : "",
  ...
  }, /*#__PURE__*/React.createElement("td", null, /*#__PURE__*/React.createElement("b", null, r.symbol), r.rz_zone_touched ? " ●" : r.rz_share_exceeded >= 75 ? " ⚠" : ""), /*#__PURE__*/React.createElement("td", {
    className: `rev-scan-st tone-${REV_STATUS_TONE[r.rz_status] || "muted"}`
  }, r.rz_status || "—"), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, r.price == null ? "—" : `$${Number(r.price).toFixed(2)}`), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, r.rz_extreme_price == null ? "—" : `$${Number(r.rz_extreme_price).toFixed(2)}`), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, Math.abs(r.rz_cur_pct != null ? r.rz_cur_pct : r.swing_pct || 0).toFixed(1), "%"), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, r.rz_extreme_pct == null ? "—" : `${Number(r.rz_extreme_pct).toFixed(1)}%`), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, r.swing_days), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, r.rz_off_extreme_pct == null ? "—" : r.rz_off_extreme_pct === 0 ? "—" : `${mode === "bounce" ? "+" : "−"}${r.rz_off_extreme_pct}%`), /*#__PURE__*/React.createElement("td", {
    className: "scan-num",
    title: r.rz_stage ? `${r.rz_stage} — ${r.rz_stage_ratio}% of a typical move.` : ""
  }, r.rz_penetration == null ? "—" : `${r.rz_penetration}%`, r.rz_stage === "EARLY IN THE MOVE" ? /*#__PURE__*/React.createElement("small", {
    className: "muted"
  }, " early") : null), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, r.rz_dist_median_pct == null ? "—" : `${r.rz_dist_median_pct > 0 ? "+" : ""}${r.rz_dist_median_pct}%`), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, r.rz_median_price == null ? "—" : `$${Number(r.rz_median_price).toFixed(2)}`, /*#__PURE__*/React.createElement("small", {
    className: "muted"
  }, r.rz_band_low == null ? "" : ` (${Number(r.rz_band_low).toFixed(2)}–${Number(r.rz_band_high).toFixed(2)})`)), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
  }, r.rz_more_move_pct == null ? "—" : `${Number(r.rz_more_move_pct).toFixed(1)}%`), /*#__PURE__*/React.createElement("td", {
    className: "scan-num"
...
function SwingPatternCard({
  ...
  const load = async (sym, pct) => {
    ...
    try {
      ...
      if (seq === reqRef.current) setErr(String(e));
    }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    disabled: loading
  }, loading ? "Loading…" : "Refresh"))), zz && zz.pct != null && /*#__PURE__*/React.createElement("div", {
    className: "swing-sens-note",
    title: SENS_TIP
  }, "A swing here means a reversal of ", /*#__PURE__*/React.createElement("b", null, (zz.pct * 100).toFixed(1), "%"), zz.source === "adaptive" && zz.typical_move_pct != null ? /*#__PURE__*/React.createElement(React.Fragment, null, " \u2014 scaled to ", ticker, "'s own travel of ", zz.typical_move_pct, "% in a typical month", zz.clamped ? " (capped)" : "") : zz.source === "explicit" ? /*#__PURE__*/React.createElement(React.Fragment, null, " \u2014 as asked for") : /*#__PURE__*/React.createElement(React.Fragment, null, " \u2014 not enough history to measure this stock's own travel, so the legacy fixed setting is used"), zz.min_move_pct != null && /*#__PURE__*/React.createElement(React.Fragment, null, ". Tables and chart lines below hide swings under ", /*#__PURE__*/React.createElement("b", null, zz.min_move_pct, "%"), ".")), err && /*#__PURE__*/React.createElement("div", {
    className: "ab-status"
  }, /*#__PURE__*/React.createElement("span", {
    className: "ab-err"
  }, err)), loading && !data && /*#__PURE__*/React.createElement("div", {
    ...
    data: data
  }), data && (data.bars || []).length > 0 && /*#__PURE__*/React.createElement(TVAdvancedChart, {
    apiFetch: apiFetch,
    ticker: ticker,
    data: data,
  ...
  }), data && data.reversal && data.reversal.ok && /*#__PURE__*/React.createElement(SwingWhatIf, {
    apiFetch: apiFetch,
    ticker: ticker,
    sens: sens,
  ...
  }, more ? "▾" : "▸", " More details \u2014 decision engine, scores, odds, target ladder, trade plan"), more && a && (rv0 => /*#__PURE__*/React.createElement("div", {
    className: "rev-context",
    title: "Two readings the reversal projection deliberately does NOT use: both were tested as ways to narrow the comparable-swing set and both were rejected for failing to improve out-of-sample accuracy. Shown as context because they describe where price sits, not because they predict anything."
  }, /*#__PURE__*/React.createElement("span", {
    title: REV_TIP.rangepos
  }, "20-day range position: ", /*#__PURE__*/React.createElement("b", null, rv0 && rv0.current && rv0.current.range_pos_20 != null ? `${rv0.current.range_pos_20} / 100` : "—")), /*#__PURE__*/React.createElement("span", {
    title: REV_TIP.regime
  }, "Trend regime: ", /*#__PURE__*/React.createElement("b", null, rv0 && rv0.current ? rv0.current.regime === "uptrend" ? "Uptrend" : rv0.current.regime === "downtrend" ? "Downtrend" : "Unknown" : "—"), " ", /*#__PURE__*/React.createElement("small", {
    className: "muted"
  }, "vs 200-day")), /*#__PURE__*/React.createElement("span", {
    title: "Both were rejected by walk-forward testing as cohort filters \u2014 kept visible, kept out of the arithmetic."
  }, "Neither narrows the comparison set.")))(data && data.reversal), more && a && a.decision && /*#__PURE__*/React.createElement("div", {
  ...
  }, "because ", a.decision.drivers.join(" · "))), more && a && a.status === "ok" && (() => {
    const reasons = [];
  ...
  })(), more && a && (a.status === "ok" || a.status === "no_rhythm") && /*#__PURE__*/React.createElement("div", {
  ...
  }, "Bullish premium"), /*#__PURE__*/React.createElement("b", {
    className: "up"
  }, fmtUsd(a.flow.bull_premium, 1))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    title: "Total premium in bearish options flow today (puts bought / calls sold)"
  ...
  })(), more && a && a.status === "ok" && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 12
    }
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "scan-th-num",
    title: "Projected price at this target"
  }, "Price"), /*#__PURE__*/React.createElement("th", {
    className: "scan-th-num",
    title: "Percent move still needed from the current price ('reached' if already hit)"
  }, "From here"), /*#__PURE__*/React.createElement("th", {
    className: "scan-th-num",
    title: "Estimated date this target is reached, based on similar past moves"
    ...
    k: "confidence_rating"
  }, "Confidence: ", a.confidence.level)), " ", /*#__PURE__*/React.createElement("span", null, "because ", (a.confidence.reasons || []).join(", "), "."))), more && a && a.status === "ok" && a.trade_plan && /*#__PURE__*/React.createElement("div", {
    className: "swing-plan"
  }, /*#__PURE__*/React.createElement("div", {
    className: "swing-subtitle",
    title: "A concrete plan for this setup: where to enter, where it's wrong, targets, and how long to hold"
    ...
    title: "Factors that support staying in / adding to this trade"
    ...
    title: "Signs that argue for taking profits or exiting"
    ...
    onClick: () => setTab("up"),
    title: "Past upward swings in this stock's history (count)"
  }, "Up-swings (", upSwings.length, ")"), /*#__PURE__*/React.createElement("button", {
    type: "button",
    className: tab === "down" ? "active" : "",
    onClick: () => setTab("down"),
    title: "Past downward swings in this stock's history (count)"
    ...
    className: "ab-empty"
  }, filtersOn && allHistSwings.length > 0 ? `No ${tab === "up" ? "up" : "down"}-swings match these filters — adjust or clear them.` : `No major ${tab === "up" ? "up" : "down"}-swings found for ${ticker} in this window.`), /*#__PURE__*/React.createElement(SwingReversalScan, {
    apiFetch: apiFetch
  }));
}
...
function WatchlistTableCard({
  ...
  const edgeCell = r => {
    ...
    return /*#__PURE__*/React.createElement("span", {
      ...
      className: cls
    }, r.edge > 0 ? "+" : "", r.edge), f && /*#__PURE__*/React.createElement("span", {
      className: `wt-flowdot ${f.tone}`,
      title: `Options flow ${f.label}${r.flow_cached ? " (cached — outside this scan's live-flow budget)" : " (live this scan)"}`
    }));
  };
  const setupCell = r => {
...
function ScanAllButton({
  ...
  const poll = async () => {
    try {
      const r = await apiFetch("/api/scan_all/status", {
        noCache: true
      });
      const d = await r.json();
      setSt(d);
  ...
  useEffect(() => {
    poll().then(d => {
      if (d && d.running && !pollRef.current) pollRef.current = setInterval(poll, 4000);
    });
  ...
  }, running ? cur ? `Scanning ${cur.label}… ${doneN}/${q.length}` : `Scanning… ${doneN}/${q.length}` : label), st && !running && q.length > 0 && st.finished_at && /*#__PURE__*/React.createElement("span", {
    className: "scanall-note",
    title: q.map(x => `${x.label}: ${x.state}${x.note ? ` (${x.note})` : ""}`).join(" · ")
  }, "last run: ", q.filter(x => x.state === "done").length, " scanned \xB7 ", q.filter(x => x.state === "skipped").length, " still fresh"));
}
function ScreenersHub({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => pick(s.id)
  }, s.label)), /*#__PURE__*/React.createElement(ScanAllButton, {
    apiFetch: apiFetch
  })), sub === "playbook" && /*#__PURE__*/React.createElement(PlaybookCard, {
    apiFetch: apiFetch,
...
function MarketContextBar({
  ...
  const rotation = useMemo(() => {
    ...
    scored.forEach(r => {
      if (r.edge == null || !r.sector) return;
      const s = agg[r.sector] || (agg[r.sector] = {
        sector: r.sector,
        net: 0,
        n: 0
      });
  ...
  if (!ctx && !board) return /*#__PURE__*/React.createElement("div", {
    className: "mctx mctx-skel"
  });
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "mctx-line"
  }, gamma ? /*#__PURE__*/React.createElement("span", {
    className: `mctx-gamma mctx-g-${gamma.regime}`,
    title: gamma.regime === "long" ? `SPY dealers are LONG gamma (net GEX +$${gamma.net_gex}B) — expect pinning / mean-reversion. Premium-selling friendly.` : `SPY dealers are SHORT gamma (net GEX $${gamma.net_gex}B) — expect trending / explosive moves. Premium is a trap; favor buying or sizing down.`
  }, /*#__PURE__*/React.createElement("span", {
    className: "mctx-dot"
  ...
  }, "No major catalysts today"), macro.map((e, i) => /*#__PURE__*/React.createElement("span", {
    key: i,
    className: "mctx-ev",
    title: `${e.event} — ${e.today ? "today " + (e.time || "") : "tomorrow"}. High-impact macro; expect a volatility spike.`
  }, /*#__PURE__*/React.createElement("span", {
    ...
    title: `Load ${x.sym} — earnings in ${x.days}d`
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    if (a && a.ok) {
      ...
      if (a.authish.some(c => c.partitioned)) {
        return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("b", null, "The login cookie is partitioned."), " The frame holds its own isolated copy rather than your real session, so it reads as signed out.");
      }
      return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("b", null, "The login cookie looks correctly set for framing"), " ", "(", a.crossSiteReady, " cross-site-ready). If it still drops, the app is rejecting the session for another reason.");
    }
    if (!diag.topTab) {
  ...
  if (helperVer >= needVer) {
    return /*#__PURE__*/React.createElement("div", {
      ...
      onClick: () => {
        ...
        const w = window.open("https://www.tradingview.com/accounts/signin/", "jerry_tv_login", "width=520,height=760,noopener=no");
        if (!w) return;
        const t = setInterval(() => {
          try {
            if (w.closed) {
...
function FinvizPanel({
  ...
  }, [["elite", "Elite"], ["free", "Free"]].map(([v, l]) => /*#__PURE__*/React.createElement("button", {
    key: v,
    className: base === v ? "active" : "",
    onClick: () => {
      FINVIZ.setBase(v);
      setBase(v);
      setSrc(FINVIZ.quoteUrl(ticker));
    }
  }, l))), /*#__PURE__*/React.createElement("button", {
    className: "rr-btn",
...
function _wosGreeks(spot, strike, ivPct, dteDays, side) {
  ...
  const T = Math.max(0.5, dteDays) / 365,
    r = 0.04;
  ...
  const theta = (-(spot * pdf * iv) / (2 * Math.sqrt(T)) + (side === "put" ? r * strike * Math.exp(-r * T) * _wosCdf(-d2) : -r * strike * Math.exp(-r * T) * _wosCdf(d2))) / 365;
  return {
    delta,
    theta,
    probOTM: side === "put" ? (1 - Math.abs(delta)) * 100 : (1 - delta) * 100
  };
}
...
function WeeklySellSetupCard({
  ...
  const lows = rows.map(r => r.low_return),
    highs = rows.map(r => r.high_return);
  const worstLow = Math.min(...lows),
    bestHigh = Math.max(...highs);
  ...
  function ZoneScale() {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "wos-rl-h",
      title: `Every number here is measured over the ${plan.sessions} trading session${plan.sessions === 1 ? "" : "s"} between now and this expiry — not a calendar week.`
    }, "SELL ZONE \xB7 NEXT ", plan.sessions, " SESSION", plan.sessions === 1 ? "" : "S"), /*#__PURE__*/React.createElement("div", {
      ...
      className: "num"
    }, fmt$(currentPrice, currentPrice >= 1000 ? 0 : 2))), /*#__PURE__*/React.createElement("div", {
      className: "wos-track wsz-track"
    }, tLo != null && tHi != null && /*#__PURE__*/React.createElement("i", {
      className: "wsz-typical",
      style: {
        left: `${tLo}%`,
        width: `${Math.max(1, tHi - tLo)}%`
      },
      title: `Half of all matched ${plan.sessions}-session windows closed inside this band.`
    }), putK != null && /*#__PURE__*/React.createElement("i", {
      className: "wsz-strike put",
      style: {
        left: `${at(putK)}%`
      },
      title: `Recommended put strike ${fmt$(putK, 2)}`
    }), callK != null && /*#__PURE__*/React.createElement("i", {
      className: "wsz-strike call",
      style: {
        left: `${at(callK)}%`
      },
      title: `Recommended call strike ${fmt$(callK, 2)}`
    }), /*#__PURE__*/React.createElement("i", {
      ...
      title: `Sell puts at or below this level. Over ${plan.sample.n} matched ${plan.sessions}-session windows, this stock closed below it about ${plan.band.target_itm_pct.toFixed(0)}% of the time. The line came from ${srcWord(lo.source)} — the engine keeps whichever of the two is more cautious.`
    }, /*#__PURE__*/React.createElement("em", null, "SELL PUTS BELOW"), /*#__PURE__*/React.createElement("b", {
      className: "num"
    }, f$(lo.price)), /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, fp(lo.pct), " \xB7 ", lo.source)), /*#__PURE__*/React.createElement("span", {
      className: "wos-end hi",
      title: `Sell calls at or above this level. Over ${plan.sample.n} matched ${plan.sessions}-session windows, this stock closed above it about ${plan.band.target_itm_pct.toFixed(0)}% of the time. The line came from ${srcWord(hi.source)} — the engine keeps whichever of the two is more cautious.`
    }, /*#__PURE__*/React.createElement("em", null, "SELL CALLS ABOVE"), /*#__PURE__*/React.createElement("b", {
      className: "num"
    }, f$(hi.price)), /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, fp(hi.pct), " \xB7 ", hi.source))), typical && typical.low && typical.low.price != null && /*#__PURE__*/React.createElement("div", {
    ...
    }, /*#__PURE__*/React.createElement("em", null, "ORDINARY WEEK"), /*#__PURE__*/React.createElement("b", {
      className: "num"
    }, fmt$(typical.low.price, 2)), /*#__PURE__*/React.createElement("span", null, "to ", fmt$(typical.high.price, 2))), /*#__PURE__*/React.createElement("div", {
      className: "wos-posline"
    }, /*#__PURE__*/React.createElement("span", {
      title: `Overlapping windows from ${plan.sample.n + plan.sessions} sessions of daily history. Overlap is why the count is high — every observation the history holds is used, and the count is shown rather than dressed up as independent weeks.`
    }, "measured on ", /*#__PURE__*/React.createElement("b", {
      className: "num"
    }, plan.sample.n), " matching ", plan.sessions, "-session windows", plan.sample.from ? /*#__PURE__*/React.createElement(React.Fragment, null, " since ", /*#__PURE__*/React.createElement("b", {
      className: "num"
    }, plan.sample.from)) : null), emPct != null && /*#__PURE__*/React.createElement("span", {
      title: plan.em && plan.em.source === "straddle" ? "The at-the-money straddle mid on the live chain — what the option market is charging for the move to this expiry." : "Expected move to this expiry."
    }, "market\u2019s priced move ", /*#__PURE__*/React.createElement("b", {
      className: "num"
    }, "\xB1", emPct.toFixed(1), "%"))));
  }
  function PickCol({
    ...
    const mine = p && userK != null && Math.abs(userK - p.strike) > 0.001 ? _wosFind(S.all, userK) : null;
    if (!p) {
      return /*#__PURE__*/React.createElement("div", {
        className: "wos-side"
      }, /*#__PURE__*/React.createElement("div", {
        className: "wos-side-h",
        style: {
          color: col
        }
    ...
    const evTip = side === "put" ? `Average result of selling this put across all ${p.sample} matched windows: the credit collected, minus what it cost on the windows that finished below the strike, as a yearly rate on the cash the trade ties up. History, not a forecast.` : `Average result of selling this call instead of simply holding the shares, across all ${p.sample} matched windows, as a yearly rate. A negative number means the stock ran through this strike often enough that the premium did not pay for the upside given up.`;
    return /*#__PURE__*/React.createElement("div", {
      className: "wos-side"
    }, /*#__PURE__*/React.createElement("div", {
      className: "wos-side-h",
      ...
      className: "wos-strike num",
      title: (p.why || []).join(" · ")
    }, fmt$(p.strike, p.strike >= 1000 ? 0 : 2))), p.relaxed && /*#__PURE__*/React.createElement("div", {
      className: "wos-relaxed",
      title: S.note || ""
    }, "best available \u2014 see note"), /*#__PURE__*/React.createElement(Row, {
      l: "Credit \xB7 bid/mid",
      v: /*#__PURE__*/React.createElement("span", {
        className: "num"
      }, p.bid != null ? fmt$(p.bid, 2) : "—", " / ", fmt$(p.mid, 2)),
      ...
      v: /*#__PURE__*/React.createElement("span", {
        className: "num"
      }, fmt$(p.breakeven, 2), " \xB7 ", fp(p.cushion_pct)),
      ...
      v: /*#__PURE__*/React.createElement("span", {
        className: "num"
      }, pct0(p.itm_pct), " hist \xB7 ", p.p_otm == null ? "—" : pct0(100 - p.p_otm), " mkt"),
      ...
      hot: true,
      l: /*#__PURE__*/React.createElement(React.Fragment, null, "Edge \xB7 a year ", /*#__PURE__*/React.createElement("i", {
        className: "wos-est"
      }, "hist")),
      v: /*#__PURE__*/React.createElement("span", {
        className: "num"
      }, p.ev_ann_pct == null ? "—" : `${p.ev_ann_pct >= 0 ? "+" : ""}${p.ev_ann_pct.toFixed(0)}%`),
      tone: evTone,
      tip: evTip
    }), /*#__PURE__*/React.createElement(Row, {
      l: "Return \xB7 week \xB7 year",
      v: /*#__PURE__*/React.createElement("span", {
        className: "num"
      }, p.rbp_pct.toFixed(2), "% \xB7 ", p.ann_pct == null ? "—" : p.ann_pct.toFixed(0) + "%"),
    ...
    }, fmt$(a.strike, a.strike >= 1000 ? 0 : 2)))), mine && /*#__PURE__*/React.createElement("div", {
      className: `wos-mine ${mine.gate ? "warn" : ""}`,
      title: mine.gate ? `Your picked strike, measured exactly the same way. It is ${mine.gate} — the engine would not sell it here.` : "Your picked strike, measured exactly the same way. It clears the zone too; the engine simply scored another strike higher."
    }, /*#__PURE__*/React.createElement("em", null, "YOUR PICK"), /*#__PURE__*/React.createElement("b", {
      className: "num"
    }, fmt$(mine.strike, mine.strike >= 1000 ? 0 : 2)), /*#__PURE__*/React.createElement("span", null, "assigned ", pct0(mine.itm_pct), " hist \xB7 ", mine.p_otm == null ? "—" : pct0(100 - mine.p_otm), " mkt", mine.ev_ann_pct != null ? ` · ${mine.ev_ann_pct >= 0 ? "+" : ""}${mine.ev_ann_pct.toFixed(0)}%/yr` : "", mine.gate ? ` — ${mine.gate}` : "")));
  }
  ...
  function LegacyRange() {
    const dLow$ = currentPrice - pLow,
      dLowPts = currReturn - worstLow;
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      ...
      className: "wos-end lo",
      title: `Worst weekly low of the ${rows.length} displayed weeks.`
    }, /*#__PURE__*/React.createElement("em", null, "WORST LOW"), /*#__PURE__*/React.createElement("b", {
      className: "num"
    }, fp(worstLow)), /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, f$(pLow))), /*#__PURE__*/React.createElement("span", {
      className: "wos-end hi",
      title: `Best weekly high of the ${rows.length} displayed weeks.`
      ...
      className: "wos-prox-box",
      title: "Where this week's return sits inside the displayed range. A location measure only."
      ...
      className: "num cu"
    }, f$(Math.abs(dHigh$))), " \xB7 ", Math.abs(dHighPts).toFixed(1), "% below the best high")));
  }
  const verdict = plan && plan.verdict ? plan.verdict : null;
  const vTone = !verdict ? "mut" : verdict.side === "put" ? "up" : verdict.side === "call" ? "down" : "mut";
  return /*#__PURE__*/React.createElement("div", {
    className: "card wos-card"
  }, /*#__PURE__*/React.createElement("div", {
    className: "card-head"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    className: "kicker",
    title: plan ? `Every strike on the live chain for this expiry, scored on what this stock actually did over the last ${plan.sample ? plan.sample.n : 0} matching ${plan.sessions}-session windows and on whether the strike can be filled. Not a fixed delta.` : `Where this week sits inside the last ${rows.length} weeks' range.`
  }, plan ? "Which strike is worth selling" : "Weekly option selling setup"), /*#__PURE__*/React.createElement("div", {
    className: "card-title"
  }, plan ? "Sell where history and the option market both say it is safe" : "Sell puts near the lows · calls near the highs")), dte != null && expiration && /*#__PURE__*/React.createElement("span", {
    className: "wos-dte",
    title: `Selected weekly expiration ${expiration} — ${dte} days away${plan ? `, ${plan.sessions} trading session${plan.sessions === 1 ? "" : "s"} of risk` : ""}.`
  }, "EXP FRI ", expiration.slice(5).replace("-", "/"), " \xB7 ", /*#__PURE__*/React.createElement("b", null, dte, "d")), plan && plan.earnings && plan.earnings.inside && /*#__PURE__*/React.createElement("span", {
    className: "wos-earn-warn",
    title: `Earnings are due ${plan.earnings.date}, inside this expiry. The history behind these numbers is mostly ordinary weeks, so it understates the risk here.`
  }, "EARNINGS INSIDE"), /*#__PURE__*/React.createElement("span", {
    className: `wos-bias ${vTone}`,
    title: plan ? "Leads with expected value measured on this stock's own history, then assignment odds, fill quality and the cushion against the option market's own priced move. A read on where the pay is, not an instruction." : "The engine's plan is not available for this symbol right now, so the panel is showing range location only."
  }, verdict ? verdict.line : "Range location only")), /*#__PURE__*/React.createElement("div", {
    className: "wos-range"
  }, plan && zone ? /*#__PURE__*/React.createElement(ZoneScale, null) : /*#__PURE__*/React.createElement(LegacyRange, null), lowsInBy != null && /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement("em", null, DAY_NAMES[dow].toUpperCase(), dow === 4 ? " · WEEK NEARLY COMPLETE" : ""), /*#__PURE__*/React.createElement("span", null, "low already in: ", /*#__PURE__*/React.createElement("b", {
    className: `num ${lowsInBy >= 70 ? "cu" : ""}`
  }, lowsInBy.toFixed(0), "%"), "  ·  ", "high already in: ", /*#__PURE__*/React.createElement("b", {
    className: "num"
  }, highsInBy.toFixed(0), "%"))), (() => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "wos-plain",
      title: "The same numbers as the column on the right, said once in plain words."
    }, "Sell the ", /*#__PURE__*/React.createElement("b", null, fmt$(p.strike, p.strike >= 1000 ? 0 : 2), " ", lead), " for about ", /*#__PURE__*/React.createElement("b", null, fmt$(p.mid, 2)), ".", " ", "Over ", /*#__PURE__*/React.createElement("b", null, p.sample), " matching ", plan.sessions, "-session stretches ", ticker, " closed ", word, " it", " ", /*#__PURE__*/React.createElement("b", null, pct0(p.itm_pct)), " of the time", mkt != null ? /*#__PURE__*/React.createElement(React.Fragment, null, ", and the option market puts it at ", /*#__PURE__*/React.createElement("b", null, pct0(mkt))) : null, ".", p.ev_ann_pct != null && /*#__PURE__*/React.createElement(React.Fragment, null, " ", "Counting the times it went wrong, that trade averaged", " ", /*#__PURE__*/React.createElement("b", {
      className: p.ev_ann_pct >= 0 ? "cu" : "cd"
    }, p.ev_ann_pct >= 0 ? "+" : "", p.ev_ann_pct.toFixed(0), "% a year"), lead === "call" ? " against just holding the shares" : " on the cash it ties up", "."), p.relaxed ? " Nothing cleared the usual line this week, so this is the best available." : "", plan.earnings && plan.earnings.inside ? " Earnings land inside this expiry, so the history behind these odds understates the risk." : "");
  })(), plan && !plan.ok && plan.note && /*#__PURE__*/React.createElement("div", {
    className: "wos-plan-note",
    title: "The engine ran and found nothing it would sell. That is an answer."
  }, plan.note)), /*#__PURE__*/React.createElement("div", {
    className: "wos-sides"
  }, plan && zone ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(PickCol, {
    side: "put"
  }), /*#__PURE__*/React.createElement(PickCol, {
    side: "call"
  })) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(LegacySide, {
    label: "SELL PUT",
    c: putC,
    side: "put",
    ...
    emUp: emUp,
    emDn: emDn,
    emFromBand: emFromBand
  }), /*#__PURE__*/React.createElement(LegacySide, {
    label: "SELL CALL",
    c: callC,
    side: "call",
    currentPrice: currentPrice,
...
function LegacySide({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "wos-strike num"
  }, fmt$(c.strike, 2))), /*#__PURE__*/React.createElement(R, {
    l: "Bid / mid",
    v: /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, c.bid != null ? fmt$(c.bid, 2) : "—", " / ", fmt$(mid, 2))
  }), /*#__PURE__*/React.createElement(R, {
    hot: true,
    l: "Breakeven",
    v: /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, fmt$(breakeven, 2), " \xB7 ", beDistPct >= 0 ? "+" : "", beDistPct.toFixed(1), "%")
  }), /*#__PURE__*/React.createElement(R, {
    l: "Delta",
    v: delta != null ? /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, delta.toFixed(2)) : NA
  }), /*#__PURE__*/React.createElement(R, {
    hot: true,
    l: "P(expire OTM)",
    v: probOTM != null ? /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, probOTM.toFixed(0), "% ", /*#__PURE__*/React.createElement("i", {
      className: "wos-est"
    }, "est")) : NA
  }), /*#__PURE__*/React.createElement(R, {
    hot: true,
    ...
    v: /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, (breachN / rows.length * 100).toFixed(0), "% (", breachN, "/", rows.length, ")")
  }), /*#__PURE__*/React.createElement(R, {
    l: "EM to Friday",
    v: emPct != null ? /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, "\xB1", emPct.toFixed(1), "%") : NA
  }), /*#__PURE__*/React.createElement(R, {
    l: "Return on BP",
    v: /*#__PURE__*/React.createElement("span", {
      className: "num"
