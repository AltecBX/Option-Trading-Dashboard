...
const TABS = [
  ...
  { id: "buyers", label: "Insider & Congress" },
...
const TAB_GROUPS = [
  ...
  { id: "research", label: "Research",
    ...
    ids: ["context", "sectors", "gex", "breadth", "flow", "hedge", "invest",
          "backtest", "calendar", "treasuries", "news"] },
...
const TVIEW = {
  ...
  helperVersionLabel() {
    try { return document.documentElement.dataset.finvizHelperVersion || ""; } catch (e) { return ""; }
  },
  ...
  learned() {
    try { return JSON.parse(localStorage.getItem(this.LEARN_KEY) || "{}") || {}; }
    catch (e) { return {}; }
  },
  learnedUrl(sym) {
...
function workspaceEl() {
  try { return document.querySelector(".main"); } catch (e) { return null; }
}
...
function scrollWorkspaceTo(y, smooth) {
  const el = workspaceEl();
  if (!el) return;
  try { el.scrollTo({ top: y, behavior: smooth ? "smooth" : "auto" }); }
...
const DATA_STATUS = {
  ...
  pending:  { label: "Not scanned yet", cls: "ds-pending",
...
function DataStatus({ kind, at, note, label }) {
  const k = DATA_STATUS[kind] || DATA_STATUS.none;
  const when = fmtStatusAt(at);
  const tip = [k.tip, when ? `As of ${when}.` : null, note || null]
    .filter(Boolean).join(" ");
  return (
    <span className={`ds-badge ${k.cls}`} title={tip}>
      <span className="ds-dot" aria-hidden="true" />
      <span className="ds-lbl">{label || k.label}</span>
      {when && <span className="ds-at">{when}</span>}
    </span>
  );
}
...
function SectionNav({ tab, active, groups, label }) {
  ...
  const scan = React.useCallback(() => {
    ...
    if (!roots.length) { setItems([]); return; }
    const out = [];
    const seen = new Set();
    roots.forEach((root) => root.querySelectorAll(".card").forEach((card) => {
      ...
      if (!card.id) {
        card.id = "sec-" + text.toLowerCase().replace(/[^a-z0-9]+/g, "-")
          .replace(/^-|-$/g, "");
      }
      seen.add(text);
      ...
      out.push({ id: card.id, text, short, group: (groups && groups(text)) || "" });
    }));
    ...
    if (groups) {
      const rank = new Map();
      out.forEach((it) => { if (it.group && !rank.has(it.group)) rank.set(it.group, rank.size); });
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      for (const it of items) {
        ...
        if (t <= top && t > bestTop) { bestTop = t; best = it.id; }
      }
      setHere(best);
    };
    onScroll();
  ...
  const go = (id) => {
    ...
    const root = workspaceEl();
    if (!el || !root) return;
    const y = root.scrollTop + el.getBoundingClientRect().top
  ...
  if (isPhone) {
    ...
    return (
      <React.Fragment>
        <nav className="secnav secnav-phone" aria-label={label || "Sections on this page"}>
          <button type="button" className="secnav-open" onClick={() => setPick(true)}
                  title="Every panel on this page, searchable. Nothing is hidden — this only moves you to one.">
            <span className="secnav-lbl-sm">Jump to</span>
            <span className="secnav-here">{here ? here.short || here.text : "a section"}</span>
            <span className="secnav-count">{items.length}</span>
          </button>
        </nav>
        {pick && (
          <div className="tabsheet-overlay" onClick={() => { setPick(false); setFind(""); }}>
            <div className="tabsheet secnav-sheet" role="dialog"
                 aria-label={label || "Sections on this page"}
                 onClick={e => e.stopPropagation()}>
              <div className="tabsheet-head">
                <span>{label || "Sections"}</span>
                <button className="tabsheet-x" aria-label="Close"
                        onClick={() => { setPick(false); setFind(""); }}>✕</button>
              </div>
              <input className="tabsheet-find" type="search" autoFocus
                     placeholder="Find a panel on this page…" aria-label="Find a panel"
              ...
              <div className="tabsheet-scroll">
                {(() => {
                  ...
                  if (!shown.length) {
                    return <div className="secnav-none">No panel on this page matches “{find}”.</div>;
                  }
                  let g = null;
                  return shown.map((it) => {
                    const head = it.group && it.group !== g
                      ? <div className="tabsheet-glbl" key={it.id + "-g"}>{it.group}</div> : null;
                    g = it.group || g;
                    return (
                      <React.Fragment key={it.id}>
                        ...
                        <button type="button"
                                ...
                                onClick={() => { go(it.id); setPick(false); setFind(""); }}>
                          {it.text}
                        </button>
                      </React.Fragment>
                    );
      ...
      <div className="secnav-row">
        {items.map((it) => {
          const head = it.group && it.group !== lastGroup
            ? <span key={it.id + "-g"} className="secnav-group">{it.group}</span> : null;
          ...
          return (
            <React.Fragment key={it.id}>
              ...
              <button type="button"
                      ...
                      onClick={() => go(it.id)}
                      title={`Jump to ${it.text}`}>{it.short || it.text}</button>
            </React.Fragment>
          );
        })}
      </div>
