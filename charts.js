...
function attachTouchZoom(el, chart, {
  getBarCount,
  onDoubleTap
} = {}) {
  ...
  const onStart = e => {
    if (e.touches.length === 2) {
      ...
      let anchor = null;
      try {
        anchor = ts.coordinateToLogical(x);
      } catch (err) {}
...
function TVPriceChart({
  ...
  React.useEffect(() => {
    ...
    if (!chart) return;
    let main;
    if (chartStyle === "area") main = chart.addAreaSeries({
      lineColor: "#22c55e",
      topColor: "rgba(34,197,94,0.35)",
      bottomColor: "rgba(34,197,94,0.02)",
      lineWidth: 2
    });else if (chartStyle === "ohlc") main = chart.addBarSeries({
      upColor: "#22c55e",
      downColor: "#ef4444"
    });else main = chart.addCandlestickSeries({
      upColor: "#22c55e",
      downColor: "#ef4444",
  ...
  React.useEffect(() => {
    ...
    extraRef.current.series.forEach(s => {
      try {
        chart.removeSeries(s);
      } catch (e) {}
    });
    extraRef.current.lines.forEach(pl => {
      try {
    ...
    const addLine = (data, color) => {
      ...
      s.setData(data);
      extraRef.current.series.push(s);
    };
    ...
    const pl = (price, color, style, title) => {
      if (price == null) return;
      extraRef.current.lines.push(main.createPriceLine({
        price: +price,
        color,
        lineWidth: 1,
        lineStyle: style,
    ...
    const RCV_COLOR = {
      prior_high: "rgba(34,197,94,0.9)",
      correction_low: "rgba(234,179,8,0.9)",
      invalidation: "rgba(239,68,68,0.95)",
      bounce_high: "rgba(56,189,248,0.85)",
      higher_low: "rgba(168,85,247,0.85)"
    };
    (levels || []).forEach(lv => {
      if (!lv || lv.price == null) return;
      pl(lv.price, RCV_COLOR[lv.kind] || "rgba(148,163,184,0.7)", lv.kind === "invalidation" ? LC.LineStyle.Dashed : LC.LineStyle.Dotted, lv.label);
    });
    if (earnings) {
      const m = [];
      (earnings.past || []).forEach(d => {
        const t = norm(d && d.date ? d.date : d);
        if (t) m.push({
          time: t,
          position: "belowBar",
          color: "#a855f7",
      ...
      if (earnings.next) m.push({
        time: norm(earnings.next),
        position: "aboveBar",
        color: "#a855f7",
        shape: "circle",
        text: "E"
...
function ReturnsChart({
  ...
  Math.abs(yScale(t) - yScale(dataHi)) < 12 || Math.abs(yScale(t) - yScale(dataLo)) < 12 || medianLines.some(m => Math.abs(yScale(t) - yScale(m.v)) < 12) ? null : /*#__PURE__*/React.createElement("g", {
    ...
    className: "axis-text",
    textAnchor: "end"
  }, t.toFixed(0), "%"))), /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("line", {
    x1: padL,
    x2: W - padR,
    y1: yScale(dataHi),
    y2: yScale(dataHi),
  ...
  }, dataHi >= 0 ? "+" : "", dataHi.toFixed(1), "%"), /*#__PURE__*/React.createElement("line", {
    x1: padL,
    x2: W - padR,
    y1: yScale(dataLo),
    y2: yScale(dataLo),
  ...
  }, dataLo.toFixed(1), "%")), /*#__PURE__*/React.createElement("line", {
    x1: padL,
    x2: W - padR,
    y1: yScale(0),
    y2: yScale(0),
...
function WeeklyRecap({
  ...
  const day = d => fmtDate(d.week_start, {
    month: "short",
    day: "numeric"
  });
  ...
  const Tile = ({
    ...
    className: "wrc-tile",
    title: tip
  }, /*#__PURE__*/React.createElement("em", null, lbl), /*#__PURE__*/React.createElement("b", {
    className: tone ? `wrc-${tone}` : ""
  }, val), /*#__PURE__*/React.createElement("span", null, sub));
  return /*#__PURE__*/React.createElement("div", {
    className: "wrc"
  }, /*#__PURE__*/React.createElement("div", {
    className: "wrc-head"
  }, "What these ", n, " weeks add up to"), /*#__PURE__*/React.createElement("div", {
    className: "wrc-tiles"
  }, /*#__PURE__*/React.createElement(Tile, {
    lbl: "WEEKS CLOSED GREEN",
    val: `${green} of ${n}`,
    ...
    lbl: "TYPICAL WEEK RANGE",
    val: `${avgRange.toFixed(1)}%`,
  ...
  }), trend ? /*#__PURE__*/React.createElement(Tile, {
    lbl: "RANGE TREND",
    ...
    className: "wrc-strip-lbl"
  }, /*#__PURE__*/React.createElement("span", null, "WEEKLY RANGE \xB7 OLDEST TO NEWEST"), /*#__PURE__*/React.createElement("span", {
    className: "wrc-strip-avg"
  }, "average ", avgRange.toFixed(1), "%")), /*#__PURE__*/React.createElement("svg", {
    viewBox: `0 0 ${SW} ${SH}`,
    ...
    }, /*#__PURE__*/React.createElement("title", null, `${day(d)} · range ${r.toFixed(1)}% · closed ${pct(d.close_return, 1)}`));
  }))));
}
...
function DayBarChart({
  ...
  const uid = useMemo(() => "dow" + mode + Math.floor(Math.random() * 1e6), [mode]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("g", {
      key: d
    }, h > 0 && /*#__PURE__*/React.createElement("path", {
      d: topRoundRect(x, y, bw, h, 7),
      className: "dow-bar",
      style: {
...
function IntradayChart({
  ...
  React.useEffect(() => {
    ...
    set(r.vwap, vw && vw.vwap);
    set(r.u1, vw && vw.upper1);
    set(r.l1, vw && vw.lower1);
    set(r.u2, vw && vw.upper2);
    set(r.l2, vw && vw.lower2);
    // Level map as labeled price lines (replace on every refresh).
    r.priceLines.forEach(pl => {
      try {
        r.candles.removePriceLine(pl);
      } catch (e) {}
    });
    r.priceLines = [];
    const lvColor = k => k === "pdh" || k === "pdl" || k === "pdc" ? "rgba(234,179,8,0.8)" : k === "pmh" || k === "pml" ? "rgba(168,85,247,0.8)" : k === "orh" || k === "orl" ? "rgba(255,255,255,0.55)" : k === "emh" || k === "eml" ? "rgba(56,189,248,0.8)" : "rgba(148,163,184,0.5)";
    (data.levels || []).forEach(lv => {
      try {
        r.priceLines.push(r.candles.createPriceLine({
          price: lv.price,
          color: lvColor(lv.kind),
          lineWidth: 1,
