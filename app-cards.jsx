...
function SwingChart({ data, focusKey, onPickSwing, onClearFocus }) {
  ...
  const [show, setShow] = useState(() => {
    ...
    return { markers: true, lines: true, up: true, down: true, current: true, targets: true, zones: true, labels: !phone };
  });
  ...
  useEffect(() => {
    ...
    candleRef.current.setData(ascendingByTime(drawable.map(b => ({ time: b.t, open: b.o, high: b.h, low: b.l, close: b.c }))));
    volRef.current.setData(ascendingByTime(drawable.map(b => ({ time: b.t, value: b.v, color: b.c >= b.o ? "rgba(34,197,94,0.30)" : "rgba(239,68,68,0.30)" }))));
    applyHome();
    /* eslint-disable-next-line */
  }, [data, collapsed]);
  ...
  useEffect(() => {
    ...
    if (show.zones && rv && rv.ok && rv.zone) {
      ...
      // The running extreme (v4.51). When it has already entered the zone
      // and price has moved away from it, these two lines are the picture:
      // the marker sits inside the band, the current price does not.
      if (rv.current && rv.current.extreme_price != null)
        mk2(rv.current.extreme_price, "rgba(148,163,184,0.75)", LC.LineStyle.Dashed);
      // The band the STATUS refers to: where this stock's swings usually
      // end, unconditionally. Fainter than the projection, because it is
      // the reference the extreme travels into rather than the forecast.
      if (rv.typical_zone) {
        mk2(rv.typical_zone.band_low_price, "rgba(245,158,11,0.28)", LC.LineStyle.Dotted);
        mk2(rv.typical_zone.band_high_price, "rgba(245,158,11,0.28)", LC.LineStyle.Dotted);
      }
      if (rv.next && rv.next.target_median_price != null)
        mk2(rv.next.target_median_price, rv.direction === "down" ? UPC : DNC, LC.LineStyle.Dotted);
    }
    /* eslint-disable-next-line */
  ...
  if (show.zones && rv && rv.ok && rv.zone) {
    ...
    const st = rv.status || {}, rc = rv.current || {};
    // The unfinished swing, captioned once: where it is now, how deep it
    // has been, and how long it has been running.
    if (rc.pct != null)
      legend.push({
        name: "swing",
        text: `CURRENT ${rv.direction === "down" ? "↓" : "↑"}${Math.abs(rc.pct).toFixed(1)}% · EXTREME ${rv.direction === "down" ? "↓" : "↑"}${Number(rc.extreme_abs_pct).toFixed(1)}% · ${rc.days}d`,
        tip: REV_TIP.extreme,
      });
    legend.push({ name: `${revName} zone`, price: rv.zone.band_low_price, price2: rv.zone.band_high_price, color: "rgba(245,158,11,0.85)" });
    legend.push({ name: `med ${revName}`, price: rv.zone.median_price, color: "#f59e0b" });
    if (rc.extreme_price != null) {
      // Label carries the state so the chart says the same thing as the
      // panel: how far price has come back off the extreme, and whether
      // that extreme ever reached the zone.
      const off = rc.off_extreme_pct ? ` ${rv.direction === "down" ? "+" : "−"}${rc.off_extreme_pct}% off` : "";
      legend.push({
        name: `${rv.direction === "down" ? "running low" : "running high"}${st.zone_touched ? " ✓zone" : ""}${off}`,
        price: rc.extreme_price, color: "rgba(148,163,184,0.95)",
      });
    }
    if (rv.typical_zone)
      legend.push({ name: `usual ${revName}s`, price: rv.typical_zone.band_low_price, price2: rv.typical_zone.band_high_price, color: "rgba(245,158,11,0.5)" });
    if (rv.next && rv.next.target_median_price != null)
      legend.push({ name: rv.direction === "down" ? "bounce tgt" : "pullback tgt", price: rv.next.target_median_price, color: rv.direction === "down" ? UPC : DNC });
  }
...
const REV_TIP = {
  ...
  extreme: "The running extreme: the lowest low (or highest high) this swing has printed so far, and how far the current price has already travelled back from it. This is the distinction that matters at the right edge — a stock that touched its historical bottom zone and has since risen 6% is in a different situation from one still falling into that zone, and everything below is conditioned on the EXTREME while every distance is measured from today's PRICE.",
  status: "Where the running extreme stands against the historical reversal zone, decided by three yes/no questions in a fixed order and no invented confirmation threshold — the zone itself is the yardstick. APPROACHING: the extreme has not reached the near edge yet. IN ZONE: the extreme is inside the band. BOUNCING / FADING OFF ZONE: the extreme reached the band and price has since traded back out of it. BEYOND TYPICAL ZONE: the extreme is past the far edge and price is still out there. BEYOND HISTORY: no completed swing ever reached this magnitude, so there is no band at all.",
  maturity: "How far into a NORMAL move this one is, as a share of the median completed swing in this direction — and, from walk-forward testing rather than taste, how much the conditional projection is worth at that stage. Below 100% the comparison set is nearly the whole history and conditioning measurably adds nothing over the plain rhythm. At 100% it starts to pay (about a fifth less error). Past 125% it matters most: there the unconditional rhythm keeps projecting a turn the move has already passed, and its band covered only a quarter of outcomes. A label, never a multiplier — no number on this panel is scaled by it.",
  ...
  typical: "The band where EVERY completed swing in this direction has ended — the 25th to 75th percentile of all of them, projected from this swing's own starting pivot. This is the band the reversal status is measured against, and deliberately not the conditional zone below it: every swing in that conditional set ended at or beyond the current depth, so its shallow edge sits at or past the running extreme BY CONSTRUCTION and 'in the zone' could never fire. Two different questions — where do this stock's swings usually end, and where does THIS one end given it has already come this far.",
  ...
  next: "What the opposite swing after the reversal historically looked like, across the same comparable swings. Sizes are measured from each actual reversal pivot. The reversal has to happen first — none of these is a probability of reversing.",
  paired: "The follow-on target, built from PAIRS rather than from a product. Each comparable episode contributes its own final depth AND the size of the move that actually followed it; both are applied to today's swing origin, giving one projected target per episode, and the band is the 25th-to-75th percentile of those projections. The older 'median depth × median rebound' number is kept only as a diagnostic: multiplying two medians produces a target no single historical episode ever traced, and its band mixes a fixed depth with a variable move. Honest caveat — across 16 symbols the rank correlation between reversal depth and the size of the next move is about zero (−0.02), so pairing moves the median target by only about 1%; what it fixes is the BAND, which is now a distribution of real outcomes.",
  horizon: "Fixed-horizon excursions: how often the stock moved X% within exactly N trading days of a historical reversal, measured from that day's CLOSE — beside the identical question asked of every ordinary day in the record. Same target, same horizon, or the two numbers cannot be subtracted. The bracketed figure is the conservative (Wilson lower-bound) end of the conditional rate; the baseline is a point estimate only, because its windows overlap. READ THESE AS WHAT FOLLOWED A TURN, NOT AS THE CHANCE OF A TURN: they start at bottoms that were only known to be bottoms afterwards. The what-if below is the version that starts where you would actually have had to buy.",
  ...
  whatif: "A first-touch race from RIGHT HERE, not from the projected reversal: entry is simulated on the day each comparable historical swing FIRST reached the current depth, at the price actually available that day — the session open whenever the market gapped straight through the level overnight, because that level never existed as a live price. So the extra adverse move before the actual reversal is inside the trade, which is the real risk of entering before the turn. Both levels inside one daily bar counts as ambiguous (the intraday order is unknowable from daily bars) and is never counted in the trade's favor. Episodes whose follow-on leg contains a split or an incredible print are dropped rather than raced through bad prices.",
  gap: "How many of these historical entries had to be filled at the session open because the stock gapped through the theoretical level overnight. Typically a third of them — pretending you got the exact level would flatter every one of these numbers.",
...
function RevStat({ label, tip, tone, children }) {
  return (
    <div className="rev-stat" title={tip}>
      <span>{label}</span>
...
function SwingWhatIf({ apiFetch, ticker, sens, reversal, onResult }) {
  ...
  // The SETUP identity (v4.51). Prefills used to be filled only when the
  // field was empty, so a target computed for one ticker survived into the
  // next one and silently raced the wrong number. Anything that changes what
  // "here" means — the symbol, the sensitivity, the swing's origin or
  // direction, or the depth the cohort is conditioned on — resets them.
  // Nothing else does, so typed values survive while the setup does not move.
  const setupKey = [ticker, sens, reversal && reversal.direction,
    cur.from_date, cur.extreme_abs_pct].join("|");
  useEffect(() => {
    setTgt(nxt && nxt.pct_median != null ? String(Math.round(nxt.pct_median)) : "");
    setStp(rem && rem.more_move_p75_pct != null
      ? String(Math.max(1, Math.round(rem.more_move_p75_pct))) : "");
    /* eslint-disable-next-line */
  }, [setupKey]);
  const run = async () => {
    const t = parseFloat(tgt), s = parseFloat(stp);
    if (!(t > 0) || !(s > 0)) return;
    setBusy(true);
    try {
  ...
  return (
    <div className="rev-whatif" title={REV_TIP.whatif}>
      <span className="rev-whatif-lbl">What if — {long ? "buy here" : "short here"}:</span>
      <label>target <input value={tgt} onChange={e => setTgt(e.target.value)} inputMode="decimal" />%</label>
      <label>stop <input value={stp} onChange={e => setStp(e.target.value)} inputMode="decimal" />%</label>
      <button className="rr-btn" disabled={busy} onClick={run}
        ...
        {busy ? "racing…" : "Run the race"}</button>
      {wi && wi.ok && (
        <span className="rev-whatif-res"
          ...
          target first <b className="up">{wi.p_target_first_pct}%</b> · stop first <b className="down">{wi.p_stop_first_pct}%</b>
          {wi.p_ambiguous_pct > 0 && <> · ambiguous {wi.p_ambiguous_pct}%</>}
          {wi.p_neither_pct > 0 && <> · neither {wi.p_neither_pct}%</>}
          <small className="muted"> · n={wi.n}{wi.median_days_to_target != null ? ` · ~${wi.median_days_to_target}d to target` : ""}</small>
          {wi.gapped_entries > 0 && (
            <small className="muted" title={REV_TIP.gap}> · {wi.gapped_entries} filled at the open after a gap</small>
          )}
          {wi.excluded_contaminated > 0 && (
            <small className="muted" title="Episodes whose follow-on leg contains a split or an incredible single-day print are dropped rather than raced through prices the engine does not trust.">
              {" "}· {wi.excluded_contaminated} dropped
            </small>
          )}
        </span>
      )}
      {wi && !wi.ok && wi.reason && <span className="rev-whatif-res muted">{wi.reason}</span>}
    </div>
  );
}

// The five reversal states, in the order the scan ranks them. Tone is the
// only styling input — no state implies a recommendation.
const REV_STATUS_TONE = {
  "BOUNCING OFF ZONE": "up", "FADING OFF ZONE": "down",
  "IN ZONE": "warn", "APPROACHING": "muted",
  "BEYOND TYPICAL ZONE": "warn", "BEYOND HISTORY": "warn",
};

function SwingReversalBlock({ data }) {
  const rv = data && data.reversal;
  if (!rv) return null;
  if (!rv.ok) {
    return (
      <div className="rev-block" title={REV_TIP.block}>
        <div className="rev-summary muted">Reversal projection unavailable: {rv.reason || "not enough history"}.</div>
      </div>
    );
  }
  const down = rv.direction === "down";
  const c = rv.current, z = rv.zone, rm = rv.remaining, nx = rv.next, co = rv.cohort;
  const st = rv.status || {}, mat = rv.maturity || {}, tz = rv.typical_zone;
  ...
  return (
    <div className="rev-block" title={REV_TIP.block}>
      {st.code && (
        <div className={`rev-status tone-${REV_STATUS_TONE[st.code] || "muted"}`}
          title={REV_TIP.status + (st.meaning ? " — " + st.meaning : "")}>
          <span className="rev-status-code">{st.code}</span>
          <span className="rev-status-why">{st.meaning}</span>
        </div>
      )}
      <div className="rev-summary">{rv.summary}</div>
      {(rv.flags || []).length > 0 && (
        <div className="rev-flags" title={REV_TIP.flags}>
          {rv.flags.map((f, i) => <span key={i} className="rev-flag">⚑ {f}</span>)}
        </div>
      )}
      <div className="rev-grid">
        <div className="rev-box" title={REV_TIP.current}>
          <div className="rev-box-title">Current {dirWord}</div>
          <RevStat label={down ? "Down from the swing high" : "Up from the swing low"} tone={down ? "down" : "up"}
            tip={`From the ${fmtLongDate(c.from_date)} swing ${down ? "high" : "low"} at ${usd(c.from_price)} to the current price of ${usd(c.price)}. The deepest point so far was ${pctf(c.extreme_abs_pct)} on ${fmtLongDate(c.extreme_date)} — that extreme is what the comparison set is conditioned on.`}>
            {pctf(Math.abs(c.pct))} <small className="muted">· {c.days} day{c.days === 1 ? "" : "s"} · {usd(c.price)}</small>
          </RevStat>
          <RevStat label={down ? "Deepest so far (running low)" : "Highest so far (running high)"}
            tip={REV_TIP.extreme} tone={down ? "down" : "up"}>
            {pctf(c.extreme_abs_pct)}
            <small className="muted"> · {usd(c.extreme_price)} on {fmtLongDate(c.extreme_date)}</small>
          </RevStat>
          <RevStat label={down ? "Back off that low" : "Back off that high"} tip={REV_TIP.extreme}
            tone={c.off_extreme_pct ? (down ? "up" : "down") : ""}>
            {c.off_extreme_pct == null ? "—" : (c.off_extreme_pct === 0 ? "at the extreme" : `${down ? "+" : "−"}${c.off_extreme_pct}%`)}
            {c.off_extreme_dollars ? <small className="muted"> · {usd(c.off_extreme_dollars)}</small> : null}
          </RevStat>
          {mat.code && (
            <RevStat label="Stage of the move" tip={REV_TIP.maturity + (mat.note ? " — " + mat.note : "")}
              tone={mat.code === "EARLY IN THE MOVE" ? "muted" : ""}>
              {mat.code === "EARLY IN THE MOVE" ? "Early" : mat.code === "AT ITS NORMAL SIZE" ? "Normal size" : mat.code === "BEYOND ITS NORMAL SIZE" ? "Beyond normal" : "—"}
              <small className="muted"> · {mat.ratio_pct == null ? "—" : `${mat.ratio_pct}% of a typical ${dirWord}`}</small>
            </RevStat>
          )}
          <RevStat label="Completion vs median" tip={REV_TIP.completion}>
            {c.move_completion_pct == null ? "—" : `${c.move_completion_pct}% move`}
            <small className="muted"> · {c.time_completion_pct == null ? "—" : `${c.time_completion_pct}% time`}</small>
          </RevStat>
          <RevStat label="Comparable swings" tip={REV_TIP.cohort + (co.floor_note ? " — " + co.floor_note : "")}
            tone={co.insufficient ? "warn" : ""}>
            n = {co.n}
            <small className="muted"> · of {co.n_direction} {dirWord}s · deeper than {co.share_of_history_already_exceeded_pct}% of them</small>
          </RevStat>
        </div>

        <div className="rev-box" title={REV_TIP.zone}>
          <div className="rev-box-title">Historical {revWord}{co.insufficient ? " · thin sample" : ""}</div>
          {tz && (
            <RevStat label={`Where its ${dirWord}s usually end`} tip={REV_TIP.typical}>
              {usd(tz.band_low_price)} – {usd(tz.band_high_price)}
              <small className="muted"> · {pctf(tz.p25_abs_pct)}–{pctf(tz.p75_abs_pct)} · all {tz.n} of them</small>
            </RevStat>
          )}
          {!z ? (
            <div className="rev-empty">{(co && co.note) || "No comparable swing has gone this far — the stock is beyond its own history."}</div>
          ) : (
            <>
              <RevStat label={`Given it got this far, typical ${dirWord}`} tip={REV_TIP.zone}>
                {pctf(z.p25_abs_pct)}–{pctf(z.p75_abs_pct)}
                <small className="muted"> · median {pctf(z.median_abs_pct)}</small>
              </RevStat>
              <RevStat label={`Median ${revWord} price`} tip={REV_TIP.zone} tone={down ? "down" : "up"}>
                {usd(z.median_price)}
              </RevStat>
              <RevStat label={`Typical ${revWord} zone`} tip={REV_TIP.zone}>
                {usd(z.band_low_price)} – {usd(z.band_high_price)}
              </RevStat>
              {rm && (
                <RevStat label={`Remaining to median ${revWord}`} tip={REV_TIP.remaining}
                  tone={down ? "down" : "up"}>
                  {sgnp(rm.to_median_pct)} <small className="muted">({usd(Math.abs(rm.to_median_dollars))}{rm.days_median != null ? ` · ~${rm.days_median} day${rm.days_median === 1 ? "" : "s"}` : ""})</small>
                </RevStat>
              )}
              {co.share_of_history_already_exceeded_pct >= 75 && (
                <div className="rev-note warn" title={REV_TIP.beyond}>
                  Deeper than {co.share_of_history_already_exceeded_pct}% of its own history — the zone ahead rests on the few swings that went further.
                </div>
              )}
              {rm && (rm.reversed_within || []).length > 0 && (
                <div className="rev-ladder" title={REV_TIP.reversedWithin}>
                  <span>{down ? "Bottomed" : "Topped"} within another…</span>
                  {rm.reversed_within.map(l => (
                    <em key={l.within_more_pct}>{l.within_more_pct}%: <b>{l.rate_pct}%</b><small> ({l.wilson_lo_pct})</small></em>
                  ))}
                </div>
              )}
              {co.earnings_note && (
                <div className="rev-note" title={REV_TIP.earnA}>{co.earnings_note}</div>
              )}
            </>
          )}
        </div>

        <div className="rev-box" title={REV_TIP.next}>
          <div className="rev-box-title">After the {revWord}</div>
          {!nx ? (
            <div className="rev-empty">Not enough completed follow-on swings to describe.</div>
          ) : (
            <>
              <RevStat label={`Median ${oppWord}`} tip={REV_TIP.next} tone={down ? "up" : "down"}>
                {sgnp(down ? nx.pct_median : -nx.pct_median)}
                <small className="muted"> · typically {pctf(nx.pct_p25)}–{pctf(nx.pct_p75)}</small>
              </RevStat>
              <RevStat label={`${oppWord[0].toUpperCase()}${oppWord.slice(1)} target`}
                tip={nx.target_basis + " " + REV_TIP.paired}
                tone={down ? "up" : "down"}>
                {usd(nx.target_median_price)}
                <small className="muted"> · zone {usd(Math.min(nx.target_p25_price, nx.target_p75_price))}–{usd(Math.max(nx.target_p25_price, nx.target_p75_price))}{nx.target_is_paired ? ` · ${(nx.paired || {}).n} paired episodes` : ""}</small>
              </RevStat>
              <RevStat label="Typical duration" tip={REV_TIP.next}>
                {nx.days_median == null ? "—" : `${nx.days_median} days`}
                <small className="muted"> · {nx.days_p25}–{nx.days_p75}</small>
              </RevStat>
              {(nx.horizon_touch || []).length > 0 && (
                <div className="rev-ladder rev-ladder-wide" title={REV_TIP.horizon + " " + (nx.horizon_note || "")}>
                  <span>After the turn, {down ? "rose" : "fell"}…</span>
                  {nx.horizon_touch.map(t => (
                    <em key={`${t.pct}-${t.days}`}>
                      {down ? "+" : "−"}{t.pct}% in {t.days}d: <b>{t.rate_pct}%</b>
                      <small> ({t.wilson_lo_pct}){t.baseline_pct != null ? ` vs ${t.baseline_pct}% any day · ${t.edge_pp >= 0 ? "+" : ""}${t.edge_pp}pp` : ""}</small>
                    </em>
                  ))}
                </div>
              )}
              {nx.reclaim_20d && nx.reclaim_20d.rate_pct != null && (
                <RevStat label={down ? "Reclaimed the 20-day high" : "Revisited the 20-day low"} tip={REV_TIP.reclaim}>
                  {nx.reclaim_20d.rate_pct}%
                  <small className="muted"> ({nx.reclaim_20d.wilson_lo_pct}) · n={nx.reclaim_20d.n}</small>
                </RevStat>
              )}
              {(nx.earnings_note || nx.earnings_included_note) && (
                <div className="rev-note" title={REV_TIP.earnA}>
                  {nx.earnings_note || nx.earnings_included_note}
                </div>
              )}
            </>
...
function SwingReversalScan({ apiFetch }) {
  ...
  // breaks the tie. Nothing is weighted or summed.
  const STATUS_RANK = {
    "BOUNCING OFF ZONE": 0, "FADING OFF ZONE": 0, "IN ZONE": 1,
    "APPROACHING": 2, "BEYOND TYPICAL ZONE": 3, "BEYOND HISTORY": 4,
  };
  const val = (r, k) => ({
    sym: r.symbol,
    px: r.price == null ? -1 : r.price,
    ext: r.rz_extreme_price == null ? -1 : r.rz_extreme_price,
    cur: Math.abs(r.rz_cur_pct != null ? r.rz_cur_pct : (r.swing_pct || 0)),
    ...
    n: r.rz_n || 0, earn: r.days_to_earnings == null ? 999 : r.days_to_earnings,
  })[k];
  // Default ordering is a comparator chain over three named fields, in
  // order of precedence — never a weighted blend of them, which would let
  // a big distance quietly outvote the status the row is grouped by.
  const byStatus = (a, b) => {
    const ra = STATUS_RANK[a.rz_status] == null ? 5 : STATUS_RANK[a.rz_status];
    const rb = STATUS_RANK[b.rz_status] == null ? 5 : STATUS_RANK[b.rz_status];
    if (ra !== rb) return ra - rb;
    const ta = (a.rz_n || 0) >= 6 ? 0 : 1, tb = (b.rz_n || 0) >= 6 ? 0 : 1;
    if (ta !== tb) return ta - tb;
    return val(a, "dist") - val(b, "dist");
  };
  const sorted = pick.slice().sort((a, b) => {
    if (sortK === "status") return byStatus(a, b) * sortD;
    const x = val(a, sortK), y = val(b, sortK);
    return (x < y ? -1 : x > y ? 1 : 0) * sortD;
  });
  const TH = ({ k, children, tip }) => (
    <th className="scan-th-num" title={tip + " Click to sort."}
      onClick={() => { if (sortK === k) setSortD(-sortD); else { setSortK(k); setSortD(1); } }}
      style={{ cursor: "pointer" }}>
      {children}{sortK === k ? (sortD === 1 ? " ▲" : " ▼") : ""}
    </th>
  );
  return (
    <div className="rev-scan">
      <button className="swing-chart-toggle" onClick={() => setOpen(o => !o)}
        title={REV_TIP.scan}>
        {open ? "▾" : "▸"} Reversal scan — watchlist names near their historical zones
      </button>
      {open && (
        <>
          <div className="rev-scan-modes">
            <button className={`rr-btn ${mode === "bounce" ? "pd-f-on" : ""}`} onClick={() => setMode("bounce")}
              title="Stocks in active DOWN swings, placed against their own historical bottoms — long candidates.">
              Bounce candidates ({(rows || []).filter(r => r.rz_median_price != null && r.swing_dir === "short").length})</button>
          ...
          {rows !== null && sorted.length === 0 && (
            <div className="rev-empty">No scanned watchlist name is in an active {mode === "bounce" ? "down" : "up"} swing with a computable zone. Run a watchlist scan first — the board this reads is the one the Watchlist tab fills.</div>
          )}
          {sorted.length > 0 && (
            <div className="scan-table-wrap">
              <table className="scan-table rev-scan-table">
                <thead><tr>
                  <TH k="sym" tip="Ticker.">Symbol</TH>
                  <TH k="status" tip={REV_TIP.scanStatus}>Reversal status</TH>
                  <TH k="px" tip="Last scanned price.">Price</TH>
                  <TH k="ext" tip={`The running ${mode === "bounce" ? "low" : "high"} of the active swing — the deepest point it has reached, which is what the comparison set is conditioned on.`}>{mode === "bounce" ? "Running low" : "Running high"}</TH>
                  <TH k="cur" tip={`Current ${mode === "bounce" ? "decline" : "rally"} measured to the CURRENT price.`}>Now</TH>
                  ...
                  <TH k="left" tip="Median remaining trading days, from where comparable historical swings stood at this depth.">Days left</TH>
                  <TH k="next" tip={`Median ${mode === "bounce" ? "rebound" : "pullback"} after comparable reversals.`}>{mode === "bounce" ? "Rebound" : "Pullback"}</TH>
                  <TH k="tgt" tip="The follow-on target: the median of one projection per comparable episode, each pairing that episode's own depth with its own follow-on move.">Target</TH>
                  <TH k="n" tip="Comparable historical swings behind the numbers (5-year scan frame — the card reads 10 years).">n</TH>
                  <TH k="earn" tip="Calendar days to the next known earnings report. Blank = none on file. The scan does not earnings-filter its history; the card does.">Earn</TH>
                </tr></thead>
                <tbody>
                  {sorted.slice(0, 20).map(r => (
                    <tr key={r.symbol} className={r.rz_extreme_in_zone ? "rev-inzone" : ""}
                      title={(r.rz_status ? r.rz_status + ". " : "") + (r.rz_share_exceeded >= 75 ? "Deeper than most of its own history — the zone rests on the few swings that went further." : "")}>
                      <td><b>{r.symbol}</b>{r.rz_zone_touched ? " ●" : r.rz_share_exceeded >= 75 ? " ⚠" : ""}</td>
                      <td className={`rev-scan-st tone-${REV_STATUS_TONE[r.rz_status] || "muted"}`}>{r.rz_status || "—"}</td>
                      <td className="scan-num">{r.price == null ? "—" : `$${Number(r.price).toFixed(2)}`}</td>
                      <td className="scan-num">{r.rz_extreme_price == null ? "—" : `$${Number(r.rz_extreme_price).toFixed(2)}`}</td>
                      <td className="scan-num">{Math.abs(r.rz_cur_pct != null ? r.rz_cur_pct : (r.swing_pct || 0)).toFixed(1)}%</td>
                      ...
                      <td className="scan-num">{r.rz_more_move_pct == null ? "—" : `${Number(r.rz_more_move_pct).toFixed(1)}%`}</td>
                      <td className="scan-num">{r.rz_days_median == null ? "—" : `~${r.rz_days_median}`}</td>
                      <td className="scan-num">{r.rz_next_pct == null ? "—" : `${mode === "bounce" ? "+" : "−"}${r.rz_next_pct}%`}</td>
                      <td className="scan-num">{r.rz_next_target == null ? "—" : `$${Number(r.rz_next_target).toFixed(2)}`}</td>
                      <td className="scan-num muted">{r.rz_n || "—"}</td>
                      <td className="scan-num">{r.days_to_earnings != null && r.days_to_earnings >= 0 && r.days_to_earnings <= 14 ? `⚠ ${r.days_to_earnings}d` : ""}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function SwingPatternCard({ apiFetch, ticker }) {
  ...
  return (
    <div className="card ab-card" ref={cardRef}>
      ...
      {/* ── Reversal projection (v4.50) — the empirical read leads ──────── */}
      {data && <SwingReversalBlock data={data} />}

      {/* ── The chart, immediately under the projection (v4.51) ──────────
          Jerry's workflow is: read the projection, look at the chart,
          inspect the historical swings. The chart used to sit below the
          decision banner, the odds table, the target ladder and the trade
          plan; all of that is now behind "More details" below. Nothing was
          deleted — it stopped leading.                                   */}
      ...
      {/* ── What if I took this trade right here ─────────────────────── */}
      {data && data.reversal && data.reversal.ok && (
        <SwingWhatIf apiFetch={apiFetch} ticker={ticker} sens={sens}
                     reversal={data.reversal} onResult={setData} />
      )}

      {/* ── Everything below is the older analytical layer, collapsed ──── */}
      {a && (
        <button type="button" className="swing-chart-toggle" onClick={() => setMore(m => !m)}
          title="The decision banner, continuation and exhaustion scores, odds, target ladder and trade plan. All still computed — they simply no longer sit between the projection and the chart.">
          {more ? "▾" : "▸"} More details — decision engine, scores, odds, target ladder, trade plan
        </button>
      )}
      {more && a && (rv0 => (
        <div className="rev-context" title="Two readings the reversal projection deliberately does NOT use: both were tested as ways to narrow the comparable-swing set and both were rejected for failing to improve out-of-sample accuracy. Shown as context because they describe where price sits, not because they predict anything.">
          <span title={REV_TIP.rangepos}>20-day range position: <b>{rv0 && rv0.current && rv0.current.range_pos_20 != null ? `${rv0.current.range_pos_20} / 100` : "—"}</b></span>
          <span title={REV_TIP.regime}>Trend regime: <b>{rv0 && rv0.current ? (rv0.current.regime === "uptrend" ? "Uptrend" : rv0.current.regime === "downtrend" ? "Downtrend" : "Unknown") : "—"}</b> <small className="muted">vs 200-day</small></span>
          <span title="Both were rejected by walk-forward testing as cohort filters — kept visible, kept out of the arithmetic.">Neither narrows the comparison set.</span>
        </div>
      ))(data && data.reversal)}
      ...
      {/* ── Live decision box ───────────────────────────────────────────── */}
      {more && a && (a.status === "ok" || a.status === "no_rhythm") && (
      ...
      {/* ── Trade plan ──────────────────────────────────────────────────── */}
      {more && a && a.status === "ok" && a.trade_plan && (
      ...
      <SwingReversalScan apiFetch={apiFetch} />
    </div>
  );
}
...
function TabBar({ active, onChange, ticker, earnDate, earnDays, tabs, onReorder, apiFetch }) {
  ...
  useEffect(() => { setOpenGroup(activeGroup); }, [active]);
...
function CRSeriesSvg({ series }) {
  ...
  return (
    <div className="cr-chart" title={`Merton model 5Y credit spread, daily, ${series[0].date} → ${series[series.length - 1].date}. Range ${Math.round(lo)}–${Math.round(hi)} bps. A STRUCTURAL MODEL from market cap, equity vol and balance-sheet debt — trends with real CDS, but is not a traded quote.`}>
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
        <polyline className={`cr-line ${widening ? "down" : "up"}`}
                  points={series.map((p, i) => `${x(i).toFixed(1)},${y(p.spread_bps).toFixed(1)}`).join(" ")} />
      </svg>
      <div className="cr-chart-lbls num">
        <span>{series[0].date}</span>
        <span>{Math.round(lo)}–{Math.round(hi)} bps</span>
...
function CreditRiskCard({ apiFetch, ticker }) {
  const [d, setD] = useState(null);
  const [err, setErr] = useState(null);
  useEffect(() => { setD(null); setErr(null); }, [ticker]);
  useEffect(() => {
    let stop = false;
    const load = () => {
      sharedJson(apiFetch, `/api/credit_risk?symbol=${encodeURIComponent(ticker)}`, 5 * 60 * 1000)
        .then(x => { if (!stop) { if (x && !x.error) setD(x); else setErr((x && x.error) || "no data"); } })
  ...
  if (err && !d) return (
    <div className="card cr-card"><div className="card-head"><div>
      <div className="kicker">structural credit · no Bloomberg required</div>
      <div className="card-title">Credit Risk</div></div></div>
      <div className="tsy-sigd">Unavailable: {err}</div></div>
  );
  if (!d) return null;
  const m = d.merton || {};
  ...
  return (
    <div className="card cr-card">
      <div className="card-head">
        <div>
          <div className="kicker" title={(d.notes || []).join("\n\n")}>
            structural credit · single-name CDS is paid dealer data — this is the honest free version
          </div>
          <div className="card-title">Credit Risk — model 5Y spread</div>
        </div>
        {latest && (
          <div className="cr-big num" title={`Merton structural estimate. Distance-to-default ${latest.dd ?? "—"}σ · risk-neutral 5Y default probability ${latest.pd_pct}% · leverage (default point ÷ assets) ${latest.leverage_pct}%.`}>
            {Math.round(latest.spread_bps)} <em>bps</em>
          </div>
        )}
      </div>

      {m.interpretation && <div className="cr-interp">{m.interpretation}</div>}
      {m.unavailable_reason
        ? <div className="tsy-sigd">Model series unavailable: {m.unavailable_reason}.</div>
        : <CRSeriesSvg series={m.series} />}

      {latest && (
        <div className="cr-stats">
          <span title="Distance to default: how many standard deviations of asset value sit between the firm and its default point over 5 years. Higher = safer.">DD <b className="num">{latest.dd ?? "—"}σ</b></span>
          <span title="Risk-neutral probability of hitting the default point within 5 years — the model's raw output; real-world PDs are lower.">PD₅ᵧ <b className="num">{latest.pd_pct}%</b></span>
          <span title={m.inputs ? `Default point ${fmt$M(m.inputs.default_point)} (${m.inputs.default_point_basis || "—"}) vs total debt ${fmt$M(m.inputs.debt_total)}. Risk-free: ${m.inputs.risk_free ? m.inputs.risk_free.source : "—"}. Equity vol: ${m.inputs ? m.inputs.equity_vol : ""}.` : ""}>leverage <b className="num">{latest.leverage_pct}%</b></span>
        </div>
      )}

      {skew && (
        <div className="cr-skew" title={skew.note}>
          <em>LIVE CREDIT FEAR (options market · {skew.expiry}, {skew.dte}d)</em>
          {skew.rr25_vol_pts != null && (
            <span title={`25Δ risk reversal: OTM put IV ${skew.put25_iv}% − OTM call IV ${skew.call25_iv}%. Rising = downside protection getting bid — the same flow that widens CDS.`}>
              25Δ skew <b className="num">{skew.rr25_vol_pts} pts</b>
            </span>
          )}
          {skew.crash_put && (
            <span title={`The ${skew.crash_put.strike} put (${skew.crash_put.otm_pct}% OTM) costs ${skew.crash_put.cost_pct_of_spot}% of spot — ${skew.crash_put.annualized_pct}%/year to insure a >20% collapse. This is the equity market's closest listed analogue to paying CDS premium.`}>
              crash insurance <b className="num">{skew.crash_put.annualized_pct}%/yr</b>
            </span>
          )}
        </div>
      )}
      ...
      {mk && (mk.ig || mk.hy) && (
        <div className="cr-mkt" title={`${mk.source}. The sector backdrop: if IG/HY are widening too, it's a credit-market story; if only this name's model spread moves, it's idiosyncratic.`}>
          <em>REAL TRADED CREDIT (ICE BofA OAS · FRED)</em>
          {["ig", "bbb", "hy"].map(k => mk[k] && (
            <span key={k} className={mk[k].chg_1m_bps > 0 ? "warn" : ""}>
              {mk[k].label.replace("US ", "").replace(" OAS", "")} <b className="num">{mk[k].bps}</b>
              <i className="num">{fmtChg(mk[k].chg_1m_bps)}</i>
            </span>
          ))}
        </div>
      )}
      <div className="cr-note">Model + options-market gauges — not a traded CDS quote (that data is dealer/paid only). Hover anything for the full math.</div>
    </div>
  );
}
...
function SWSTPanel({ ticker, onSwitchTicker, inWatchlist, onAddWatchlist,
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      if (v >= SWS_NEED_VER) { setHelperVer(v); clearInterval(t); }
  ...
  const resolve = React.useCallback((sym, bust) => {
    ...
    return sharedJson(apiFetch,
      ...
      12 * 3600 * 1000)
      .then(d => {
        if (d && d.url) { setSrc(d.url); setSrcSym(sym); return d.url; }
        setResolveErr((d && d.reason) || "no page found for this ticker");
        return null;
      })
      .catch(e => { setResolveErr(String(e)); return null; })
  ...
  useEffect(() => {
    ...
    resolve(ticker).then(url => {
      if (!url && !cancelled) setTimeout(() => { if (!cancelled) resolve(ticker, Date.now()); }, 1200);
    });
    return () => { cancelled = true; };
  ...
  return (
    <div className="card fv-card fv-live" style={{ marginBottom: "var(--row-gap)" }}>
      <div className="fv-toolbar" style={{ marginTop: 0 }}>
        <span className="fv-now" title="The Simply Wall St company page follows the dashboard's globally selected ticker — valuation, future growth, past performance, financial health and dividend for the symbol you're working. Navigating to another company inside Simply Wall St drives the app's ticker back.">{ticker}</span>
        {inWatchlist ? (
          <span className="fv-star on fv-star-static"
                title={`${ticker} is on your JerryTrade watchlist. Not a button — removal only happens in Manage.`}>
            ★ on watchlist
      ...
      <div className="fv-toolbar fv-row2">
        <button className={`pj-toggle ${follow ? "active" : ""}`}
                onClick={() => { SWST.setFollow(!follow); setFollow(!follow); }}
        ...
        <button className="rr-btn" onClick={() => resolve(ticker, Date.now())}
        ...
        {[["Stocks", "/stocks", "Simply Wall St's stock screener and browse pages."],
          ...
          ["Watchlist", "/user/watchlist", "Your Simply Wall St watchlist (requires login inside the frame)."],
          ["Portfolio", "/user/portfolio", "Your Simply Wall St portfolio (requires login inside the frame)."]].map(([l, p, tip]) => (
      ...
      {resolving && (!src || stale) && (
        <div className="fv-hint" role="status">Resolving the Simply Wall St page for {ticker}…</div>
      )}
      {resolveErr && (!src || stale) && (
        <div className="sws-err" role="alert">
          Couldn't load the Simply Wall St page for <b>{ticker}</b> — {resolveErr}.
          <button className="rr-btn" onClick={() => resolve(ticker, Date.now())}>Retry</button>
          <button className="rr-btn" onClick={() => { setSrc(SWST.home()); setSrcSym(ticker); }}>Browse stocks instead</button>
        </div>
      )}
      {stale && !resolving && !resolveErr && (
        <div className="sws-err" role="status">
          Showing <b>{srcSym}</b> — still switching to <b>{ticker}</b>.
            ...
            <React.Fragment>
              <div className="sws-diag-verdict">
                {(() => {
                  ...
                  const a = diag.cookieAudit;
                  // The cookie jar is decisive and needs no normal tab, so it
                  // is read FIRST — the earlier flow asked the user to stage a
                  ...
                  if (a && a.ok) {
                    ...
                    if (a.crossSiteReady === 0) {
                      return <React.Fragment>
                        <b>A login cookie exists but can't be sent from a frame.</b>
                        {" "}{a.authish.map(c => c.name).slice(0, 4).join(", ")} — none is
                        {" "}<code>SameSite=None</code>, so the browser withholds it here. The helper
                        rewrites these on sign-in; sign in again inside the frame and re-check.
                      </React.Fragment>;
                    }
                    if (a.authish.some(c => c.partitioned)) {
                      return <React.Fragment>
                        <b>The login cookie is partitioned.</b> The frame holds its own isolated copy
                        rather than your real session, so it reads as signed out.
                      </React.Fragment>;
                    }
                    return <React.Fragment>
                      <b>The login cookie looks correctly set for framing</b>
                      {" "}({a.crossSiteReady} cross-site-ready). If it still drops, the app is
                  ...
                  if (lsMissing.length && !ckMissing.length) {
                    return <React.Fragment>
                      <b>Partitioned storage.</b> A normal tab has {lsMissing.length} localStorage
                      key{lsMissing.length === 1 ? "" : "s"} this frame cannot see
                      ({lsMissing.slice(0, 4).join(", ")}{lsMissing.length > 4 ? "…" : ""}).
                      Chrome isolates localStorage per top-level site and no extension can cross
                      that boundary — if the login lives there, it cannot be carried into the panel.
                    </React.Fragment>;
                  }
                  if (ckMissing.length) {
                    return <React.Fragment>
                      <b>Cookies are missing in the frame.</b> A normal tab has
                      {" "}{ckMissing.slice(0, 5).join(", ")}{ckMissing.length > 5 ? "…" : ""} and this
                      frame does not — that is a cookie-delivery problem, which the helper can act on.
                    </React.Fragment>;
                  }
                  return <React.Fragment>
                    The frame sees <b>everything a normal tab sees</b>. If the login still drops, it is
                    not storage isolation — most likely the session cookie is <code>httpOnly</code>
                    {" "}(invisible to this check) and is being refused on the request itself.
                  </React.Fragment>;
                })()}
...
Object.assign(window, { TickerLogo, MarketBreadthCard: _memo(MarketBreadthCard),
  ...
  SWSTPanel: _memo(SWSTPanel), HelperDownloadChip,
...
// ── The plan comes from the server's strike engine (weekly_sell.py). It is
// matched to the ticker on screen so a stale plan from the previous symbol
// can never be drawn under this one's price.
function _wosPlan(ticker) {
  try {
    const L = window.__LIVE;
    if (!L || !L.sellPlan || !ticker) return null;
    if (String(L.ticker || "").toUpperCase() !== String(ticker).toUpperCase()) return null;
    return L.sellPlan;
  } catch (e) { return null; }
}

// A strike the engine already measured, looked up by price. Used to show
// Jerry's OWN picked strike on the same footing as the recommendation.
function _wosFind(list, strike) {
  if (!Array.isArray(list) || strike == null) return null;
  for (const r of list) {
    if (r && r.strike != null && Math.abs(r.strike - strike) < 0.001) return r;
  }
  return null;
}
...
function WeeklySellSetupCard({ rows, weeks, ticker, currentPrice, baselinePrice, currReturn,
  ...
  // Day-of-week context: CURRENT is a week in progress measured against
  // COMPLETED weeks, so how much week is left matters.
  ...
  if (plan && plan.dte_cal != null) dte = plan.dte_cal;

  // Expected move to the selected Friday — the engine's band when loaded,
  // else the chain's own ATM straddle mid.
  ...
  let emPct = emUp != null && emDn != null ? ((emUp + emDn) / 2) / currentPrice * 100 : null;
  if (emPct == null && plan && plan.em && plan.em.pct != null) emPct = plan.em.pct;
  ...
  const zone = plan && plan.band ? plan.band.zone : null;
  const typical = plan && plan.band ? plan.band.typical : null;

  function ZoneScale() {
    const lo = zone.low, hi = zone.high;
    const pk = s => plan[s] && plan[s].pick ? plan[s].pick.strike : null;
    const putK = pk("put"), callK = pk("call");
    const ends = [lo.price, hi.price, putK, callK, currentPrice].filter(v => v != null);
    let min = Math.min(...ends), max = Math.max(...ends);
    const pad = Math.max((max - min) * 0.06, currentPrice * 0.004);
    min -= pad; max += pad;
    const at = v => v == null ? null : Math.max(0, Math.min(100, (v - min) / (max - min) * 100));
    const tLo = typical && typical.low ? at(typical.low.price) : null;
    const tHi = typical && typical.high ? at(typical.high.price) : null;
    const nowAt = at(currentPrice);
    const srcWord = s => s === "history" ? "this stock's own history" : "the option market's implied move";
    return (
      <React.Fragment>
        <div className="wos-rl-h" title={`Every number here is measured over the ${plan.sessions} trading session${plan.sessions === 1 ? "" : "s"} between now and this expiry — not a calendar week.`}>
        ...
        <div className="wos-trackwrap">
          ...
          <div className="wos-track wsz-track">
            {tLo != null && tHi != null && (
              <i className="wsz-typical" style={{ left: `${tLo}%`, width: `${Math.max(1, tHi - tLo)}%` }}
                 title={`Half of all matched ${plan.sessions}-session windows closed inside this band.`}></i>
            )}
            {putK != null && <i className="wsz-strike put" style={{ left: `${at(putK)}%` }} title={`Recommended put strike ${fmt$(putK, 2)}`}></i>}
            {callK != null && <i className="wsz-strike call" style={{ left: `${at(callK)}%` }} title={`Recommended call strike ${fmt$(callK, 2)}`}></i>}
            <i className="wos-marker" style={{ left: `${nowAt}%` }}></i>
          </div>
        </div>
        <div className="wos-ends">
          <span className="wos-end lo" title={`Sell puts at or below this level. Over ${plan.sample.n} matched ${plan.sessions}-session windows, this stock closed below it about ${plan.band.target_itm_pct.toFixed(0)}% of the time. The line came from ${srcWord(lo.source)} — the engine keeps whichever of the two is more cautious.`}>
            <em>SELL PUTS BELOW</em><b className="num">{f$(lo.price)}</b>
            <span className="num">{fp(lo.pct)} · {lo.source}</span>
          </span>
          <span className="wos-end hi" title={`Sell calls at or above this level. Over ${plan.sample.n} matched ${plan.sessions}-session windows, this stock closed above it about ${plan.band.target_itm_pct.toFixed(0)}% of the time. The line came from ${srcWord(hi.source)} — the engine keeps whichever of the two is more cautious.`}>
            <em>SELL CALLS ABOVE</em><b className="num">{f$(hi.price)}</b>
            <span className="num">{fp(hi.pct)} · {hi.source}</span>
          </span>
        </div>
        {typical && typical.low && typical.low.price != null && (
          <div className="wos-prox-box"
               title={`The middle half of matched windows: a quarter closed below the low end and a quarter above the high end. Measured exactly like the sell zone, one line looser, so it always sits inside it.`}>
            <em>ORDINARY WEEK</em>
            <b className="num">{fmt$(typical.low.price, 2)}</b>
            <span>to {fmt$(typical.high.price, 2)}</span>
          </div>
        )}
        <div className="wos-posline">
          <span title={`Overlapping windows from ${plan.sample.n + plan.sessions} sessions of daily history. Overlap is why the count is high — every observation the history holds is used, and the count is shown rather than dressed up as independent weeks.`}>
            measured on <b className="num">{plan.sample.n}</b> matching {plan.sessions}-session windows
            {plan.sample.from ? <React.Fragment> since <b className="num">{plan.sample.from}</b></React.Fragment> : null}
          </span>
          {emPct != null && (
            <span title={plan.em && plan.em.source === "straddle" ? "The at-the-money straddle mid on the live chain — what the option market is charging for the move to this expiry." : "Expected move to this expiry."}>
              market&rsquo;s priced move <b className="num">±{emPct.toFixed(1)}%</b>
            </span>
          )}
        </div>
      </React.Fragment>
  ...
  function PickCol({ side }) {
    const S = plan[side] || {};
    const p = S.pick;
    const label = side === "put" ? "SELL PUT" : "SELL CALL";
    const col = side === "put" ? "var(--up)" : "var(--down)";
    const userC = side === "put" ? putC : callC;
    const userK = userC && userC.strike != null ? userC.strike : null;
    const mine = p && userK != null && Math.abs(userK - p.strike) > 0.001
      ? _wosFind(S.all, userK) : null;
    if (!p) {
      return (
        <div className="wos-side">
          <div className="wos-side-h" style={{ color: col }}>{label}</div>
          <div className="wos-na" style={{ padding: "8px 0" }}
               title="The engine scored every strike on the live chain for this expiry and none of them cleared both the sell zone and the fill-quality checks. Not selling is the answer, not a missing number.">
            {S.note || "Nothing worth selling on this side."}
          </div>
        </div>
      );
    }
    const evTone = p.ev_ann_pct == null ? "" : p.ev_ann_pct >= 0 ? "good" : "bad";
    const evTip = side === "put"
      ? `Average result of selling this put across all ${p.sample} matched windows: the credit collected, minus what it cost on the windows that finished below the strike, as a yearly rate on the cash the trade ties up. History, not a forecast.`
      : `Average result of selling this call instead of simply holding the shares, across all ${p.sample} matched windows, as a yearly rate. A negative number means the stock ran through this strike often enough that the premium did not pay for the upside given up.`;
    return (
      <div className="wos-side">
        <div className="wos-side-h" style={{ color: col }}>
          {label}
          <span className="wos-strike num" title={(p.why || []).join(" · ")}>{fmt$(p.strike, p.strike >= 1000 ? 0 : 2)}</span>
        </div>
        {p.relaxed && (
          <div className="wos-relaxed" title={S.note || ""}>best available — see note</div>
        )}
        <Row l="Credit · bid/mid" v={<span className="num">{p.bid != null ? fmt$(p.bid, 2) : "—"} / {fmt$(p.mid, 2)}</span>}
             tip="Live chain bid and mid for this contract." />
        <Row hot l="Breakeven" v={<span className="num">{fmt$(p.breakeven, 2)} · {fp(p.cushion_pct)}</span>}
             tip="Strike ∓ the credit, and how far price is from it. The real cushion." />
        <Row hot l="Assigned odds"
             v={<span className="num">{pct0(p.itm_pct)} hist · {p.p_otm == null ? "—" : pct0(100 - p.p_otm)} mkt</span>}
             tip={`Two independent reads on the one thing that costs money. HIST: out of ${p.sample} matched ${plan.sessions}-session windows in this stock's own daily history, this share FINISHED through the strike. MKT: the option market's own implied odds, from the chain's delta. A strike has to clear ${plan.band.target_itm_pct.toFixed(0)}% on BOTH to be offered here — which is why a strike history likes can still be refused.`} />
        <Row l="Tested" v={<span className="num">{pct0(p.touch_pct)}</span>}
             tip={`How often price merely REACHED the strike at some point before expiry. Always higher than the assigned number — being tested is a scare, not a loss.`} />
        <Row hot l={<React.Fragment>Edge · a year <i className="wos-est">hist</i></React.Fragment>}
             v={<span className="num">{p.ev_ann_pct == null ? "—" : `${p.ev_ann_pct >= 0 ? "+" : ""}${p.ev_ann_pct.toFixed(0)}%`}</span>}
             tone={evTone} tip={evTip} />
        <Row l="Return · week · year" v={<span className="num">{p.rbp_pct.toFixed(2)}% · {p.ann_pct == null ? "—" : p.ann_pct.toFixed(0) + "%"}</span>}
             tip={side === "put" ? "Credit ÷ cash-secured collateral for this week, and the same rate annualized." : "Credit ÷ the share value it covers for this week, and the same rate annualized."} />
        <Row l="Delta" v={<span className="num">{p.delta == null ? "—" : p.delta.toFixed(2)}</span>}
        ...
        {Array.isArray(S.alts) && S.alts.length > 0 && (
          <div className="wos-alts" title="Runners-up on the same scoring. Each one cleared the sell zone and the fill checks too.">
            <em>ALSO</em>
            {S.alts.map(a => (
              <span key={a.strike} className="wos-alt num"
                    title={`${fmt$(a.strike, 2)} · assigned ${pct0(a.itm_pct)} hist / ${a.p_otm == null ? "—" : pct0(100 - a.p_otm)} mkt · ${a.ev_ann_pct == null ? "—" : (a.ev_ann_pct >= 0 ? "+" : "") + a.ev_ann_pct.toFixed(0) + "%/yr"} · fill ${a.liq_grade}`}>
                {fmt$(a.strike, a.strike >= 1000 ? 0 : 2)}
              </span>
            ))}
          </div>
        )}
        {mine && (
          <div className={`wos-mine ${mine.gate ? "warn" : ""}`}
               title={mine.gate
                 ? `Your picked strike, measured exactly the same way. It is ${mine.gate} — the engine would not sell it here.`
                 : "Your picked strike, measured exactly the same way. It clears the zone too; the engine simply scored another strike higher."}>
            <em>YOUR PICK</em>
            <b className="num">{fmt$(mine.strike, mine.strike >= 1000 ? 0 : 2)}</b>
            <span>assigned {pct0(mine.itm_pct)} hist · {mine.p_otm == null ? "—" : pct0(100 - mine.p_otm)} mkt{mine.ev_ann_pct != null ? ` · ${mine.ev_ann_pct >= 0 ? "+" : ""}${mine.ev_ann_pct.toFixed(0)}%/yr` : ""}{mine.gate ? ` — ${mine.gate}` : ""}</span>
          </div>
        )}
      </div>
    );
  ...
  // ══ FALLBACK VIEW ═══════════════════════════════════════════════════════
  // No plan (an old cached payload, or the engine could not measure this
  // symbol). The panel keeps working on the range numbers it always had,
  // and says plainly that the recommendation is missing.
  function LegacyRange() {
    ...
    return (
      <React.Fragment>
        ...
        <div className="wos-ends">
          <span className="wos-end lo" title={`Worst weekly low of the ${rows.length} displayed weeks.`}>
            <em>WORST LOW</em><b className="num">{fp(worstLow)}</b><span className="num">{f$(pLow)}</span>
          </span>
          <span className="wos-end hi" title={`Best weekly high of the ${rows.length} displayed weeks.`}>
            <em>BEST HIGH</em><b className="num">{fp(bestHigh)}</b><span className="num">{f$(pHigh)}</span>
          </span>
        </div>
        <div className="wos-prox-box" title="Where this week's return sits inside the displayed range. A location measure only.">
        ...
        <div className="wos-posline">
          <span><b className="num cd">{f$(Math.abs(dLow$))}</b> · {Math.abs(dLowPts).toFixed(1)}% above the worst low
            &nbsp;·&nbsp; <b className="num cu">{f$(Math.abs(dHigh$))}</b> · {Math.abs(dHighPts).toFixed(1)}% below the best high</span>
        </div>
      </React.Fragment>
    );
  }

  const verdict = plan && plan.verdict ? plan.verdict : null;
  const vTone = !verdict ? "mut"
    : verdict.side === "put" ? "up" : verdict.side === "call" ? "down" : "mut";

  return (
    <div className="card wos-card">
      <div className="card-head">
        <div>
          <div className="kicker" title={plan
            ? `Every strike on the live chain for this expiry, scored on what this stock actually did over the last ${plan.sample ? plan.sample.n : 0} matching ${plan.sessions}-session windows and on whether the strike can be filled. Not a fixed delta.`
            : `Where this week sits inside the last ${rows.length} weeks' range.`}>
            {plan ? "Which strike is worth selling" : "Weekly option selling setup"}
          </div>
          <div className="card-title">{plan
            ? "Sell where history and the option market both say it is safe"
            : "Sell puts near the lows · calls near the highs"}</div>
        </div>
        {dte != null && expiration && (
          <span className="wos-dte" title={`Selected weekly expiration ${expiration} — ${dte} days away${plan ? `, ${plan.sessions} trading session${plan.sessions === 1 ? "" : "s"} of risk` : ""}.`}>
            EXP FRI {expiration.slice(5).replace("-", "/")} · <b>{dte}d</b>
          </span>
        )}
        {plan && plan.earnings && plan.earnings.inside && (
          <span className="wos-earn-warn" title={`Earnings are due ${plan.earnings.date}, inside this expiry. The history behind these numbers is mostly ordinary weeks, so it understates the risk here.`}>
            EARNINGS INSIDE
          </span>
        )}
        <span className={`wos-bias ${vTone}`}
              title={plan
                ? "Leads with expected value measured on this stock's own history, then assignment odds, fill quality and the cushion against the option market's own priced move. A read on where the pay is, not an instruction."
                : "The engine's plan is not available for this symbol right now, so the panel is showing range location only."}>
          {verdict ? verdict.line : "Range location only"}
        </span>
      </div>

      <div className="wos-range">
        {plan && zone ? <ZoneScale /> : <LegacyRange />}
        {lowsInBy != null && (
          <div className="wos-dayctx"
               title={`CURRENT is this week IN PROGRESS measured against COMPLETED weeks' full Mon–Fri extremes. In the displayed ${withDays.length} weeks, the weekly LOW had already printed by ${DAY_NAMES[dow]} in ${lowsInBy.toFixed(0)}% of them, the HIGH in ${highsInBy.toFixed(0)}%. Late in the week and near the range low means historically little room left below.`}>
            <em>{DAY_NAMES[dow].toUpperCase()}{dow === 4 ? " · WEEK NEARLY COMPLETE" : ""}</em>
            <span>low already in: <b className={`num ${lowsInBy >= 70 ? "cu" : ""}`}>{lowsInBy.toFixed(0)}%</b>
              {"  ·  "}high already in: <b className="num">{highsInBy.toFixed(0)}%</b></span>
          </div>
        )}
        {(() => {
          // The whole panel in one sentence. Hovering a tooltip is fine for
          // detail, but the headline answer should be readable without it.
          if (!plan || !verdict || verdict.side === "none") return null;
          const lead = verdict.side === "call" ? "call" : "put";
          const S = plan[lead] || {};
          const p = S.pick;
          if (!p) return null;
          const word = lead === "put" ? "below" : "above";
          const mkt = p.p_otm == null ? null : 100 - p.p_otm;
          return (
            <div className="wos-plain" title="The same numbers as the column on the right, said once in plain words.">
              Sell the <b>{fmt$(p.strike, p.strike >= 1000 ? 0 : 2)} {lead}</b> for about <b>{fmt$(p.mid, 2)}</b>.
              {" "}Over <b>{p.sample}</b> matching {plan.sessions}-session stretches {ticker} closed {word} it{" "}
              <b>{pct0(p.itm_pct)}</b> of the time{mkt != null ? <React.Fragment>, and the option market puts it at <b>{pct0(mkt)}</b></React.Fragment> : null}.
              {p.ev_ann_pct != null && (
                <React.Fragment>{" "}Counting the times it went wrong, that trade averaged{" "}
                  <b className={p.ev_ann_pct >= 0 ? "cu" : "cd"}>{p.ev_ann_pct >= 0 ? "+" : ""}{p.ev_ann_pct.toFixed(0)}% a year</b>
                  {lead === "call" ? " against just holding the shares" : " on the cash it ties up"}.</React.Fragment>
              )}
              {p.relaxed ? " Nothing cleared the usual line this week, so this is the best available." : ""}
              {plan.earnings && plan.earnings.inside ? " Earnings land inside this expiry, so the history behind these odds understates the risk." : ""}
            </div>
          );
        })()}
        {plan && !plan.ok && plan.note && (
          <div className="wos-plan-note" title="The engine ran and found nothing it would sell. That is an answer.">{plan.note}</div>
        )}
      </div>
      ...
      <div className="wos-sides">
        {plan && zone ? (
          ...
          <React.Fragment>
            <LegacySide label="SELL PUT" c={putC} side="put" currentPrice={currentPrice}
                        baselinePrice={baselinePrice} rows={rows} weeks={weeks} dte={dte}
                        emPct={emPct} emUp={emUp} emDn={emDn} emFromBand={emFromBand} />
            <LegacySide label="SELL CALL" c={callC} side="call" currentPrice={currentPrice}
                        baselinePrice={baselinePrice} rows={rows} weeks={weeks} dte={dte}
                        emPct={emPct} emUp={emUp} emDn={emDn} emFromBand={emFromBand} />
          </React.Fragment>
        )}
...
// The pre-engine side column, kept for the fallback path only.
function LegacySide({ label, c, side, currentPrice, baselinePrice, rows, weeks, dte,
                      emPct, emUp, emDn, emFromBand }) {
  const col = side === "put" ? "var(--up)" : "var(--down)";
  const chainOK = o => o && o.strike != null && ((o.bid || 0) > 0 || (o.ask || 0) > 0);
  const midOf = o => (o.bid > 0 && o.ask > 0) ? (o.bid + o.ask) / 2 : (o.bid || o.ask || null);
  const NA = <span className="wos-na">Data unavailable</span>;
  if (!chainOK(c)) {
    return (
      <div className="wos-side">
        <div className="wos-side-h" style={{ color: col }}>{label}</div>
        <div className="wos-na" style={{ padding: "8px 0" }}>Data unavailable</div>
      </div>
    );
  }
  const mid = midOf(c);
  ...
  const rbp = side === "put" ? mid / (c.strike - mid) * 100 : mid / currentPrice * 100;
  const R = ({ l, v, hot }) => (<div className={`wos-r ${hot ? "hot" : ""}`}><em>{l}</em><b>{v}</b></div>);
  return (
    <div className="wos-side">
      <div className="wos-side-h" style={{ color: col }}>{label}
        <span className="wos-strike num">{fmt$(c.strike, 2)}</span>
      </div>
      <R l="Bid / mid" v={<span className="num">{c.bid != null ? fmt$(c.bid, 2) : "—"} / {fmt$(mid, 2)}</span>} />
      <R hot l="Breakeven" v={<span className="num">{fmt$(breakeven, 2)} · {beDistPct >= 0 ? "+" : ""}{beDistPct.toFixed(1)}%</span>} />
      <R l="Delta" v={delta != null ? <span className="num">{delta.toFixed(2)}</span> : NA} />
      <R hot l="P(expire OTM)" v={probOTM != null ? <span className="num">{probOTM.toFixed(0)}% <i className="wos-est">est</i></span> : NA} />
      <R hot l={`Breach rate · ${weeks}w`} v={<span className="num">{(breachN / rows.length * 100).toFixed(0)}% ({breachN}/{rows.length})</span>} />
      <R l="EM to Friday" v={emPct != null ? <span className="num">±{emPct.toFixed(1)}%</span> : NA} />
...
function FridayCard({ onOpenTab }) {
  ...
  const daysOut = (() => {
    ...
    return Math.round((Date.parse(fridayISO + "T00:00:00Z")
    ...
    <div className="card fri-card">
      ...
      <div className="fri-state">
        <div className={`fri-pill ${zeroDte ? "on" : ""}`}>
          ...
          <span className="fri-pill-v">Fri {fridayLabel(fridayISO)}</span>
        </div>
