...
(function () {
  ...
  const STRATEGIES = [
    // 1. Covered Call
    {
      ...
      definedRisk: false,
      build: (ctx) => {
        const { sugCall, callAtSug, callMid, currentPrice, FRONT_DTE } = ctx;
        const legs = [
          { type: "stock", strike: currentPrice, qty: 100, premium: 0, dte: 0, iv: 0 },
          { type: "call", strike: sugCall, qty: -100, premium: callMid, dte: FRONT_DTE, iv: callAtSug.iv },
        ];
        return {
          legs,
          structure: `Sell the $${sugCall.toFixed(2)} call against 100 shares you already own.`,
          stats: [
            ["Premium", `$${callMid.toFixed(2)}/sh`],
            ["If unchanged", `+$${callMid.toFixed(2)}/sh (${(callMid / currentPrice * 100).toFixed(2)}%)`],
            ["If called away", `+$${(sugCall - currentPrice + callMid).toFixed(2)}/sh (${((sugCall + callMid) / currentPrice * 100 - 100).toFixed(2)}%)`],
          ],
    ...
    // 2. Cash-Secured Put
    {
      ...
      definedRisk: false,
      build: (ctx) => {
        const { sugPut, putAtSug, putMid, FRONT_DTE } = ctx;
        const legs = [
          { type: "put", strike: sugPut, qty: -100, premium: putMid, dte: FRONT_DTE, iv: putAtSug.iv },
        ];
        return {
          legs,
          structure: `Sell the $${sugPut.toFixed(2)} put, backed by $${(sugPut * 100).toFixed(0)} cash per contract.`,
          stats: [
            ["Premium", `$${putMid.toFixed(2)}/sh`],
            ["Max profit", `$${putMid.toFixed(2)}/sh (${(putMid / sugPut * 100).toFixed(2)}% on cash)`],
            ["Cost basis if assigned", `$${(sugPut - putMid).toFixed(2)}`],
          ],
    ...
    // 3. Short Strangle
    {
      ...
      definedRisk: false,
      build: (ctx) => {
        const { sugCall, sugPut, callAtSug, putAtSug, callMid, putMid, FRONT_DTE } = ctx;
        ...
        return {
          ...
          stats: [
            ["Total credit", `$${totalCredit.toFixed(2)}/sh`],
            ["Upper B/E", `$${(sugCall + totalCredit).toFixed(2)}`],
            ["Lower B/E", `$${(sugPut - totalCredit).toFixed(2)}`],
          ],
    ...
    // 4. Iron Condor
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { sugCall, sugPut, callAtSug, putAtSug, callMid, putMid, FRONT_DTE, calls, puts } = ctx;
        const callsAbove = calls.filter(c => c.strike > sugCall);
        const putsBelow = puts.filter(p => p.strike < sugPut);
        const callWing = callsAbove.length ? nearest(callsAbove, sugCall * 1.05) : { strike: sugCall, bid: 0, ask: 0, iv: callAtSug.iv };
        const putWing = putsBelow.length ? nearest(putsBelow, sugPut * 0.95) : { strike: sugPut, bid: 0, ask: 0, iv: putAtSug.iv };
        ...
        const legs = [
          { type: "call", strike: sugCall, qty: -100, premium: callMid, dte: FRONT_DTE, iv: callAtSug.iv },
          { type: "call", strike: callWing.strike, qty: 100, premium: callWingMid, dte: FRONT_DTE, iv: callWing.iv || callAtSug.iv },
          { type: "put", strike: sugPut, qty: -100, premium: putMid, dte: FRONT_DTE, iv: putAtSug.iv },
          { type: "put", strike: putWing.strike, qty: 100, premium: putWingMid, dte: FRONT_DTE, iv: putWing.iv || putAtSug.iv },
        ];
        return {
          legs,
          structure: `Strangle + buy $${callWing.strike.toFixed(2)} call & $${putWing.strike.toFixed(2)} put wings.`,
          stats: [
            ["Net credit", `$${totalCredit.toFixed(2)}/sh`],
            ["Wing widths", `$${callWidth.toFixed(2)} call · $${putWidth.toFixed(2)} put`],
            ["Profit zone", `$${sugPut.toFixed(2)} – $${sugCall.toFixed(2)}`],
            ["Max loss", `$${maxLoss.toFixed(2)}/sh`],
          ],
    ...
    // 5. Short Straddle
    {
      ...
      definedRisk: false,
      build: (ctx) => {
        const { atmCall, atmPut, atmCallMid, atmPutMid, FRONT_DTE } = ctx;
        const totalCredit = atmCallMid + atmPutMid;
        const legs = [
          { type: "call", strike: atmCall.strike, qty: -100, premium: atmCallMid, dte: FRONT_DTE, iv: atmCall.iv },
          { type: "put", strike: atmPut.strike, qty: -100, premium: atmPutMid, dte: FRONT_DTE, iv: atmPut.iv },
        ];
        return {
          legs,
          ...
          stats: [
            ["Total credit", `$${totalCredit.toFixed(2)}/sh`],
            ["Upper B/E", `$${(atmCall.strike + totalCredit).toFixed(2)}`],
            ["Lower B/E", `$${(atmPut.strike - totalCredit).toFixed(2)}`],
          ],
    ...
    // 6. Bull Put Spread
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { sugPut, putAtSug, putMid, FRONT_DTE, puts } = ctx;
        const putsBelow = puts.filter(p => p.strike < sugPut);
        const putWing = putsBelow.length ? nearest(putsBelow, sugPut * 0.95) : { strike: sugPut, bid: 0, ask: 0, iv: putAtSug.iv };
        const putWingMid = midOf(putWing);
        ...
        return {
          ...
          stats: [
            ["Net credit", `$${credit.toFixed(2)}/sh`],
            ["Spread width", `$${width.toFixed(2)}`],
            ["Max loss", `$${maxLoss.toFixed(2)}/sh`],
          ],
    ...
    // 7. Bear Call Spread
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { sugCall, callAtSug, callMid, FRONT_DTE, calls } = ctx;
        const callsAbove = calls.filter(c => c.strike > sugCall);
        const callWing = callsAbove.length ? nearest(callsAbove, sugCall * 1.05) : { strike: sugCall, bid: 0, ask: 0, iv: callAtSug.iv };
        const callWingMid = midOf(callWing);
    ...
    // 8. Calendar Spread (long calendar at ATM)
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { atmCall, atmCallMid, FRONT_DTE } = ctx;
        const BACK_DTE = 30;
        const backPrem = backMonthPremium(atmCallMid, FRONT_DTE, BACK_DTE);
        const debit = backPrem - atmCallMid;
        const legs = [
          { type: "call", strike: atmCall.strike, qty: -100, premium: atmCallMid, dte: FRONT_DTE, iv: atmCall.iv },
          { type: "call", strike: atmCall.strike, qty: 100, premium: backPrem, dte: BACK_DTE, iv: atmCall.iv },
        ];
        return {
          legs,
          structure: `Sell $${atmCall.strike.toFixed(2)} front-week call, buy $${atmCall.strike.toFixed(2)} call ~30 DTE.`,
          stats: [
            ["Net debit", `$${debit.toFixed(2)}/sh (est.)`],
            ["Peak P/L at", `$${atmCall.strike.toFixed(2)} (the strike)`],
            ["Back-month IV", `${(atmCall.iv * 100).toFixed(1)}%`],
            ["Max loss", `$${debit.toFixed(2)}/sh (debit paid)`],
          ],
    ...
    // 9. Diagonal Spread (sell OTM front, buy ATM back)
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { sugCall, callAtSug, callMid, atmCall, atmCallMid, FRONT_DTE } = ctx;
        const BACK_DTE = 30;
        const backPrem = backMonthPremium(atmCallMid, FRONT_DTE, BACK_DTE);
        ...
        return {
          ...
          stats: [
            ["Net debit", `$${debit.toFixed(2)}/sh (est.)`],
            ["Short strike", `$${sugCall.toFixed(2)}`],
            ["Long strike", `$${atmCall.strike.toFixed(2)}`],
            ["Best at expiry", `$${sugCall.toFixed(2)} (short strike)`],
          ],
    ...
    // 10. Jade Lizard
    {
      ...
      definedRisk: false,
      build: (ctx) => {
        const { sugCall, sugPut, callAtSug, putAtSug, callMid, putMid, FRONT_DTE, calls } = ctx;
        ...
        for (const wing of callsAbove.sort((a, b) => a.strike - b.strike)) {
          ...
          if (totalCredit >= callWidth) { chosenWing = wing; break; }
        }
        ...
        return {
          ...
          stats: [
            ["Total credit", `$${totalCredit.toFixed(2)}/sh`],
            ["Call wing width", `$${callWidth.toFixed(2)}`],
            ["Upside risk", noUpside ? "None (credit ≥ width)" : `$${(callWidth - totalCredit).toFixed(2)}/sh`],
            ["Lower B/E", `$${(sugPut - totalCredit).toFixed(2)}`],
          ],
          note: noUpside
            ? "No upside risk: credit covers the call spread. Only loses if stock crashes below the put."
            : "Wider wings would eliminate upside risk. Loses on a rally past the long call.",
        };
      },
    },
    ...
    // 11. Call Ratio Spread (1x2, debit version)
    {
      ...
      definedRisk: false,
      build: (ctx) => {
        const { atmCall, atmCallMid, sugCall, callAtSug, callMid, currentPrice, FRONT_DTE } = ctx;
        ...
        const legs = [
          { type: "call", strike: atmCall.strike, qty: 100, premium: atmCallMid, dte: FRONT_DTE, iv: atmCall.iv },
          { type: "call", strike: sugCall, qty: -200, premium: callMid, dte: FRONT_DTE, iv: callAtSug.iv },
        ];
        return {
          legs,
          structure: `Buy 1 $${atmCall.strike.toFixed(2)} call, sell 2 $${sugCall.toFixed(2)} calls.`,
          stats: [
            ["Net cost", debit >= 0 ? `$${debit.toFixed(2)}/sh debit` : `$${(-debit).toFixed(2)}/sh credit`],
            ["Peak P/L at", `$${peakPrice.toFixed(2)}`],
            ["Max profit", `$${peakPL.toFixed(2)}/sh`],
            ["Upper B/E", `$${upperBE.toFixed(2)}`],
          ],
    ...
    // 12. Long Call — directional bullish, defined risk
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { sugCall, callAtSug, callMid, FRONT_DTE } = ctx;
        const legs = [
          { type: "call", strike: sugCall, qty: 100, premium: callMid, dte: FRONT_DTE, iv: callAtSug.iv },
        ];
        return {
          ...
          stats: [
            ["Cost", `$${callMid.toFixed(2)}/sh ($${(callMid * 100).toFixed(0)}/contract)`],
            ["Break-even", `$${(sugCall + callMid).toFixed(2)}`],
            ["Max loss", `$${callMid.toFixed(2)}/sh`],
            ["Max profit", "Unlimited above break-even"],
          ],
    ...
    // 13. Long Put — directional bearish, defined risk
    {
      ...
      build: (ctx) => {
        ...
        return {
          ...
          stats: [
            ["Cost", `$${putMid.toFixed(2)}/sh`],
            ["Break-even", `$${(sugPut - putMid).toFixed(2)}`],
            ["Max loss", `$${putMid.toFixed(2)}/sh`],
            ["Max profit", `$${(sugPut - putMid).toFixed(2)}/sh if stock goes to zero`],
          ],
    ...
    // 14. Long Straddle — vol expansion play
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { calls, puts, currentPrice, FRONT_DTE } = ctx;
        const atmCall = nearest(calls, currentPrice);
        const atmPut = nearest(puts, currentPrice);
        const cMid = midOf(atmCall), pMid = midOf(atmPut);
        const cost = cMid + pMid;
        const legs = [
          { type: "call", strike: atmCall.strike, qty: 100, premium: cMid, dte: FRONT_DTE, iv: atmCall.iv },
          { type: "put",  strike: atmPut.strike,  qty: 100, premium: pMid, dte: FRONT_DTE, iv: atmPut.iv },
        ];
        return {
          legs,
          structure: `Buy ATM $${atmCall.strike.toFixed(2)} call + buy ATM $${atmPut.strike.toFixed(2)} put.`,
          stats: [
            ["Total cost", `$${cost.toFixed(2)}/sh`],
            ["Upper break-even", `$${(atmCall.strike + cost).toFixed(2)}`],
            ["Lower break-even", `$${(atmPut.strike - cost).toFixed(2)}`],
            ["Max loss", `$${cost.toFixed(2)}/sh if pinned at strike`],
          ],
    ...
    // 15. Long Strangle — wider vol expansion play
    {
      ...
      build: (ctx) => {
        ...
        return {
          ...
          stats: [
            ["Total cost", `$${cost.toFixed(2)}/sh`],
            ["Upper break-even", `$${(sugCall + cost).toFixed(2)}`],
            ["Lower break-even", `$${(sugPut - cost).toFixed(2)}`],
            ["Max loss", `$${cost.toFixed(2)}/sh if pinned between strikes`],
          ],
    ...
    // 16. Bull Call Spread — debit, bullish, defined risk
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { calls, currentPrice, FRONT_DTE } = ctx;
        const longLeg = nearest(calls, currentPrice);
        const shortLeg = nearest(calls.filter(c => c.strike >= currentPrice * 1.03), currentPrice * 1.03)
          || nearest(calls, currentPrice * 1.03);
        const longMid = midOf(longLeg), shortMid = midOf(shortLeg);
        const debit = longMid - shortMid;
        const width = shortLeg.strike - longLeg.strike;
        const legs = [
          { type: "call", strike: longLeg.strike,  qty:  100, premium: longMid,  dte: FRONT_DTE, iv: longLeg.iv  },
          { type: "call", strike: shortLeg.strike, qty: -100, premium: shortMid, dte: FRONT_DTE, iv: shortLeg.iv },
        ];
        return {
          legs,
          structure: `Buy $${longLeg.strike.toFixed(2)} call, sell $${shortLeg.strike.toFixed(2)} call.`,
          stats: [
            ["Net debit", `$${debit.toFixed(2)}/sh`],
            ["Max profit", `$${(width - debit).toFixed(2)}/sh`],
            ["Max loss", `$${debit.toFixed(2)}/sh`],
            ["Break-even", `$${(longLeg.strike + debit).toFixed(2)}`],
          ],
    ...
    // 17. Bear Put Spread — debit, bearish, defined risk
    {
      ...
      definedRisk: true,
      build: (ctx) => {
        const { puts, currentPrice, FRONT_DTE } = ctx;
        const longLeg = nearest(puts, currentPrice);
        const shortLeg = nearest(puts.filter(p => p.strike <= currentPrice * 0.97), currentPrice * 0.97)
          || nearest(puts, currentPrice * 0.97);
        const longMid = midOf(longLeg), shortMid = midOf(shortLeg);
        const debit = longMid - shortMid;
        const width = longLeg.strike - shortLeg.strike;
        const legs = [
          { type: "put", strike: longLeg.strike,  qty:  100, premium: longMid,  dte: FRONT_DTE, iv: longLeg.iv  },
          { type: "put", strike: shortLeg.strike, qty: -100, premium: shortMid, dte: FRONT_DTE, iv: shortLeg.iv },
        ];
        return {
          legs,
    ...
    // 18. Long Call Butterfly — pin-the-strike play
    {
      ...
      build: (ctx) => {
        ...
        const lowerWing = nearest(calls.filter(c => c.strike <= center.strike - wingW), center.strike - wingW)
          || nearest(calls, center.strike - wingW);
        const upperWing = nearest(calls.filter(c => c.strike >= center.strike + wingW), center.strike + wingW)
          || nearest(calls, center.strike + wingW);
        const cMid = midOf(center), lMid = midOf(lowerWing), uMid = midOf(upperWing);
        const debit = lMid + uMid - 2 * cMid;
        const width = center.strike - lowerWing.strike;
        const legs = [
          { type: "call", strike: lowerWing.strike, qty:  100, premium: lMid, dte: FRONT_DTE, iv: lowerWing.iv },
          { type: "call", strike: center.strike,    qty: -200, premium: cMid, dte: FRONT_DTE, iv: center.iv    },
          { type: "call", strike: upperWing.strike, qty:  100, premium: uMid, dte: FRONT_DTE, iv: upperWing.iv },
        ];
        return {
          legs,
          ...
          stats: [
            ["Net debit", `$${debit.toFixed(2)}/sh`],
            ["Max profit", `$${(width - debit).toFixed(2)}/sh at $${center.strike.toFixed(2)}`],
            ["Max loss", `$${debit.toFixed(2)}/sh`],
            ["Profit zone", `$${(lowerWing.strike + debit).toFixed(2)} to $${(upperWing.strike - debit).toFixed(2)}`],
          ],
    ...
    // 19. Iron Butterfly — short body + long wings, credit
    {
      ...
      build: (ctx) => {
        ...
        const callWing = nearest(calls.filter(c => c.strike >= atmK + wingW), atmK + wingW)
          || nearest(calls, atmK + wingW);
        const putWing = nearest(puts.filter(p => p.strike <= atmK - wingW), atmK - wingW)
          || nearest(puts, atmK - wingW);
        const acMid = midOf(atmCall), apMid = midOf(atmPut);
        const cwMid = midOf(callWing), pwMid = midOf(putWing);
        const credit = (acMid + apMid) - (cwMid + pwMid);
        const width = callWing.strike - atmK;
        const legs = [
          { type: "put",  strike: putWing.strike,  qty:  100, premium: pwMid, dte: FRONT_DTE, iv: putWing.iv  },
          { type: "put",  strike: atmK,            qty: -100, premium: apMid, dte: FRONT_DTE, iv: atmPut.iv   },
          { type: "call", strike: atmK,            qty: -100, premium: acMid, dte: FRONT_DTE, iv: atmCall.iv  },
          { type: "call", strike: callWing.strike, qty:  100, premium: cwMid, dte: FRONT_DTE, iv: callWing.iv },
        ];
        return {
          legs,
          ...
          stats: [
            ["Net credit", `$${credit.toFixed(2)}/sh`],
            ["Max profit", `$${credit.toFixed(2)}/sh at $${atmK.toFixed(2)}`],
            ["Max loss", `$${(width - credit).toFixed(2)}/sh`],
            ["Profit zone", `$${(atmK - credit).toFixed(2)} to $${(atmK + credit).toFixed(2)}`],
          ],
    ...
    // 20. Long Risk Reversal — bullish synthetic
    {
      ...
      build: (ctx) => {
        ...
        return {
          ...
          stats: [
            ["Net cost", `${netDebit >= 0 ? "$" : "-$"}${Math.abs(netDebit).toFixed(2)}/sh`],
            ["Upside break-even", `$${(sugCall + Math.max(0, netDebit)).toFixed(2)}`],
            ["Downside risk", `Assignment at $${sugPut.toFixed(2)}`],
    ...
    // 21. Short Risk Reversal — bearish synthetic
    {
      ...
      build: (ctx) => {
        ...
        return {
          ...
          stats: [
            ["Net credit", `${netCredit >= 0 ? "$" : "-$"}${Math.abs(netCredit).toFixed(2)}/sh`],
            ["Downside break-even", `$${(sugPut - Math.max(0, netCredit)).toFixed(2)}`],
            ["Upside risk", `Naked short call above $${sugCall.toFixed(2)} — undefined`],
            ["Max profit", `$${sugPut.toFixed(2)}/sh on stock to zero`],
          ],
    ...
    // 22. Call Ratio Backspread — bullish with vol expansion
    {
      ...
      build: (ctx) => {
        ...
        const longLeg = nearest(calls.filter(c => c.strike >= currentPrice * 1.05), currentPrice * 1.05)
          || nearest(calls, currentPrice * 1.05);
        const sMid = midOf(shortLeg), lMid = midOf(longLeg);
        const net = sMid - 2 * lMid;
        const legs = [
          { type: "call", strike: shortLeg.strike, qty: -100, premium: sMid, dte: FRONT_DTE, iv: shortLeg.iv },
          { type: "call", strike: longLeg.strike,  qty:  200, premium: lMid, dte: FRONT_DTE, iv: longLeg.iv  },
        ];
        return {
          legs,
          structure: `Sell 1 $${shortLeg.strike.toFixed(2)} call + buy 2 $${longLeg.strike.toFixed(2)} calls.`,
          stats: [
            ["Net credit/(debit)", `${net >= 0 ? "+$" : "-$"}${Math.abs(net).toFixed(2)}/sh`],
            ["Max loss zone", `Around $${longLeg.strike.toFixed(2)} at expiration`],
            ["Max profit", "Unlimited above upper break-even"],
          ],
    ...
    // 23. Put Ratio Backspread — bearish with vol expansion
    {
      ...
      build: (ctx) => {
        ...
        const longLeg = nearest(puts.filter(p => p.strike <= currentPrice * 0.95), currentPrice * 0.95)
          || nearest(puts, currentPrice * 0.95);
        const sMid = midOf(shortLeg), lMid = midOf(longLeg);
        const net = sMid - 2 * lMid;
        const legs = [
          { type: "put", strike: shortLeg.strike, qty: -100, premium: sMid, dte: FRONT_DTE, iv: shortLeg.iv },
          { type: "put", strike: longLeg.strike,  qty:  200, premium: lMid, dte: FRONT_DTE, iv: longLeg.iv  },
        ];
        return {
          legs,
          ...
          stats: [
            ["Net credit/(debit)", `${net >= 0 ? "+$" : "-$"}${Math.abs(net).toFixed(2)}/sh`],
            ["Max loss zone", `Around $${longLeg.strike.toFixed(2)} at expiration`],
            ["Max profit", `$${longLeg.strike.toFixed(2)}/sh on stock to zero`],
          ],
    ...
    // 24. Long Synthetic Stock
    {
      ...
      build: (ctx) => {
        ...
        const net = cMid - pMid;
        const legs = [
          { type: "call", strike: atmK, qty:  100, premium: cMid, dte: FRONT_DTE, iv: atmCall.iv },
          { type: "put",  strike: atmK, qty: -100, premium: pMid, dte: FRONT_DTE, iv: atmPut.iv  },
        ];
        return {
          legs,
          structure: `Buy ATM $${atmK.toFixed(2)} call + sell ATM $${atmK.toFixed(2)} put.`,
          stats: [
            ["Net debit/(credit)", `${net >= 0 ? "$" : "-$"}${Math.abs(net).toFixed(2)}/sh`],
            ["Equivalent stock cost", `$${(atmK + net).toFixed(2)}`],
            ["Max profit", "Unlimited"],
            ["Max loss", `$${(atmK + net).toFixed(2)}/sh on stock to zero`],
          ],
    ...
    // 25. Short Synthetic Stock
    {
      ...
      build: (ctx) => {
        ...
        return {
          ...
          stats: [
            ["Net credit/(debit)", `${net >= 0 ? "$" : "-$"}${Math.abs(net).toFixed(2)}/sh`],
            ["Equivalent short price", `$${(atmK + net).toFixed(2)}`],
            ["Max profit", `$${(atmK + net).toFixed(2)}/sh on stock to zero`],
            ["Max loss", "Unlimited above synthetic short price"],
          ],
    ...
    // 26. The Wheel
    {
      ...
      definedRisk: false,
      build: (ctx) => {
        const { sugCall, sugPut, callAtSug, putAtSug, callMid, putMid, currentPrice, FRONT_DTE } = ctx;
        ...
        return {
          ...
          stats: [
            ["Phase 1 premium", `$${putMid.toFixed(2)}/sh`],
            ["Phase 2 premium (next cycle)", `$${callMid.toFixed(2)}/sh`],
            ["Cost basis if assigned", `$${(sugPut - putMid).toFixed(2)}`],
            ["Annualized yield (put leg)", `${annualizedReturn.toFixed(1)}%`],
          ],
